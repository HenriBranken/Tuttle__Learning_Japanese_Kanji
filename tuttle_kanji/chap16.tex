%%--------------------------------------------------------------------
%1--------------------------------------------------------------------
\vspace*{\fill}
\begin{glyph}{Taste (味)}{taste}\label{glyph:taste}
Taste, Flavour.
\end{glyph}

\begin{comps}
\comprtwocone & 口 + 未 \\\hline
\comprthreecone & Mouth + Not Yet (\ref{glyph:not_yet}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Taste} the \remcom{vampires} \remcom{not yet}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ミ (MI)} & \textbf{あじ (aji)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Tasting} \comon{mitigated} by \comkun{ajin}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
味 & \textit{taste} & あじ \mbox{(aji)}\\\hline
気味 & spirit + taste = \textit{feeling; sensation} & キ{\middel}ミ \mbox{(KI{\middel}MI)}\\\hline
意味 & mind + taste = \textit{meaning; significance} & イ{\middel}ミ \mbox{(I{\middel}MI)}\\\hline
\end{somme}

\begin{irrr}
	美味しい & beautiful + taste = \textit{delicious} & おい{\middel}しい \mbox{(oi{\middel}shii)}\\\hline
\end{irrr}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%2--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Measurement (尺)}{measurement}\label{glyph:measurement}
Measurement.\\\hline
This kanji derives from and old Japanese unit of measurement equivalent to about 30.3 cm.\\\hline
It is of most use as a component in other characters.
\end{glyph}

\begin{comps}
\comprtwocone & 丶 + 尸 \\\hline
\comprthreecone & Dot + Corpse/Body \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Measure} the \remcom{dot} on the \remcom{body}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{シャク (SHAKU)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Measure} the \comon{shakuhachi}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
尺八 & measurement + eight = \textit{shakuhachi flute} & シャク{\middel}ハチ \mbox{(SHAKU{\middel}HACHI)}\\\hline
\notyet{尺度} & measurement + degree = \textit{linear measure; scale} & シャク{\middel}ド \mbox{(SHAKU{\middel}DO)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%3--------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Flat (平)}{flat}\label{glyph:flat}
Flat.\\\hline
Extended meanings such as ordinary and peaceful.
\end{glyph}

\begin{comps}
\comprtwocone & 艹 + 干 \\\hline
\comprthreecone & Grass/Herb/Plant + Dry \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Flat} and \remcom{dried} \remcom{herbs}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ヘイ (HEI)} & \textbf{ひら (hira)}\\\hline
\rye{3}{ヘイ is the more common of these readings.}
\pronsrthreecone & ビョウ (BY\={O}) & たい (tai) \\\hline
\end{prons}

\begin{remprons}
\hooglig{The flats} \comkun{he rustled} went \comon{Haywire}.\\\hline
\hooglig{Flats} in \ucomkun{Tailand} will \ucomon{be yours}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
平たい & \textit{flat; level} & ひら{\middel}たい \mbox{(hira{\middel}tai)}\\\hline
平和 & flat + harmony = \textit{peace} & ヘイ{\middel}ワ \mbox{(HEI{\middel}WA)}\\\hline
太平洋 & fat + flat + ocean = \textit{the Pacific Ocean} & タイ{\middel}ヘイ{\middel}ヨウ \mbox{(TAI{\middel}HEI{\middel}Y\={O})}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%4--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Degree (度)}{degree}\label{glyph:degree}
Degree, Extent.\\\hline
This kanji can also mean `times' in the sense of `how many times'.
\end{glyph}

\begin{comps}
\comprtwocone & 凵　+ 一 + 广 + 又ヌ \\\hline
\comprthreecone & Container/Wide-opened mouth + One +Slanted roof + Again \\\hline
\end{comps}

\begin{remglyph}
\hooglig{The extent} of the \remcom{one} \remcom{slanted roof} left my \remcom{mouth} \remcom{wide opened again}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ド (DO)} & \textbf{たび (tabi)}\\\hline
\pronsrthreecone & タク、ト (TAKU, TO) & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{The extent} of \comkun{tubby} \comon{dots} \ucomon{tucked} at the \ucomon{top}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
度々 & degree + degree = \textit{often; over and over} & たび{\middel}たび \mbox{(tabi{\middel}tabi)}\\\hline
一度 & one + degree (times) = \textit{once; one time} & イチ{\middel}ド \mbox{(ICHI{\middel}DO)}\\\hline
丁度 & block + degree = \textit{exactly} & チョウ{\middel}ド \mbox{(CH\={O}{\middel}DO)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%5--------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Edge (際)}{edge}\label{glyph:edge}
Edge, Limit.\\\hline
A secondary meaning has to do with `occasion'.
\end{glyph}

\begin{comps}
\comprtwocone & 阝阜 + 祭 \\\hline
\comprthreecone & Hill/Mound + Festival (\ref{glyph:festival}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{At the edge} of the \remcom{dam} is a \remcom{festival}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{サイ (SAI)} & \textbf{きわ (kiwa)}\\\hline
\rye{3}{The kun-yomi is always voice outside the first position with the exception of 一際 (hito{\middel}kiwa/ひと{\middel}きわ), \textit{`remarkably, conspicuous'}; recall from Entry \ref{glyph:voice} that no kanji becomes voiced following 一 when it is read with its kun-yomi of ひと (hito).}
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Edgy} \comon{psycho} \comkun{key-warden}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
水際 & water + edge = \textit{water's edge; waterside} & みず{\middel}ぎわ \mbox{(mizu{\middel}giwa)}\\\hline
国際 & country + edge = \textit{international} & コク{\middel}サイ \mbox{(KOKU{\middel}SAI)}\\\hline
実際 & actual + edge = \textit{in practice; in actuality} & ジッ{\middel}サイ \mbox{(JIS{\middel}SAI)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%6--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Lend (貸)}{lend}\label{glyph:lend}
Lend.
\end{glyph}

\begin{comps}
\comprtwocone & 代 + 貝 \\\hline
\comprthreecone & Substitute (\ref{glyph:substitute}) + Shellfish \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Lending} \remcom{shellfish} to the \remcom{substitute}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & --- & か (ka) \\\hline
\pronsrthreecone & タイ (TAI) & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Lending} to \comkun{customers} is really a \ucomon{tight} situation.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{As the last two examples show, this kanji can absorb its hiragana accompaniment when forming compounds.  This reading is also frequently voiced when not in the first position.}
貸す & \textit{lend} & か{\middel}す \mbox{(ka{\middel}su)}\\\hline
貸切 & lend + cut = \textit{reserved; chartered} & かし{\middel}きり \mbox{(kashi{\middel}kiri)}\\\hline
貸主 & lend + primary = \textit{lender; landlord} & かし{\middel}ぬし \mbox{(kashi{\middel}nushi)}\\\hline
\end{somme} 
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%7--------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Command (令)}{command}\label{glyph:command}
Command.  Order.\\\hline
This kanji is also used as an honorific prefix in certain compounds.
\end{glyph}

\begin{comps}
\comprtwocone & 人 + 一 + 卩 \\\hline
\comprthreecone & Man/Person + One + Seal/Stamp \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Command} that \remcom{one} \remcom{person} to get his \remcom{seal} of approval.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{レイ (REI)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Command} it to start \comon{raining}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
指令 & finger + command = \textit{order; directive} & シ{\middel}レイ \mbox{(SHI{\middel}REI)}\\\hline
\notyet{号令} & title + command = \textit{command; order} & ゴウ{\middel}レイ \mbox{(G\={O}{\middel}REI)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%8--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Field (野)}{field}\label{glyph:field}
Field.  The wild.\\\hline
Interestingly, this kanji is used to denote a political party in opposition.
\end{glyph}

\begin{comps}
\comprtwocone & 里 + 予 \\\hline
\comprthreecone & Hamlet + Beforehand (\ref{glyph:beforehand}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Fields} in the \remcom{hamlet} must be watered \remcom{beforehand}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ヤ (YA)} & \textbf{の (no)}\\\hline
\rye{3}{This is a tougher character than might be expected; both readings will be encountered frequently, although ヤ appears more often in the second position.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{In the wild} \comon{yachting} with the \comkun{nobs}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
野心 & field + heart = \textit{ambition} & ヤ{\middel}シン \mbox{(YA{\middel}SHIN)}\\\hline
分野 & part + field = \textit{field (of study, etc.)} & ブン{\middel}ヤ \mbox{(BUN{\middel}YA)}\\\hline
\notyet{野原} & field + original = \textit{field(s)} & no{\middel}hara \mbox{(の{\middel}はら)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%9--------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Wide (広)}{wide}\label{glyph:wide}
Wide.  Broad.  Spacious.
\end{glyph}

\begin{comps}
\comprtwocone & 广 + 厶 \\\hline
\comprthreecone & Slanted roof + Self/Private \\\hline
\end{comps}

\begin{remglyph}
\hooglig{The wide} \remcom{slanted roof} is \remcom{private}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{コウ (K\={O})} & \textbf{ひろ (hiro)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Wide} open, \comkun{Hiroshima} was its own \comon{cause} of defeat.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
広い & \textit{wide; spacious} & ひろ{\middel}い \mbox{(hiro{\middel}i)}\\\hline
広島 & wide + island = \textit{Hiroshima} & ひろ{\middel}しま \mbox{(hiro{\middel}shima)}\\\hline
広大 & wide + large = \textit{extensive; immense} & コウ{\middel}ダイ \mbox{(K\={O}{\middel}DAI)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%10-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Stone (石)}{stone}\label{glyph:stone}
Stone.
\end{glyph}

\begin{comps}
\comprtwocone & 石 \\\hline
\comprthreecone & Stone/Rock \\\hline
\end{comps}

\begin{remglyph}
Part of the Kanji radicals (\#{112}).\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{セキ (SEKI)} & \textbf{いし (ishi)}\\\hline
\rye{3}{Both readings are used frequently in compounds, although セキ is more common outside of the first position.}
\pronsrthreecone & シャク、コク (SHAKU, KOKU) & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{The stone}-colored \comkun{Ishihara} test that \ucomon{cocked} my interest was \ucomon{shockingly} \comon{sexy}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
石 & \textit{stone} & いし \mbox{(ishi)}\\\hline
化石 & change + stone = \textit{fossil} & カ{\middel}セキ \mbox{(KA{\middel}SEKI)}\\\hline
一石二鳥 & one + stone + two + bird = \textit{kill two birds with one stone} & イッ{\middel}セキ{\middel}ニ{\middel}チョウ \mbox{(IS{\middel}SEKI{\middel}NI{\middel}CH\={O})}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%11-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{China (漢)}{china}\label{glyph:china}
China, the Han Dynasty.
\end{glyph}

\begin{comps}
\comprtwocone & 水氵氺 + 艹 + 口 + 夫 \\\hline
\comprthreecone & Water + Grass/Herb/Plant + Mouth + Husband (\ref{glyph:husband}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{My Chinese} \remcom{vampire} \remcom{husband} drank \remcom{water} mixed with \remcom{herbs}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{カン (KAN)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{The Chinese} are \comon{cunning}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
漢字 & China + character = \textit{kanji} & カン{\middel}ジ \mbox{(KAN{\middel}JI)}\\\hline
漢方 & China + direction = \textit{Chinese medicine} & カン{\middel}ポウ \mbox{(KAN{\middel}P\={O})}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%12-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Different (別)}{different}\label{glyph:different}
Different.\\\hline
The ideas of separation and farewell.
\end{glyph}

\begin{comps}
\comprtwocone & 口 + 勹 + 刂刁刀ク \\\hline
\comprthreecone & Mouth) + Embracing/Wrap + Knife/Sword \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Different} \remcom{vampires} \remcom{embraced} the \remcom{sword} fights.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ベツ (BETSU)} & \textbf{わか (waka)}\\\hline
\pronsrthreecone & --- & わ (wa) \\\hline
\end{prons}

\begin{remprons}
\hooglig{Different} \ucomkun{wanderers} made \comkun{wakaberry} \comon{bets}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
別れる & \textit{part company; branch off} & わか{\middel}れる \mbox{(waka{\middel}reru)}\\\hline
別々 & different + different = \textit{separate; individual} & ベツ{\middel}ベツ \mbox{(BETSU{\middel}BETSU)}\\\hline
送別 & send + different = \textit{farewell; send-off} & ソウ{\middel}ベツ \mbox{(S\={O}{\middel}BETSU)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%13-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Garden (園)}{garden}\label{glyph:garden}
Garden.
\end{glyph}

\begin{comps}
\comprtwocone & 囗 + 土 + 口 + 衣衤 \\\hline
\comprthreecone & Prison/Enclosure + Earth/Soil + Mouth + Clothes \\\hline
\end{comps}

\begin{remglyph}
\hooglig{The garden} was an \remcom{enclosure} of \remcom{soil} where the first \remcom{opening} was made for \remcom{clothing}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{エン (EN)} & --- \\\hline
\pronsrthreecone & --- & その (sono) \\\hline
\end{prons}

\begin{remprons}
\hooglig{Garden} \comon{entrance} graced with \ucomkun{sonorous} sounds.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{Recall from Entry \ref{glyph:move} that the first two characters of the final compound mean \textit{``animal''} when taken together.}
公園 & public + garden = \textit{park} & コウ{\middel}エン \mbox{(K\={O}{\middel}EN)}\\\hline
学園 & study + garden = \textit{educational institute} & ガク{\middel}エン \mbox{(GAKU{\middel}EN)}\\\hline
動物園 & animal + garden = \textit{zoo} & ドウ{\middel}ブツ{\middel}エン \mbox{(D\={O}{\middel}BUTSU{\middel}EN)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%14-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{All (皆)}{all}\label{glyph:all}
All.
\end{glyph}

\begin{comps}
\comprtwocone & 比 + 白 \\\hline
\comprthreecone & Compare (\ref{glyph:compare}) + White \\\hline
\end{comps}

\begin{remglyph}
\hooglig{All} \remcom{white} \remcom{comparisons}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & --- & \textbf{みな (mina)}\\\hline
\rye{3}{In spoken Japanese, this reading is often pronounced \mbox{みんな} (minna) for emphasis.}
\pronsrthreecone & カイ (KAI) & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{All} \comkun{minatory} \ucomon{kites}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
皆 & \textit{all; everybody} & みな \mbox{(mina)}\\\hline
\notyet{皆様} & all + manner = \textit{``ladies and gentlemen''} & みな{\middel}さま \mbox{(mina{\middel}sama)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%15-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Immediate (即)}{immediate}\label{glyph:immediate}
Immediate; At once.
\end{glyph}

\begin{comps}
\comprtwocone & 卩 + 艮 \\\hline
\comprthreecone & Seal/Stamp + Boundary \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Immediately} \remcom{stop} at the \remcom{boundary}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ソク (SOKU)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Immediately}, the \comon{sokkie} came to an end.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
即日 & immediate + sun (day) = \textit{on the same day} & ソク{\middel}ジツ \mbox{(SOKU{\middel}JITSU)}\\\hline
即死 & immediate + death = \textit{instant death} & ソク{\middel}シ \mbox{(SOKU{\middel}SHI)}\\\hline
即売 & immediate + sell = \textit{sale on the spot} & ソク{\middel}バイ \mbox{(SOKU{\middel}BAI)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%16-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Shop (店)}{shop}\label{glyph:shop}
Shop; Store.
\end{glyph}

\begin{comps}
\comprtwocone & 广 + 卜 + 口 \\\hline
\comprthreecone & Slanted roof + Divining + Mouth \\\hline
\end{comps}

\begin{remglyph}
\hooglig{A shop} specialising in \remcom{divining} about \remcom{vampires} living under \remcom{slanted roofs}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{テン (TEN)} & \textbf{みせ (mise)}\\\hline
\rye{3}{テン is the more common reading.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{The shop} was filled with \comon{tense} \comkun{misery}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
店 & \textit{shop; store} & みせ \mbox{(mise)}\\\hline
開店 & open + shop = \textit{shop/store opening} & カイ{\middel}テン \mbox{(KAI{\middel}TEN)}\\\hline
書店 & write + shop = \textit{bookstore} & ショ{\middel}テン \mbox{(SHO{\middel}TEN)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%17-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Institution (院)}{institution}\label{glyph:institution}
Institution, in the sense of public buildings such as hospitals, or bodies such as legislatures.
\end{glyph}

\begin{comps}
\comprtwocone & 阝阜 + 完 \\\hline
\comprthreecone & Hill/Mound + Perfect\\\hline
\end{comps}

\begin{remglyph}
\hooglig{Institution} \remcom{perfectly} suited for studying the \remcom{hill}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{イン (IN)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Institution} for \comon{ink}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
入院 & enter + institution = \textit{to be admitted to hospital} & ニュウ{\middel}イン \mbox{(NY\={U}{\middel}IN)}\\\hline
学院 & study + institution = \textit{academy} & ガク{\middel}イン \mbox{(GAKU{\middel}IN)}\\\hline
\notyet{病院} & illness + institution = \textit{hospital} & ビョウ{\middel}イン \mbox{(BY\={O}{\middel}IN)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%18-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Climb (登)}{climb}\label{glyph:climb}
Climb.\\\hline
An important secondary meaning has to do with attendance (the first compound offers an example).
\end{glyph}

\begin{comps}
\comprtwocone & 癶 + 豆 \\\hline
\comprthreecone & Footsteps/Departure + Bean/A kind of a vessel (\ref{glyph:bean}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Climb} with your \remcom{footsteps} into a \remcom{kind of a vessel}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{トウ (T\={O})} & --- \\\hline
\pronsrthreecone & ト (TO) & のぼ (nobo) \\\hline
\rye{3}{You are likely to encounter the onyomi in one word: 登山 (ト{\middel}ザン/TO{\middel}ZAN) \textit{``mountain climbing''}, with 山 being voiced.  The kunyomi for this kanji serves as an alternate form of 上る (のぼ{\middel}る/nobo{\middel}ru), which we met in Entry \ref{glyph:upper}.}
\end{prons}

\begin{remprons}
\hooglig{Climbing} is \comon{torture} when there is \ucomkun{nobody} at the \ucomon{top} to help you.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
登校 & climb (attend) + school = \textit{attend school} & トウ{\middel}コウ \mbox{(T\={O}{\middel}K\={O})}\\\hline
\notyet{登場} & climb + place = \textit{make an entrance; show up} & トウ{\middel}ジョウ \mbox{(T\={O}{\middel}J\={O})}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%19-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Original (原)}{original}\label{glyph:original}
Original, in the sense of things in their original or primitive state.\\\hline
Many compounds with this character also convey the idea of a plain or field (the first two examples below, for instance).  Think of this in connection to ``original'' as a place that is unspoiled or wilderness-like.
\end{glyph}

\begin{comps}
\comprtwocone & 厂 + 白 + 小 \\\hline
\comprthreecone & Cliff + White + Small \\\hline
\end{comps}

\begin{remglyph}
\hooglig{The original} \remcom{white} \remcom{small} \remcom{cliff}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ゲン (GEN)} & \textbf{はら (hara)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Originally}, \comon{Genghis-Khan} committed \comkun{hara-kiri}.\\\hline
\end{remprons}

\begin{irrr}
川原 & river + original = \textit{dry riverbed} & かわ{\middel}ら \mbox{(kawa{\middel}ra)}\\\hline
\end{irrr}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
野原 & field + original = \textit{field(s)} & の{\middel}はら \mbox{(no{\middel}hara)}\\\hline
高原 & tall + original = \textit{plateau; tableland} & コウ{\middel}ゲン \mbox{(K\={O}{\middel}GEN)}\\\hline
原子力 & original + child + strength = \textit{atomic energy; nuclear power} & ゲン{\middel}シ{\middel}リョク \mbox{(GEN{\middel}SHI{\middel}RYOKU)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%20-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Late (晩)}{late}\label{glyph:late}
Late, Evening.\\\hline
This kanji also imparts the idea of late in a number of more poetic words---the first two compounds below offer examples.
\end{glyph}

\begin{comps}
\comprtwocone & 日 + 免 \\\hline
\comprthreecone & Sun/Clear + Exemption \\\hline
\end{comps}

\begin{remglyph}
\hooglig{The late-evening} \remcom{sun} was \remcom{exempted} from the rest of the \remcom{day}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{バン (BAN)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Late} \comon{bunnies}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
晩秋 & late + autumn = \textit{late autumn} & バン{\middel}シュウ \mbox{(BAN{\middel}SH\={U})}\\\hline
晩年 & late + year = \textit{one's later years} & バン{\middel}ネン \mbox{(BAN{\middel}NEN)}\\\hline
\notyet{今晩は} & now + late = \textit{``good evening''} & コン{\middel}バン{\middel}は \mbox{(KON{\middel}BAN{\middel}wa)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%21-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Entwine (絡)}{entwine}\label{glyph:entwine}
Entwine, become entangled.
\end{glyph}

\begin{comps}
\comprtwocone & 糸 + 各 \\\hline
\comprthreecone & Thread + Each (\ref{glyph:each}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Entwine} \remcom{each} \remcom{thread}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ラク (RAKU)} & \textbf{から (kara)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Entangled} with \comkun{caramel} \comon{luckily}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
絡む & \textit{entwine; become entangled} & から{\middel}む \mbox{(kara{\middel}mu)}\\\hline
絡み付く & entwine + attach = \textit{cling to; coil around} & から{\middel}み　つ{\middel}く \mbox{(kara{\middel}mi tsu{\middel}ku)}\\\hline
短絡 & short + entwine = \textit{short circuit} & タン{\middel}ラク \mbox{(TAN{\middel}RAKU)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%22-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Express (表)}{express}\label{glyph:express}
Express one's feelings and beliefs, etc.\\\hline
This idea of bringing things into the open extends to this kanji's other primary meanings, namely surfaces and charts.
\end{glyph}

\begin{comps}
\comprtwocone & 衣衤 + ニ \\\hline
\comprthreecone & Clothing + Two \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Expressing} through \remcom{two} pieces of \remcom{clothing}.\\\hline
\hooglig{Expressions} \remcom{double} up with \remcom{clothing}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ヒョウ (HY\={O})} & \textbf{おもて、あらわ (omote, arawa)}\\\hline
\rye{3}{ヒョウ is the more common of these readings.  あらわ forms the two verbs in the second and third examples below; you may recognize these as being pronounced identically to 現れる and 現す from Entry \ref{glyph:present}.  Not surprisingly, the pair here can also be seen written at times as 表われる and 表わす.  Though these characters are similar in some respects, this kanji is used more for the idea of \textit{``expressing''} things such as feelings, thoughts, and opinions.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Expressing} that \comon{he yawns} when \comkun{commotions} go on in the \comkun{Arawaks}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
表 & \textit{surface; front} & おもて \mbox{(omote)}\\\hline
表れる (intr.) & \textit{be expressed} & あらわ{\middel}れる \mbox{(arawa{\middel}reru)}\\\hline
表す (tr.) & \textit{express; reveal} & あらわ{\middel}す \mbox{(arawa{\middel}su)}\\\hline
表現 & express + present = \textit{expression} & ヒョウ{\middel}ゲン \mbox{(HY\={O}{\middel}GEN)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%23-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Type (式)}{type}\label{glyph:type}
The primary meaning is that of a type or style of something (as in the second compound).\\\hline
It is also very commonly used, however, in the sense of ``ceremony'' (as is evident in the final example).\\\hline
Take care with the stroke order of this kanji.
\end{glyph}

\begin{comps}
\comprtwocone & 弋 + 工 \\\hline
\comprthreecone & Shoot + Work/Craft \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Type} of arrow \remcom{shot} that \remcom{works}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{シキ (SHIKI)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Type} of \comon{Sheikh}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
形式 & form + type = \textit{form; formality} & ケイ{\middel}シキ \mbox{(KEI{\middel}SHIKI)}\\\hline
自動式 & self + move + type = \textit{automatic} & ジ{\middel}ドウ{\middel}シキ \mbox{(JI{\middel}D\={O}{\middel}SHIKI)}\\\hline
\notyet{成人式} & become + person + type = \textit{Coming of Age ceremony} & セイ{\middel}ジン{\middel}セキ \mbox{(SEI{\middel}JIN{\middel}SHIKI)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%24-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Passage (通)}{passage}\label{glyph:passage}
The general concept is of passing through and along.\\\hline
This is a very versatile kanji that can mean a variety of things, from ``street'' (it often appears on road signs), to the ideas of thoroughly getting to know a subject or having things go according to plan.
\end{glyph}

\begin{comps}
\comprtwocone & マ + 用 + 辶 \\\hline
\comprthreecone & MA katakana + Utilize + Road/Walk \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Passage} \remcom{utility} \remcom{mother} is \remcom{walking} with.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ツウ (TS\={U})} & \textbf{とお (t\={o})}\\\hline
\rye{3}{In the first position, ツウ is by far the more common reading.  Both occur frequently in the second position, and the kunyomi is often voiced (although is never becomes so when forming verbs).}
\pronsrthreecone & ツ (TSU) & かよ (kayo) \\\hline
\end{prons}

\begin{remprons}
\hooglig{The passage} to \comon{Tsuu'tina} was \comkun{tormenting} because of the \ucomon{tsunami} \comkun{knockout}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
通る (intr.) & \textit{go along; pass by} & とお{\middel}る \mbox{(t\={o}{\middel}ru)}\\\hline
通す (tr.) & \textit{pierce; let through} & とお{\middel}す \mbox{(t\={o}{\middel}su)}\\\hline
大通り & large + passage = \textit{a main street} & おお　どお{\middel}り \mbox{(\={o} d\={o}{\middel}ri)}\\\hline
交通 & mix + passage = \textit{traffic; communication} & コウ{\middel}ツウ \mbox{(K\={O}{\middel}TS\={U})}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%25-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Past (昨)}{past}\label{glyph:past}
Past.
\end{glyph}

\begin{comps}
\comprtwocone & 日 + 丿乀　+ 丨 \\\hline
\comprthreecone & Sun/Clear + Slash + Pole \\\hline
\end{comps}

\begin{remglyph}
\hooglig{The past} \remcom{slashed} the \remcom{sun} with a \remcom{pole}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{サク (SAKU)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{The past} really \comon{sucks} hey.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
昨年 & past + year = \textit{last year} & サク{\middel}ネン \mbox{(SAKU{\middel}NEN)}\\\hline
昨夜 & past + night = \textit{last night} & サク{\middel}ヤ \mbox{(SAKU{\middel}YA)}\\\hline
\notyet{昨今} & past + now = \textit{recently; nowadays} & サッ{\middel}コン \mbox{(SAK{\middel}KON)}\\\hline
\end{somme}

\begin{irrr}
	昨日 & past + sun (day) = \textit{yesterday} & きのう \mbox{(kin\={o})}\\\hline
	\rye{3}{This compound can also be read using the character's normal onyomi (サク{\middel}ジツ/SAKU{\middel}JITSU), giving the word a more formal tone.}
\end{irrr}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%26-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Bundle (束)}{bundle}\label{glyph:bundle}
Bundle, Bunch.\\\hline
Note that the writing order of this kanji does not follow the components as listed above, with ``mouth'' written after the initial horizontal stroke.  It is similar in this respect to the character for \textit{``east''} (東) from Entry \ref{glyph:east}.
\end{glyph}

\begin{comps}
\comprtwocone & 木 + 口 \\\hline
\comprthreecone & Tree + Mouth \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Bunch} of \remcom{vampires} surrounding the \remcom{tree}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ソク (SOKU)} & \textbf{たば (taba)}\\\hline
\pronsrthreecone & --- & つか　(tsuka) \\\hline
\end{prons}

\begin{remprons}
\hooglig{A bundle} of \ucomkun{tsukas} were sent to \comkun{Thaba'NChu} in a \comon{sock}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
束 & \textit{bundle; bunch} & たば \mbox{(taba)}\\\hline
花束 & flower + bunch = \textit{bouquet} & はな{\middel}たば \mbox{(hana{\middel}taba)}\\\hline
結束 & bind + bunch = \textit{union; combination} & ケッ{\middel}ソク \mbox{(KES{\middel}SOKU)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%27-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Special (特)}{special}\label{glyph:special}
Special.
\end{glyph}

\begin{comps}
\comprtwocone & 牛 + 寺 \\\hline
\comprthreecone & Cow + Temple \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Special} \remcom{temple} dedicated to \remcom{cows}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{トク (TOKU)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Special} \comon{talk}-shows.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
特別 & special + different = \textit{special; extraordinary} & トク{\middel}ベツ \mbox{(TOKU{\middel}BETSU)}\\\hline
特売 & special + sell = \textit{bargain; special buy} & トク{\middel}バイ \mbox{(TOKU{\middel}BAI)}\\\hline
特急 & special + hasten = \textit{special express (train)} & トク{\middel}キュウ \mbox{(TOKU{\middel}KY\={U})}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%28-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{True (真)}{true}\label{glyph:true}
True.  Genuine.\\\hline
This kanji often intensifies the quality of the character that follows it.  The irregular readings and the first compound below offer examples of this.
\end{glyph}

\begin{comps}
\comprtwocone & 十 + 具 \\\hline
\comprthreecone & Ten + Tool (\ref{glyph:tool}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{True} \remcom{cross} \remcom{tool}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{シン (SHIN)} & \textbf{ま (ma)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
Truth sheens through the mucky, muddy lies.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
真新しい & true + new = \textit{brand-new} & ま　あたら{\middel}しい \mbox{(ma atara{\middel}shii)}\\\hline
真実 & true + actual = \textit{truth; reality} & シン{\middel}ジツ \mbox{(SHIN{\middel}JITSU)}\\\hline
\end{somme}

\begin{irrr}
	真赤 & true + red = \textit{deep red} & まっ{\middel}か \mbox{(mak{\middel}ka)}\\\hline
	真青 & true + blue = \textit{deep blue} & まっ{\middel}さお \mbox{(mas{\middel}sao)}\\\hline
	\rye{3}{These are the only two words in which the readings of the kanji that follow 真 change in such a radical way.  More common examples include 真白 (mas{\middel}shiro/まっ{\middel}しろ) \textit{``snow white''} and 真暗 (mak{\middel}kura/まっ{\middel}くら) \textit{``pitch dark''}.  You may also see these compounds written as 真っ赤, 真っ青, 真っ白 and 真っ暗.  Keep an eye out as well for a few words such as 真中 (まん{\middel}なか/man{\middel}naka) \textit{``right in the middle''}, which is also sometimes written as 真ん中.}
\end{irrr}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%29-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Office (局)}{office}\label{glyph:office}
Office, Bureau.\\\hline
In a few words this kanji can also mean \textit{``the situation''}, or state of something (the first compound is an example).
\end{glyph}

\begin{comps}
\comprtwocone & 尸 + 口 \\\hline
\comprthreecone & Corpse/Body + Mouth \\\hline
\end{comps}

\begin{remglyph}
\hooglig{The bureau} of \remcom{vampires} were discussing what do to with the \remcom{corpses}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{キョク (KYOKU)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Office} of \comon{Kyokushin} karate.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{Recall from Entry \ref{glyph:road} that the first two kanji of the second compound mean \textit{``water supply system''} when taken together.}
結局 & bind + office (situation) = \textit{after all; in the long run} & ケッ{\middel}キョク \mbox{(KEK{\middel}KYOKU)}\\\hline
水道局 & water supply system + office = \textit{water bureau} & スイ{\middel}ドウ{\middel}キョク \mbox{(SUI{\middel}D\={O}{\middel}KYOKU)}\\\hline
\notyet{薬局} & medicine + office = \textit{pharmacy} & ヤッ{\middel}キョク \mbox{(YAK{\middel}KYOKU)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%30-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Gather (集)}{gather}\label{glyph:gather}
Gather, Assemble.
\end{glyph}

\begin{comps}
\comprtwocone & 隹 + 木 \\\hline
\comprthreecone & Old birds + Tree \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Gathering} \remcom{old birds} in the \remcom{tree}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{シュウ (SH\={U})} & \textbf{あつ (atsu)}\\\hline
\pronsrthreecone & --- & つど (tsudo) \\\hline
\end{prons}

\begin{remprons}
\hooglig{Gather} at the \comkun{atsumori} play with our \comon{shoes} on and \ucomkun{stew dough}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
集まる (intr.) & \textit{gather; come together} & あつ{\middel}まる \mbox{(atsu{\middel}maru)}\\\hline
集める (tr.) & \textit{gather; collect} & あつ{\middel}める \mbox{(atsu{\middel}meru)}\\\hline
集中 & gather + middle = \textit{concentration} & シュウ{\middel}チュウ \mbox{(SH\={U}{\middel}CH\={U})}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%---------------------------------------------------------------------
