...
Measurement.\\\hline
This 漢字 derives from and old Japanese unit of measurement equivalent to about 30.3 cm.\\\hline
...
\rye{3}{The kun-yomi is always voice outside the first position with the exception of 一際 (hito{\middel}kiwa/ひと{\middel}きわ), \textit{`remarkably, conspicuous'}; recall from Entry \ref{glyph:voice} that no 漢字 becomes voiced following 一 when it is read with its kun-yomi of ひと (hito).}
\pronsrthreecone & --- & --- \\\hline
\end{prons}
...
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{As the last two examples show, this 漢字 can absorb its hiragana accompaniment when forming compounds.  This reading is also frequently voiced when not in the first position.}
...
\rye{3}{You are likely to encounter the onyomi in one word: 登山 (ト{\middel}ザン/TO{\middel}ZAN) \textit{``mountain climbing''}, with 山 being voiced.  The kunyomi for this 漢字 serves as an alternate form of 上る (のぼ{\middel}る/nobo{\middel}ru), which we met in Entry \ref{glyph:upper}.}
\end{prons}
...
Office, Bureau.\\\hline
In a few words this 漢字 can also mean \textit{``the situation''}, or state of something (the first compound is an example).
\end{glyph}
