...
\begin{remglyph}
\hooglig{Edging} the \remcom{hill} we found a \remcom{festival}.\\\hline
\end{remglyph}
...
\begin{remglyph}
\hooglig{Type} of \remcom{shooting ceremony} for \remcom{work and crafts}.\\\hline
\end{remglyph}
...
公式&public + type = \textit{official; formal}&コウ{\middel}シキ \mbox{(K\={O}{\middel}SHIKI)}\\\hline
自動式&automatic + type = \textit{automatic}&ジ{\middel}ドウ{\middel}シキ \mbox{(JI{\middel}D\={O}{\middel}SHIKI)}\\\hline
式次第&ceremony + order = \textit{ceremony program}&シキ{\middel}シ{\middel}ダイ \mbox{(SHIKI{\middel}SHI{\middel}DAI)}\\\hline
...
正式&proper + type = \textit{due form}&セイ{\middel}シキ \mbox{(SEI{\middel}SHIKI)}\\\hline
...
\begin{decomp}{7}
此の&報道&は&正式&の&物&だ。\\\hline
この&ホウドウ&は&セイシキ&の&もの&だ\\\hline
this&info&は&formal&の&thing&be/is\\\hline
\multicolumn{7}{|r|}{\textit{This news is official.}}\\\hline
\end{decomp}
...
This versatile 漢字 can mean many things, from \textsuperscript{[1]}\textit{street}, to the ideas of \textsuperscript{[2]}\textit{thoroughly getting to know a subject} or \textsuperscript{[3]}\textit{having things go according to plan}.
\end{glyph}
...
\begin{remglyph}
\hooglig{Passages} for \remcom{mothers} \remcom{utilize} \remcom{roads}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone&\textbf{ツウ (TS\={U})}&\textbf{とお (t\={o})}\\\hline
\rye{3}{とお is never voiced when forming verbs.}
...
%&\mbox{()}\\\hline
\begin{somme}
\splitter{通う}{intr}&\textit{to go to and from (a place)}&かよ{\middel}う \mbox{(kayo{\middel}u)}\\\hline
\splitter{通る}{intr}&\textit{go along; pass by}&とお{\middel}る \mbox{(t\={o}{\middel}ru)}\\\hline
\splitter{通す}{tr}&\textit{to pierce; let through}&とお{\middel}す \mbox{(t\={o}{\middel}su)}\\\hline
通行&passage + go = \textit{passing (of people, traffic)}&ツウ{\middel}コウ \mbox{(TS\={U}{\middel}K\={O})}\\\hline
大通り&large + passage = \textit{a main street}&おお{\wsplit}どお{\middel}り \mbox{(\={o}{\wsplit}d\={o}{\middel}ri)}\\\hline
通信&passage + trust = \textit{correspondence}&ツウ{\middel}シン \mbox{(TS\={U}{\middel}SHIN)}\\\hline
交通&mix + passage = \textit{traffic; intercourse}&コウ{\middel}ツウ \mbox{(K\={O}{\middel}TS\={U})}\\\hline
...
通行止め&traffic passage + stop = \textit{road closure}&ツウ{\middel}コウ{\wsplit}ど{\middel}め \mbox{(TS\={U}{\middel}K\={O}{\wsplit}do{\middel}me)}\\\hline
\notyet{共通}&together + passage = \textit{common; shared}&キョウ{\middel}ツウ \mbox{(KY\={O}{\middel}TS\={U})}\\\hline
\end{somme}
...
\begin{remglyph}
\hooglig{The past} \remcom{day}, \remcom{although} a while back, has been tough.\\\hline
\end{remglyph}
...
昨秋&past + autumn = \textit{fall of last year}&サク{\middel}シュウ \mbox{(SAKU{\middel}SH\={U})}\\\hline
...
\begin{irrr}
昨日&past + sun (day) = \textit{yesterday}&きのう、サク{\middel}ジツ \mbox{(kin\={o}, SAKU{\middel}JITSU)}\\\hline
\end{irrr}
...
Bundle.  Bunch.\\\hline
The stroke order is similar to that of 東 (\textit{East}, p. \pageref{glyph:east}).
\end{glyph}
...
\begin{remglyph}
\hooglig{Bundle} \remcom{one} \remcom{vampire} with a \remcom{tree} to make bundles of wood.\\\hline
\end{remglyph}
...
\splitter{束ねる}{tr}&\textit{to tie up in a bundle}&たば{\middel}ねる \mbox{(taba{\middel}neru)}\\\hline
...
束の間&bundle + interval = \textit{brief moment}&つか{\wsplit}の{\wsplit}ま \mbox{(tsuka{\wsplit}no{\wsplit}ma)}\\\hline
...
{\iscov}毛束&fur + bundle = \textit{tuft of hair}&け{\middel}たば \mbox{(ke{\middel}taba)}\\\hline
{\iscov}竹束&bamboo + bundle = \textit{bamboo bundle}&たけ{\middel}たば \mbox{(take{\middel}taba)}\\\hline
{\iscov}麦束&wheat + bundle = \textit{wheat sheaf}&むぎ{\middel}たば \mbox{(mugi{\middel}taba)}\\\hline
\end{somme}
...
\begin{remglyph}
\hooglig{Special} \remcom{cows} at the \remcom{temple}.\\\hline
\end{remglyph}
...
%&\mbox{()}\\\hline
\begin{somme}
特に&\textit{particularly; especially}&トク{\middel}に \mbox{(TOKU{\middel}ni)}\\\hline
特有&special + have = \textit{characteristic (of)}&トク{\middel}ユウ \mbox{(TOKU{\middel}Y\={U})}\\\hline
...
特急&special + hasten = \textit{special express train}&トッ{\middel}キュウ \mbox{(TOK{\middel}KY\={U})}\\\hline
...
特大&special + large = \textit{extra-large; king-size}&トク{\middel}ダイ \mbox{(TOKU{\middel}DAI)}\\\hline
特集&special + gather = \textit{special edition}&トク{\middel}シュウ \mbox{(TOKU{\middel}SH\={U})}\\\hline
\notyet{特長}&special + long = \textit{forte; merit}&トク{\middel}チョウ \mbox{(TOKU{\middel}CH\={O})}\\\hline
\end{somme}
...
True.  Genuine.\\\hline
This 漢字 often intensifies the quality of the character that follows it.
\end{glyph}
\gindex{True}{真}\strindex{10}{真}

\begin{comps}
\comprtwocone&十 + 具\\\hline
\comprthreecone&Ten + Tool (p. \pageref{glyph:tool})\\\hline
\end{comps}

\begin{remglyph}
\hooglig{A True} \remcom{cross} \remcom{tool}.\\\hline
\end{remglyph}
...
真新しい&true + new = \textit{brand-new}&ま{\wsplit}あたら{\middel}しい \mbox{(ma{\wsplit}atara{\middel}sh\={i})}\\\hline
...
真っ直ぐ&true + straight = \textit{straight (ahead)}&まっ{\middel}す{\middel}ぐ \mbox{(mas{\middel}su{\middel}gu)}\\\hline
\end{somme}
...
Office.  Bureau.\\\hline
In a few words this 漢字 can also mean \textit{the situation}, or \textit{the state of something}.
\end{glyph}
...
%&\mbox{()}\\\hline
\begin{somme}
結局&bind + office = \textit{after all; in the long run}&ケッ{\middel}キョク \mbox{(KEK{\middel}KYOKU)}\\\hline
水道局&water supply system + office = \textit{water bureau}&スイ{\middel}ドウ{\middel}キョク \mbox{(SUI{\middel}D\={O}{\middel}KYOKU)}\\\hline
局地的&locality + target = \textit{localized; isolated}&キョクチテキ \mbox{(KYOKU{\middel}CHI{\middel}TEKI)}\\\hline
局名&office + name = \textit{station name}&キョク{\middel}メイ \mbox{(KYOKU{\middel}MEI)}\\\hline
局面一転&office + reversal = \textit{situation taking a new turn}&キョク{\middel}メン{\middel}イッ{\middel}テン (KYOKU{\middel}MEN{\middel}IT{\middel}TEN)\\\hline
...
\begin{decomp}{4}
私&は&結局&失敗した。\\\hline
わたし&は&ケッキョク&シッパイした\\\hline
I&は&after all&to fail\\\hline
\multicolumn{4}{|r|}{\textit{I failed after all.}}\\\hline
\end{decomp}
\end{multicols}
...
The idea of bringing things into the open, for example, \textit{surfaces} and \textit{charts}.\\\hline
In some respects, this character is similar to 現 (\textit{Present}, p. \pageref{glyph:present}).  However, 表 relates more to expressing things such as \textit{feelings}, \textit{thoughts}, and \textit{opinions}.
\end{glyph}
\gindex{Express}{表}\strindex{8}{表}

\begin{comps}
\comprtwocone&主 + 衣衤\\\hline
\comprthreecone&Primary p. \pageref{glyph:primary} + Clothing\\\hline
\end{comps}
...
%&\mbox{()}\\\hline
\begin{somme}
\rye{3}{Compare the verb pair with the one for 現 on p. \pageref{glyph:present}.}
表&\textit{surface; front}&おもて \mbox{(omote)}\\\hline
\splitter{表れる}{intr}&\textit{be expressed}&あらわ{\middel}れる \mbox{(arawa{\middel}reru)}\\\hline
\splitter{表す}{tr}&\textit{to express/reveal (sth)}&あらわ{\middel}す \mbox{(arawa{\middel}su)}\\\hline
表現&express + present = \textit{expression}&ヒョウ{\middel}ゲン \mbox{(HY\={O}{\middel}GEN)}\\\hline
表記&express + note = \textit{expression in writing}&ヒョウ{\middel}キ \mbox{(HY\={O}{\middel}KI)}\\\hline
表紙&express + paper = \textit{cover; binding}&ヒョウ{\middel}シ \mbox{(HY\={O}{\middel}SHI)}\\\hline
表面的&express + mask + target = \textit{superficial}&ヒョウ{\middel}メン{\middel}テキ \mbox{(HY\={O}{\middel}MEN{\middel}TEKI)}\\\hline
表口&express + mouth = \textit{front door}&おもて{\middel}ぐち \mbox{(omote{\middel}guchi)}\\\hline
表通り&surface + passage = \textit{main street}&おもて{\wsplit}どお{\middel}り \mbox{(omote{\wsplit}d\={o}{\middel}ri)}\\\hline
...
表&か&裏&か。\\\hline
ヒョウ&か&うら&か\\\hline
heads&か&tails&か\\\hline
\multicolumn{4}{|r|}{\textit{Heads ot tails?}}\\\hline
\end{decomp}
\end{multicols}
...
\begin{center}
	...
	{
{\includegraphics[width=3.2cm]{taste.pdf}} &
{\includegraphics[width=3.2cm]{measurement.pdf}} &
{\includegraphics[width=3.2cm]{flat.pdf}} &
{\includegraphics[width=3.2cm]{degree.pdf}} &
{\includegraphics[width=3.2cm]{edge.pdf}} \\
{\includegraphics[width=3.2cm]{lend.pdf}} &
{\includegraphics[width=3.2cm]{command.pdf}} &
{\includegraphics[width=3.2cm]{field.pdf}} &
{\includegraphics[width=3.2cm]{wide.pdf}} &
{\includegraphics[width=3.2cm]{stone.pdf}} \\
{\includegraphics[width=3.2cm]{china.pdf}} &
{\includegraphics[width=3.2cm]{different.pdf}} &
{\includegraphics[width=3.2cm]{garden.pdf}} &
{\includegraphics[width=3.2cm]{all.pdf}} &
{\includegraphics[width=3.2cm]{immediate.pdf}} \\
{\includegraphics[width=3.2cm]{shop.pdf}} &
{\includegraphics[width=3.2cm]{institution.pdf}} &
{\includegraphics[width=3.2cm]{climb.pdf}} &
{\includegraphics[width=3.2cm]{original.pdf}} &
{\includegraphics[width=3.2cm]{late.pdf}} \\
{\includegraphics[width=3.2cm]{entwine.pdf}} &
{\includegraphics[width=3.2cm]{express.pdf}} &
{\includegraphics[width=3.2cm]{type.pdf}} &
{\includegraphics[width=3.2cm]{passage.pdf}} &
{\includegraphics[width=3.2cm]{past.pdf}} \\
{\includegraphics[width=3.2cm]{bundle.pdf}} &
{\includegraphics[width=3.2cm]{special.pdf}} &
{\includegraphics[width=3.2cm]{true.pdf}} &
{\includegraphics[width=3.2cm]{office.pdf}} &
{\includegraphics[width=3.2cm]{gather.pdf}} \\
	};
	\end{tikzpicture}
\end{center}
\vspace*{\fill}