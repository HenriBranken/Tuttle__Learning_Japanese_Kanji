...
\pronsrtwocone & \textbf{フ (FU)} & \textbf{つ (tsu)}\\\hline
    ...
    kanji that 付 will sometimes becomes voiced as づ: 気付く (キ　づ{\middel}く: `to
...
\begin{somme}
    ...
    長文  & mourn + literature = \textit{funeral address}  & チョウ{\middel}ブン \mbox{(CH\={O}{\middel}BUN)}  \\\hline
...
Silver.  Recall from Entry \ref{glyph:gold} that when the character for gold functions as a component in other kanji, it means ``metal'' in general.
\end{glyph}
...
\begin{remglyph}
 Part of the Kanji radicals (\#{83}).\\\hline
\end{remglyph}
...
\pronsrtwocone & \textbf{ショ (SHO)} & \textbf{ところ (tokoro)}\\\hline
    ...
    only a few words in which this kanji is preceded by a verb in its る
...
This kanji is usually used as a prefix for numbers, much as English uses ``No.'', or ``\#''.  In a few compounds it can also mean ``exam''.
\end{glyph}
...
\begin{remglyph}
Part of the Kanji radicals (\#{76}).\\\hline
\end{remglyph}
...
\begin{remglyph}
Part of the Kanji radicals (\#{113}).\\\hline
\end{remglyph}
...
% &  ()\\\hline
\begin{somme}
\rye{3}{This kanji is unusual in that its 音読み almost always forms compounds
...
\rye{3}{These 2 readings only occur in a few words, and always ``double up''
    with the 音読み of the kanji that follows them (as the small katakana ッ
    indicates).\\\hline
