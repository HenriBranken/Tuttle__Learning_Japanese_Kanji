...
\begin{remglyph}
\hooglig{Serve} that \remcom{person} \remcom{gentlemen}.\\\hline
\end{remglyph}
...
Match.  Combine.\\\hline
This is a 漢字 you will encounter frequently, as it forms many compound verbs (such as in the third example) that suggest some kind of reciprocal action between people or things.  These words are often translated into English with the phrase ``each other'': 押し合う (お{\middel}し　あ{\middel}う) ``to push each other'', is one example.
\end{glyph}
...
\begin{remglyph}
\hooglig{Match} that \remcom{person's} one \remcom{mouth}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ゴウ (G\={O})} & \textbf{あ (a)}\\\hline
\rye{3}{ゴウ is the most common choice in the first position, but both it and あ occur frequently in second position.\\\hline
...
The compound 合宿 (ガッ{\middel}シュク) ``to lodge together'', from the 漢字 `match' and `lodging' is an example.}
\end{prons}
