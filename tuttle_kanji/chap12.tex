...
Silver.\\\hline
Recall from Entry \ref{glyph:gold} that when the character for gold functions as a component in other 漢字, it means ``metal'' in general.
\end{glyph}
...
In some words (the second compound below is an example), this 漢字 can convey the sense of ``problem''.
\end{glyph}
...
\begin{decomp}{5}
何&が&問題な&の&か\\\hline
なに&が&モンダイな&の&か\\\hline
what&が&problem&の&か\\\hline
\multicolumn{5}{|r|}{\textit{What is the problem?}}\\\hline
\end{decomp}
\end{multicols}
\vspace*{\fill}
\pagebreak
...
\begin{remglyph}
\hooglig{Dark} \remcom{sun} \remcom{sounds}.\\\hline
\end{remglyph}
...
\rye{3}{This reading appears in only one compound.  The word, however, is commonly used: 絵画 (カイ{\middel}ガ) ``pictures and paintings'', composed of, naturally enough, the 漢字 for `picture' and `painting' (Entry \ref{glyph:painting}).}
\end{prons}
...
Surname.\\\hline
This 漢字 can also mean ``clan'' in some instances, as well as convey a more official-sounding form of ``Mr.'' when it is attached to a family name (as in the final compound below).
\end{glyph}
...
{\warn}Uji is a city in Ky\={o}to.\\\hline
\end{remprons}
...
Cause/Reason.\\\hline
This is written in the same order as the 漢字 for ``rice field'' (Entry \ref{glyph:rice_field}), but with the third stroke lengthened.
\end{glyph}
