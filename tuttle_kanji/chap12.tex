...
words---from \textit{setting} something on fire, to \textit{turning} on a light or an electrical device.
\end{glyph}
...
when this 漢字 is preceded by another \mbox{漢字} that is read with its verb
stem: 見付ける (み　つ{\middel}ける: \textit{to find}) is one example, as み is the stem
of the verb 見る.\\\hline
It is only when the preceding reading does not form a verb with its
漢字 that 付 will sometimes becomes voiced as づ: 気付く (キ　づ{\middel}く: \textit{to
notice}) and 名付ける (な　づ{\middel}ける: \textit{to name}) are examples of this, as 気
...
見付ける (tr.) & see + attach = \textit{to find} & み　つける \mbox{(mi tsu{\middel}keru)} \\\hline
気付く (tr.) & spirit + attach = \textit{to notice (sth.)} & キ　づ{\middel}く \mbox{(KI zu{\middel}ku)} \\\hline
名付ける (tr.) & name + attach = \textit{to name} & な　つ{\middel}ける \mbox{(na zu{\middel}keru)} \\\hline
...
\begin{remglyph}
\hooglig{Mourn} for the　beautiful \remcom{bow} that turned into a plain \remcom{pole}.\\\hline
\end{remglyph}
...
\begin{remglyph}
\hooglig{Crying} can be described as \remcom{water} \remcom{standing} on your face.\\\hline
\end{remglyph}
