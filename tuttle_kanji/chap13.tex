...
\begin{remglyph}
\hooglig{Intervene} the \remcom{people} with a \remcom{slashing} \remcom{pole}.\\\hline
\end{remglyph}
...
\comprthreecone&Ice/Two Dots + Lack/Gap/Defect\\\hline
\end{comps}

\begin{remglyph}
\hooglig{Next} is the \remcom{ice} \remcom{lack}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone&\textbf{ジ (JI)}&\textbf{つ、つぎ (tsu, tsugi)}\\\hline
\pronsrthreecone&シ (SHI)&---\\\hline
\rye{3}{This reading is only found in:  次第 = `Next' + `Number' = \textit{Order; circumstances} (p. \pageref{glyph:number}).}
\end{prons}
\rindex{JI}{次}
\rindex{tsu}{次}
\rindex{tsugi}{次}
...
\splitter{次ぐ}{intr}&\textit{come after; follow}&つ{\middel}ぐ \mbox{(tsu{\middel}gu)}\\\hline
次々&next + next = \textit{in succession; one by one}&つぎ{\middel}つぎ \mbox{(tsugi{\middel}tsugi)}\\\hline
次いで&\textit{next; secondly; subsequently}&つ{\middel}いで \mbox{(tsu{\middel}ide)}\\\hline
...
次回&next + rotate = \textit{next time (occasion)}&ジ{\middel}カイ \mbox{(JI{\middel}KAI)}\\\hline
次第&next + number = \textit{order; circumstances}&シ{\middel}ダイ \mbox{(SHI{\middel}DAI)}\\\hline
次第に&\textit{gradually progress into a state}&シ{\middel}ダイ{\middel}に \mbox{(SHI{\middel}DAI{\middel}ni)}\\\hline
\notyet{次官}&next + official = \textit{vice-minister}&ジ{\middel}カン \mbox{(JI{\middel}KAN)}\\\hline
\notyet{次期}&next + period = \textit{next term}&ジ{\middel}ギ \mbox{(JI{\middel}GI)}\\\hline
\end{somme}
...
\begin{remglyph}
\hooglig{Lowly} \remcom{people} in \remcom{clans} are the \remcom{ones}.\\\hline
\end{remglyph}
...
低木&fixed + tree = \textit{shrubbery}&タイ{\middel}ボク \mbox{(TEI{\middel}BOKU)}\\\hline
低温&low + warm = \textit{low temperature}&テイ{\middel}オン　\mbox{(TEI{\middel}ON)}\\\hline
...
低空&low + empty = \textit{low altitude; low sky}&テイ{\middel}クウ \mbox{(TEI{\middel}K\={U})}\\\hline
低利&slow + profit = \textit{low interest rate}&タイ{\middel}リ \mbox{(TEI{\middel}RI)}\\\hline
...
%&\mbox{()}\\\hline
\begin{somme}
孝道&filial piety + road = \textit{filial piety}&コウ{\middel}ドウ \mbox{(K\={O}{\middel}D\={O})}\\\hline
至孝&arrive + filial piety = \textit{supreme filial piety}&シ{\middel}コウ \mbox{(SHI{\middel}K\={O})}\\\hline
...
孝女&filial piety + woman = \textit{filial daughter}&コウ{\middel}ジョ \mbox{(K\={O}{\middel}JO)}\\\hline
孝子&filial piety + child = \textit{filial child}&コウ{\middel}シ \mbox{(K\={O}{\middel}SHI)}\\\hline
\end{somme}
\end{multicols}
...
filial piety&incidentally&parents&は&nil\\\hline
...
\rye{3}{This reading does show up in \textit{tea ceremony}, 茶道, サ{\middel}ドウ/SA{\middel}D\={O}.}
\end{prons}
\rindex{CHA}{茶}
\rindex{SA}{茶}
...
日本茶&Japan + tea = \textit{Japanese tea}&ニ{\middel}ホン{\middel}チャ \mbox{(NI{\middel}HON{\middel}CHA)}\\\hline
茶会&tea + meet = \textit{tea party; get together}&チャ{\middel}カイ/サ{\middel}カイ \mbox{(CHA{\middel}KAI/SA{\middel}KAI)}\\\hline
無茶&nil + tea = \textit{absurd}&ム{\middel}チャ/ムッ{\middel}チャ \mbox{(MU(C){\middel}CHA)}\\\hline
茶道&tea + road = \textit{Way of Tea}&サドウ \mbox{(SAD\={O})}\\\hline
番茶&order + tea = \textit{coarse tea}&バン{\middel}チャ　\mbox{(BAN{\middel}CHA)}\\\hline
茶番&tea + order = \textit{farce; charade}&チャ{\middel}バン \mbox{(CHA{\middel}BAN)}\\\hline
茶の間&tea + interval = \textit{Japanese style living room}&チャ{\wsplit}の{\wsplit}ま \mbox{(CHA{\wsplit}no{\wsplit}ma)}\\\hline
...
\begin{remglyph}
\hooglig{Profit} equals the \remcom{grain} \remcom{knifed} off autumn-end.\\\hline
\end{remglyph}
...
元利&origin + profit = \textit{principal and interest}&ガン{\middel}リ \mbox{(GAN{\middel}RI)}\\\hline
...
有利&have + profit = \textit{advantageous}&ユウ{\middel}リ \mbox{(Y\={U}{\middel}RI)}\\\hline
利き手&profit + hand = \textit{one's dominant hand}&き{\middel}きて \mbox{(ki{\middel}kite)}\\\hline
有利&have + profit = \textit{advantageous}&ユウ{\middel}リ　\mbox{(Y\={U}{\middel}RI)}\\\hline
不利&not + profit = \textit{disadvantage}&フ{\middel}リ \mbox{(FU{\middel}RI)}\\\hline
\end{somme}
...
\begin{remglyph}
\hooglig{Incomplete} with just \remcom{one} \remcom{tree}.\\\hline
\end{remglyph}
...
\notyet{未完}&not yet + perfect = \textit{incomplete}&ミ{\middel}カン \mbox{(MI{\middel}KAN)}\\\hline
...
\notyet{未知}&not yet + know = \textit{unknown; strange}&ミ{\middel}チ \mbox{(MI{\middel}CHI)}\\\hline
\notyet{未練}&not yet + train = \textit{lingering attachment}&ミ{\middel}レン \mbox{(MI{\middel}REN)}\\\hline
\notyet{未成年者}&not yet + adult = \textit{minor; under age}&ミ{\middel}セイ{\middel}レン{\middel}シャ \mbox{(MI{\middel}SEI{\middel}NEN{\middel}SHA)}\\\hline
\notyet{未然}&not yet + nature = \textit{before it happens}&ミ{\middel}ゼン \mbox{(MI{\middel}ZEN)}\\\hline
\end{somme}
...
末っ子&end + child = \textit{youngest child}&すえっ{\middel}こ \mbox{(suek{\middel}ko)}\\\hline
...
始末&start + end = \textit{management}&シ{\middel}マツ \mbox{(SHI{\middel}MATSU)}\\\hline
\notyet{末広}&end + wide = \textit{spread out like a fan}&すえ{\middel}ひろ \mbox{(sue{\middel}hiro)}\\\hline
\notyet{末期}&end + period = \textit{last stage}&マッ{\middel}キ \mbox{(MAK{\middel}KI)}\\\hline
\notyet{期末}&period + end = \textit{end of term}&キ{\middel}マツ \mbox{(KI{\middel}MATSU)}\\\hline
\notyet{結末}&bind + end = \textit{end; conclusion}&ケツマツ \mbox{(KETSU{\middel}MATSU)}\\\hline
\end{somme}
...
\comprthreecone&Dipper/Volume measure\\\hline
\end{comps}
...
%&\mbox{()}\\\hline
\begin{somme}
一斗&one + Dipper = \textit{18 litres}&イッ{\middel}ト \mbox{IT{\middel}TO}\\\hline
...
五斗米&five + Dipper + rice = \textit{small salary}&ゴ{\middel}ト{\middel}ベイ \mbox{(GO{\middel}TO{\middel}BEI)}\\\hline
五斗米道&small salary + road = \textit{The Way of the Celestial Masters}&ゴ{\middel}ト{\middel}ベイ{\middel}ドウ \mbox{(GO{\middel}TO{\middel}BEI{\middel}D\={O})}\\\hline
南斗&South + Dipper = \textit{The Chinese ``Dipper'' Constellation}&ナン{\middel}ト \mbox{(NAN{\middel}TO)}\\\hline
冬北斗&winter + The Big Dipper = \textit{The Big Dipper seen on a winter night}&ふゆ{\middel}ホク{\middel}ト \mbox{(fuyu{\middel}HOKU{\middel}TO)}\\\hline
\end{somme}
\end{multicols}
...
\begin{comps}
\comprtwocone&方 + ⺅ + 子 + 辶 \\\hline
\comprthreecone&Road/Walk + Direction + Person + Child\\\hline
\end{comps}

\begin{remglyph}
\hooglig{Play} that \remcom{way} with the \remcom{person's} \remcom{child} on the \remcom{road}.\\\hline
\end{remglyph}
...
%&\mbox{()}\\\hline
\begin{somme}
遊び&\textit{playing}&あそ{\middel}び \mbox{(aso{\middel}bi)}\\\hline
遊び場&play + grounds = \textit{Playground}&あそ{\middel}び{\wsplit}ば \mbox{(aso{\middel}bi{\wsplit}ba)}\\\hline
\splitter{遊ぶ}{intr}&\textit{to play}&あそ{\middel}ぶ \mbox{(aso{\middel}bu)}\\\hline
火遊び&fire + play = \textit{playing with fire}&ひ{\wsplit}あそ{\middel}び \mbox{(hi{\wsplit}aso{\middel}bi)}\\\hline
...
\begin{remglyph}
Part of the 漢字 radicals (\#{151}).\\\hline\hooglig{Beans} for the \remcom{one vampire} on the \remcom{grass}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone&---&\textbf{まめ (mame)}\\\hline
\pronsrthreecone&ズ、トウ (ZU, T\={O})&---\\\hline
\rye{3}{ズ　appears only in the word for \textit{soybean}, 大豆 (\mbox{DAI{\middel}ZU}/\mbox{ダイ{\middel}ズ}).\\\hline
トウ is used for the famous word \textit{t\={o}fu}, {豆腐{\notcov}} (\mbox{トウ{\middel}フ}/\mbox{T\={O}{\middel}FU}).}
\end{prons}
\rindex{mame}{豆}
\rindex{ZU}{豆}
\rindex{TOU@T\={O}}{豆}
...
コーヒー豆&coffee + bean = \textit{coffee beans}&コー{\middel}ヒー{\middel}まめ \mbox{(K\={O}{\middel}H\={I}{\middel}mame)}\\\hline
...
年の豆&year + bean = \textit{beans of the bean-scattering ceremony}&とし{\wsplit}の{\wsplit}まめ \mbox{(toshi{\wsplit}no{\wsplit}mame)}\\\hline
金時豆&red + bean = \textit{red-kidney bean}&キン{\middel}とき{\middel}まめ \mbox{(KIN{\middel}toki{\middel}mame)}\\\hline
青豆&blue + bean = \textit{large green soybean}&あお{\middel}まめ \mbox{(ao{\middel}mame)}\\\hline
黒大豆&black + soybean = \textit{black soybean}&くろ{\middel}ダイ{\middel}ズ \mbox{(kuro{\middel}DAI{\middel}ZU)}\\\hline
\end{somme}
...
\begin{center}
	...
	{
{\includegraphics[width=3.2cm]{painting.pdf}} &
{\includegraphics[width=3.2cm]{possible.pdf}} &
{\includegraphics[width=3.2cm]{god.pdf}} &
{\includegraphics[width=3.2cm]{perish.pdf}} &
{\includegraphics[width=3.2cm]{occupy.pdf}} \\
{\includegraphics[width=3.2cm]{society.pdf}} &
{\includegraphics[width=3.2cm]{fixed.pdf}} &
{\includegraphics[width=3.2cm]{paper.pdf}} &
{\includegraphics[width=3.2cm]{appearance.pdf}} &
{\includegraphics[width=3.2cm]{army.pdf}} \\
{\includegraphics[width=3.2cm]{elder_sister.pdf}} &
{\includegraphics[width=3.2cm]{sheep.pdf}} &
{\includegraphics[width=3.2cm]{intervene.pdf}} &
{\includegraphics[width=3.2cm]{platform.pdf}} &
{\includegraphics[width=3.2cm]{hasten.pdf}} \\
{\includegraphics[width=3.2cm]{state.pdf}} &
{\includegraphics[width=3.2cm]{spicy.pdf}} &
{\includegraphics[width=3.2cm]{clothes.pdf}} &
{\includegraphics[width=3.2cm]{lucky.pdf}} &
{\includegraphics[width=3.2cm]{utilize.pdf}} \\
{\includegraphics[width=3.2cm]{next.pdf}} &
{\includegraphics[width=3.2cm]{low.pdf}} &
{\includegraphics[width=3.2cm]{filial_piety.pdf}} &
{\includegraphics[width=3.2cm]{tea.pdf}} &
{\includegraphics[width=3.2cm]{profit.pdf}} \\
{\includegraphics[width=3.2cm]{not_yet.pdf}} &
{\includegraphics[width=3.2cm]{end.pdf}} &
{\includegraphics[width=3.2cm]{dipper.pdf}} &
{\includegraphics[width=3.2cm]{play.pdf}} &
{\includegraphics[width=3.2cm]{bean.pdf}} \\
	};
	\end{tikzpicture}
\end{center}
\vspace*{\fill}
