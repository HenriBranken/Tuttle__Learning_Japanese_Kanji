...
\begin{remglyph}
The \hooglig{shellfish} were roasted by \remcom{cyclops} over a \remcom{volcano}.\\\hline
\end{remglyph}
...
貝 & \textit{shellfish} & かい (kai)\\\hline
    \notyet{生貝} & life + shellfish = \textit{raw shellfish} & なま{\middel}がい (nama{\middel}gai)\\\hline
    \notyet{赤貝} & red + shellfish = \textit{ark shell} & あか{\middel}がい (aka{\middel}gai)\\\hline
\end{somme}
...
\comprthreecone & head/above/lid + Eight (\ref{glyph:eight}) \\\hline
\end{comps}

\begin{remglyph}
    The \hooglig{snail} protected itself under a \remcom{lid} from the \remcom{volcano}.\\\hline
\end{remglyph}
...
六つ & \textit{six (general counter)} & むっ{\middel}つ (mut{\middel}tsu)\\\hline
    \notyet{六円} & six + circle (yen) = \textit{six yen} & ロク{\middel}エン (ROKU{\middel}EN)\\\hline
    \notyet{六時} & six + time = \textit{six o'clock} & ロク{\middel}ジ (ROKU{\middel}JI)\\\hline
\end{somme}
...
女王 & woman + king = \textit{queen} & ジョ{\middel}オウ (JO{\middel}\={O})\\\hline
    \notyet{王国} & king + country = \textit{kingdom} & オウ{\middel}コク (\={O}{\middel}KOKU)\\\hline
    \notyet{王朝} & king + morning = \textit{dynasty} & オウ{\middel}チョウ (\={O}{\middel}CH\={O})\\\hline
    \notyet{王子} & king + child = \textit{prince} & オウ{\middel}ジ (\={O}{\middel}JI)\\\hline
    \notyet{海王星} & sea + king + star = \textit{Neptune} & カイ{\middel}オウ{\middel}セイ (KAI{\middel}\={O}{\middel}SEI)\\\hline
\end{somme}
...
\begin{remglyph}
    \hooglig{Jewels} are carelessly treated by \remcom{kings} as if they were
    mere \remcom{dots}.\\\hline
\end{remglyph}
...
\begin{remprons}
    A \hooglig{jewel} of a dish is \comkun{tamales}.\\\hline
\end{remprons}
...
目玉 & eye + jewel = \textit{eyeball} & め{\middel}だま (me{\middel}dama)\\\hline
    \notyet{水玉} & water + jewel = \textit{water droplet} & みず{\middel}たま (mizu{\middel}tama)\\\hline
    \notyet{火玉} & fire + jewel = \textit{fireball} & ひ{\middel}だま (hi{\middel}dama)\\\hline
    \notyet{雪玉} & snow + jewel = \textit{snowball} & ゆき{\middel}だま (yuki{\middel}dama)\\\hline
\end{somme}
...
\begin{remglyph}
You can win a \hooglig{heart} by \remcom{doting} or using brute force with a
    \remcom{hook}.\\\hline
\end{remglyph}
...
中心 & middle + heart = \textit{center} & チュウ{\middel}シン (CH\={U}{\middel}SHIN)\\\hline
    \notyet{下心} & lower + heart = \textit{ulterior motive} & した{\middel}ごころ (shita{\middel}gokoro)\\\hline
    \notyet{安心} & ease + heart = \textit{peace of mind} & アン{\middel}シン (AN{\middel}SHIN)\\\hline
    \notyet{愛国心} & love + country + heart = \textit{patriotism} & アイ{\middel}コク{\middel}シン (AI{\middel}KOKU{\middel}SHIN)\\\hline
\end{somme}
...
王国 & king + country = \textit{Kingdom} & オウ{\middel}コク (\={O}{\middel}KOKU)\\\hline
    \notyet{全国} & complete + country = \textit{the whole country/nationwide} & ゼン{\middel}コク (ZEN{\middel}KOKU)\\\hline
    \notyet{入国} & enter + country = \textit{to enter a country} & ニュウ{\middel}コク (NY\={U}{\middel}KOKU)\\\hline
    \notyet{国内} & country + inside = \textit{domestic} & コク{\middel}ナイ (KOKU{\middel}NAI)\\\hline
...
\begin{remglyph}
    A \hooglig{completely} \remcom{royal} \remcom{person}.\\\hline
\end{remglyph}
...
全国 & complete + country = \textit{the whole country/nationwide} & ゼン{\middel}コク (ZEN{\middel}KOKU)\\\hline
    \notyet{安全} & ease + complete = \textit{safety} & アン{\middel}ゼン (AN{\middel}ZEN)\\\hline
    \notyet{全校} & complete + school = \textit{the whole school/all schools} & ゼン{\middel}コウ (ZEN{\middel}K\={O})\\\hline
    \notyet{全力} & complete + strength = \textit{all one's power} & ゼン{\middel}リョク (ZEN{\middel}RYOKU)\\\hline
    \notyet{全体} & complete + body = \textit{the whole} & ゼン{\middel}タイ (ZEN{\middel}TAI)\\\hline
    \notyet{全部} & complete + part = \textit{all} & ゼン{\middel}ブ (ZEN{\middel}BU)\\\hline
\end{somme}
...
\begin{remglyph}
    \remcom{Scarecrow}.  [Nickname]\\\hline
\end{remglyph}
...
早口 & early + mouth = \textit{rapid speaking} & はや{\middel}くち (haya{\middel}kuchi)\\\hline
    \notyet{早春} & early + spring = \textit{early spring} & ソウ{\middel}シュン (S\={O}{\middel}SHUN)\\\hline
    \notyet{早朝} & early + morning = \textit{early morning} & ソウ{\middel}チョウ (S\={O}{\middel}CH\={O})\\\hline
\end{somme}
...
上げる (tr./intr.) & \textit{to lift} & あ{\middel}げる (a{\middel}geru)\\\hline
    \notyet{上手} & upper + hand = \textit{skillful} & じょう{\middel}ズ (J\={O}{\middel}ZU)\\\hline
...
上下 & upper + lower = \textit{high and low} & ジョウ{\middel}ゲ (J\={O}{\middel}GE)\\\hline
    \notyet{天下} & heaven + lower = \textit{the whole land} & テン{\middel}カ (TEN{\middel}KA)\\\hline
\end{somme}
...
\comprthreecone & dots + Tree (\ref{glyph:tree}) \\\hline
\end{comps}

\begin{remglyph}
    \hooglig{Rice} \remcom{dots} in the \remcom{tree}.\\\hline
\end{remglyph}
...
中米 & middle + rice = \textit{Central America} & チュウ{\middel}ベイ (CH\={U}{\middel}BEI)\\\hline
    \notyet{北米} & North + rice = \textit{North America} & ホク{\middel}ベイ (HOKU{\middel}BEI)\\\hline
    \notyet{南米} & South + rice = \textit{South America} & ナン{\middel}ベイ (NAN{\middel}BEI)\\\hline
\end{somme}
...
\begin{remglyph}
    \hooglig{Self}-control is lost when \remcom{cyclops} sees a \remcom{dot}.\\\hline
\end{remglyph}
...
自立 & self + stand = \textit{independence} & ジ{\middel}リツ (JI{\middel}RITSU)\\\hline
    \notyet{自分} & self + part = \textit{oneself} & ジ{\middel}ブン (JI{\middel}BUN)\\\hline
    \notyet{自体} & self + body = \textit{in itself} & ジ{\middel}タイ (JI{\middel}TAI)\\\hline
    \notyet{自然} & self + body = \textit{nature} & シ{\middel}ゼン (SHI{\middel}ZEN)\\\hline
\end{somme}
...
国内 & country + inside = \textit{domestic} & コク{\middel}ナイ (KOKU{\middel}NAI)\\\hline
    \notyet{市内} & city + inside = \textit{within the city} & シ{\middel}ナイ (SHI{\middel}NAI)\\\hline
    \notyet{内海} & inside + sea = \textit{inland sea} & ナイ{\middel}カイ (NAI{\middel}KAI)\\\hline
    \notyet{車内} & car + inside = \textit{inside the car} & シャ{\middel}ナイ (SHA{\middel}NAI)\\\hline
    \notyet{町内} & town + inside = \textit{in the town} & チョウ{\middel}ナイ (CH\={O}{\middel}NAI)\\\hline
\end{somme}
...
右下 & right + lower = \textit{lower right} & みぎ{\middel}した (migi{\middel}shita)\\\hline
    \notyet{右手} & right + hand = \textit{right hand} & みぎ{\middel}て (migi{\middel}te)\\\hline
    \notyet{右左} & right + leg = \textit{right leg} & みぎ{\middel}あし (migi{\middel}ashi)\\\hline
\end{somme}
...
有る & \textit{to have} & あ{\middel}る (a{\middel}ru)\\\hline
    \notyet{有力} & have + strength = \textit{powerful} & ユウ{\middel}リョク (Y\={U}{\middel}RYOKU)\\\hline
    \notyet{有名} & have + name = \textit{famous} & ユウ{\middel}メイ (Y\={U}{\middel}MEI)\\\hline
    \notyet{私有} & private + have = \textit{privately owned} & シ{\middel}ユウ (SHI{\middel}Y\={U})\\\hline
    \notyet{公有} & public + have = \textit{publicly owned} & コウ{\middel}ユウ (K\={O}{\middel}Y\={U})\\\hline
    \notyet{国有化} & country + have + change = \textit{nationalization} & コク{\middel}ユウ{\middel}カ (KOKU{\middel}Y\={U}{\middel}KA)\\\hline
\end{somme}
...
人肉 & person + meat = \textit{human flesh} & ジン{\middel}ニク (JIN{\middel}NIKU)\\\hline
    \notyet{牛肉} & cow + meat = \textit{beef} & ギュウ{\middel}ニク (GY\={U}{\middel}NIKU)\\\hline
    \notyet{肉親} & meat + parent = \textit{blood relative} & ニク{\middel}シン (NIKU{\middel}SHIN)\\\hline
    \notyet{肉体} & meat + body = \textit{the body} & ニク{\middel}タイ (NIKU{\middel}TAI)\\\hline
\end{somme}
...
\begin{remglyph}
    \hooglig{Halve} the \remcom{two} \remcom{dots} with this \remcom{pole}.\\\hline
\end{remglyph}
...
半日 & half + sun (day) = \textit{half a day} & ハン{\middel}ニチ (HAN{\middel}NICHI)\\\hline
    \notyet{半分} & half + part = \textit{half} & ハン{\middel}ブン (HAN{\middel}BUN)\\\hline
    \notyet{九時半} & nine + time + half = \textit{nine-thirty} & キュウ{\middel}ジ{\middel}ハン (KY\={U}{\middel}JI{\middel}HAN)\\\hline
    \notyet{半円} & half + circle = \textit{semicircle} & ハン{\middel}エン (HAN{\middel}EN)\\\hline
\end{somme}
...
九月 & nine + moon (month) = \textit{September} & ク{\middel}ガツ (KU{\middel}GATSU)\\\hline
    \notyet{九時} & nine + time = \textit{nine o'clock} & キュウ{\middel}ジ (KU{\middel}JI)\\\hline
\end{somme}
...
小春 & small + spring = \textit{Indian Summer} & こ{\middel}はる (ko{\middel}haru)\\\hline
    \notyet{青春} & Blue + spring = \textit{youth} & セイ{\middel}シュン (SEI{\middel}SHUN)\\\hline
    \notyet{売春} & sell + spring = \textit{prostitution} & バイ{\middel}シュン (BAI{\middel}SHUN)\\\hline
    \notyet{春分} & spring + part = \textit{vernal equinox} & シュン{\middel}ブン (SHUN{\middel}BUN)\\\hline
    \notyet{来春} & come + spring = \textit{next spring} & ライ{\middel}シュン (RAI{\middel}SHUN)\\\hline
\end{somme}
