...
\begin{remglyph}
\hooglig{Approve} \remcom{today} for another \remcom{roll} of scripture.\\\hline
\end{remglyph}
...
是非無い&without fail + nothingness = \textit{unavoidable; inevitable}&ゼ{\middel}ヒ{\middel}な{\middel}い \mbox{(ZE{\middel}HI{\middel}na{\middel}i)}\\\hline
...
When the verb form appears as a suffix at the end of other 漢字, this character gives the compound a sense of \textit{redoing} whatever the first 漢字 represents.\\\hline
Ensure that you can differentiate between 直 and 真 (\textit{True}, Entry \ref{glyph:true}, p. \pageref{glyph:true}).
\end{glyph}
...
直す (tr.)&\textit{to correct/repair/fix}&なお{\middel}す \mbox{(nao{\middel}su)}\\\hline
書き直す&write + straight = \textit{rewrite; write again}&か{\middel}き{\middel}なお{\middel}す \mbox{(ka{\middel}ki{\middel}nao{\middel}su)}\\\hline
直線&straight + line = \textit{straight line}&チョク{\middel}セン \mbox{(CHOKU{\middel}SEN)}\\\hline
{\diff}直ちに&\textit{immediately; at once}&ただ{\middel}ち{\middel}に \mbox{(tada{\middel}chi{\middel}ni)}\\\hline
...
出直し&exit + straight = \textit{adjustment; touch up}&で{\middel}なお{\middel}し \mbox{(de{\middel}nao{\middel}shi)}\\\hline
\end{somme}
...
{\diff}持病&hold + illness = \textit{chronic disease}&ジ{\middel}ビョウ \mbox{(JI{\middel}BY\={O})}\\\hline
...
気の病&spirit + illness = \textit{nervous breakdown}&キ{\middel}の{\middel}やまい \mbox{(KI{\middel}no{\middel}yamai)}\\\hline
...
共同体&cooperation + body = \textit{cooperative body; community}&キョウ{\middel}ドウ{\middel}タイ \mbox{(KY\={O}{\middel}D\={O}{\middel}TAI)}\\\hline
...
公共団体&community + organisation = \textit{public organisation}&コウ{\middel}キョウ{\middel}ダン{\middel}タイ \mbox{(K\={O}{\middel}KY\={O}{\middel}DAN{\middel}TAI)}\\\hline
