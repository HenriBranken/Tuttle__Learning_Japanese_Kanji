%%--------------------------------------------------------------------
%1--------------------------------------------------------------------
\vspace*{\fill}
\begin{glyph}{Floor (階)}{floor}\label{glyph:floor}
Floor, story (of a building).\\\hline
Related to this idea of layers, 階 can also refer to rank or social stratum to which a person or group of people belong to.
\end{glyph}

\begin{comps}
\comprtwocone & 阝阜 + 皆 \\\hline
\comprthreecone & Hill/Mound + All (\ref{glyph:all}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Floor} from which \remcom{all} people can see the \remcom{dam}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{カイ (KAI)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Floor-level} selling \comon{kites}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
一階 & one + floor = \textit{first floor; ground floor} & イッ{\middel}カイ \mbox{(IK{\middel}KAI)}\\\hline
階上 & floor + upper = \textit{upstairs} & カイ{\middel}ジョウ \mbox{(KAI{\middel}J\={O})}\\\hline 
階下 & floor + lower = \textit{downstairs} & カイ{\middel}カ \mbox{(KAI{\middel}KA)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%2--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Point (点)}{point}\label{glyph:point}
Point, mark.
\end{glyph}

\begin{comps}
\comprtwocone & 占 + 火灬 \\\hline
\comprthreecone & Occupy (\ref{glyph:occupy}) + Fire \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Point} \remcom{occupied} with \remcom{fire}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{テン (TEN)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Point} marks a \comon{tense} beginning.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
点火 & point + fire = \textit{ignition} & テン{\middel}カ \mbox{(TEN{\middel}KA)}\\\hline
重点 & heavy + point = \textit{important point; emphasis} & ジュウ{\middel}テン \mbox{(J\={U}{\middel}TEN)}\\\hline
評点 & ice + point = \textit{freezing point} & ヒョウ{\middel}テン \mbox{(HY\={O}{\middel}TEN)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%3--------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Official (官)}{official}\label{glyph:official}
Official, government.  This kanji is often used as a suffix to indicate an official with some sort of governmental or bureaucratic authority.
\end{glyph}

\begin{comps}
\comprtwocone & 丨 + 宀 + 口 \\\hline
\comprthreecone & Pole + Roof + Mouth \\\hline
\end{comps}

\begin{remglyph}
\hooglig{The official} \remcom{impaled} the \remcom{vampire} under the \remcom{roof}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{カン (KAN)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Officials} are very \comon{cunning}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{Recall that the first two kanji of the second compound mean `test' or `examination' (from Entry 439), and that the first two kanji of the final compound mean `foreign policy' (from Entry 185) when taken together.}
教官 & teach + official = \textit{instructor} & キョウ{\middel}カン \mbox{(KY\={O}{\middel}KAN)}\\\hline
試験管 & examination + official = \textit{examiner} & シ{\middel}ケン{\middel}カン \mbox{(SHI{\middel}KEN{\middel}KAN)}\\\hline
外交官 & foreign policy + official = \textit{diplomat} & ガイ{\middel}コウ{\middel}カン \mbox{(GAI{\middel}K\={O}{\middel}KAN)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%4--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Feather (羽)}{feather}\label{glyph:feather}
Feather.\\\hline
This kanji also can also refer to the wings of birds, insects and aeroplanes, etc.
\end{glyph}

\begin{comps}
\comprtwocone & 羽 \\\hline
\comprthreecone & Feather \\\hline
\end{comps}

\begin{remglyph}
Part of the Kanji radicals (\#{124}).\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & --- & \textbf{は、はね (ha, hane)}\\\hline
\pronsrthreecone & ウ (U) & --- \\\hline
\rye{3}{はね tends to be the reading used when the kanji appears on its own, while は is the more common in compounds.}
\end{prons}

\begin{remprons}
\hooglig{Wings} come to a halt when exposed to \ucomon{ooky} \comkun{hanepoot}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
羽 & \textit{feather; wing} & はね \mbox{(hane)}\\\hline
合羽 & match + feather = \textit{raincoat} & カッ{\middel}ぱ \mbox{(KAP{\middel}pa)}\\\hline
\rye{3}{The final compound uses the less common on-yomi of 合 (カッ/KA-); this is the only time 合 is read with this pronunciation.}
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%5--------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Warm (温)}{warm}\label{glyph:warm}
Warm.
\end{glyph}

\begin{comps}
\comprtwocone & 氵 + 日 + 皿 \\\hline
\comprthreecone & Water + Sun + Plate \\\hline
\end{comps}

\begin{remglyph}
\hooglig{A warm} \remcom{plate} under the warm \remcom{sun} warms up the \remcom{water}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{オン (ON)} & \textbf{あたた (atata)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Warming} up while I was \comon{online}, was \comkun{a Tata} car.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
温かい & \textit{warm} & あたた{\middel}かい \mbox{(atata{\middel}kai)}\\\hline
気温 & spirit + warm = \textit{(air) temperature} & キ{\middel}オン \mbox{(KI{\middel}ON)}\\\hline
温泉 & warms + springs = \textit{hot springs; spa} & オン{\middel}セン \mbox{(ON{\middel}SEN)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%6--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Cold (寒)}{cold}\label{glyph:cold}
Cold.
\end{glyph}

\begin{comps}
\comprtwocone & 宀 + 井 + 一 + 八 + 丶 \\\hline
\comprthreecone & Roof + Well/Crib/Community + One + Eight + Dot \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Cold} \remcom{community} of \remcom{8} huddled together under \remcom{one} \remcom{roof} as \remcom{dots} of snow descended. \\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{カン (KAN)} & \textbf{さむ (samu)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Cold} \comkun{Samurai} are very \comon{cunning}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
寒い & \textit{cold} & さむ{\middel}い \mbox{(samu{\middel}i)}\\\hline
寒気 & cold + spirit = \textit{coldness; the cold} & カン{\middel}キ \mbox{(KAN{\middel}KI)}\\\hline
寒冷前線 & cold + cool + before + line = \textit{cold front} & カン{\middel}レイ{\middel}ゼン{\middel}セン \mbox{(KAN{\middel}REI{\middel}ZEN{\middel}SEN)}\\\hline
\rye{3}{Note that the second compound below can also be read as さむ{\middel}ケ/samu{\middel}KE; meaning \textit{`chill (from a sickness, etc.)'}, it uses the less common onyomi of 気.}
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%7--------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Diligence (勉)}{diligence}\label{glyph:diligence}
Diligence.\\\hline
This is the only one of the general-use kanji in which the character for \textit{exemption} is stretched out in such a way.
\end{glyph}

\begin{comps}
\comprtwocone & 免 + 力 \\\hline
\comprthreecone & Exemption/Excuse/Dismissal + Strength \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Diligence} will \remcom{exempt} you from mediocrity and give you \remcom{strength}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ベン (BEN)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Diligence} will \comon{bend} the impossible.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
勉学 & diligence + study = \textit{diligent study} & ベン{\middel}ガク \mbox{(BEN{\middel}GAKU)}\\\hline
勉強 & diligence + strong = \textit{studying} & ベン{\middel}キョウ \mbox{(BEN{\middel}KY\={O})}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%8--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Technique (術)}{technique}\label{glyph:technique}
Technique, Skill.\\\hline
The most striking things about this kanji is that the character for `go' is split in half.  行 is, in fact, unusual in this respect, for with the exception of its appearance in one obscure kanji [桁{\notcov} (けた/keta) meaning \textit{``beam; girder''}] it is the only character in the general-use list that always does this when functioning as a component in other kanji.  As a result, it will be the only one that retains its original meaning despite breaking apart in this way.
\end{glyph}

\begin{comps}
\comprtwocone & 行 + 木 + 丶 \\\hline
\comprthreecone & To go + Tree + Dot \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Techniques} for \remcom{going} up \remcom{trees} to collect their \remcom{leaves}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ジュツ (JUTSU)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Technique} is the English word for \comon{Jutsu}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
美術 & beautiful + technique = \textit{art; the fine arts} & ビ{\middel}ジュツ \mbox{(BI{\middel}JUTSU)}\\\hline
手術 & hand + technique = \textit{surgical operation} & シュ{\middel}ジュツ \mbox{(SHU{\middel}JUTSU)}\\\hline
話術 & speak + technique = \textit{storytelling} & ワ{\middel}ジュツ \mbox{(WA{\middel}JUTSU)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%9--------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Personal (身)}{personal}\label{glyph:personal}
This odd-looking kanji has to do with things `personal', in the sense of someone's own body (as in the first compound) or a specific thing's characteristic (as in the third compound).\\\hline
Take care with the stroke order.
\end{glyph}

\begin{comps}
\comprtwocone & 身 \\\hline
\comprthreecone & Body \\\hline
\end{comps}

\begin{remglyph}
Part of the Kanji radicals (\#{158}).\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{シン (SHIN)} & \textbf{み (mi)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\rye{3}{み predominates in the first position, and シン in the second.}
\end{prons}

\begin{remprons}
It gets \hooglig{personal} when everything in the \comkun{mirror} needs to take on a particular \comon{sheen}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
身体 & personal + body = \textit{the body; physical} & シン{\middel}タイ \mbox{(SHIN{\middel}TAI)}\\\hline
出身 & exit + body = \textit{one's hometown} & シュッ{\middel}シン \mbox{(SHUS{\middel}SHIN)}\\\hline
中身 & middle + body = \textit{contents} & なか{\middel}み \mbox{(naka{\middel}mi)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%10-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Beginning (初)}{beginning}\label{glyph:beginning}
Beginning.
\end{glyph}

\begin{comps}
\comprtwocone & 衤 + 刀 \\\hline
\comprthreecone & Clothing + Sword \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Begin} \remcom{cutting} the \remcom{clothing}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ショ (SHO)} & \textbf{はじ、はつ (haji, hatsu)}\\\hline
\pronsrthreecone & --- & そ (so) \\\hline
\rye{3}{ショ is the most common of these 3 readings.  はじ appears only in the initial word below, and one other: \mbox{初めて} (はじ{\middel}めて/haji{\middel}mete)　\textit{`for the first time'}.  はつ tends to have a more poetic shade of meaning, and appears with only a few compounds in the first position.}
\end{prons}

\begin{remprons}
\hooglig{Beginning} of \comkun{Hajj} we \comon{shop} around for \ucomkun{soft} \comkun{huts} to stay in during our pilgrimage.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
初め & \textit{beginning; outset} & はじ{\middel}め \mbox{(haji{\middel}me)}\\\hline
初雪 & beginning + snow = \textit{first snowfall of the year} & はつ{\middel}ゆき \mbox{(hatsu{\middel}yuki)}\\\hline
初日 & beginning + sun (day) = \textit{first day; opening day} & ショ{\middel}ニチ \mbox{(SHO{\middel}NICHI)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%11-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Distress (困)}{distress}\label{glyph:distress}
Distress.
\end{glyph}

\begin{comps}
\comprtwocone & 囗 + 木 \\\hline
\comprthreecone & Enclosure/Prison + Tree \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Distressed} in the \remcom{wooden} \remcom{prison}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{コン (KON)} & \textbf{こま (koma)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Distress} from all the \comon{cons} resulted in a \comkun{coma}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
困る & \textit{be distressed; suffer} & こま{\middel}る \mbox{(koma{\middel}ru)}\\\hline
困難 & distress + difficult = \textit{distress; difficulty} & コン{\middel}ナン \mbox{(KON{\middel}NAN)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%12-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Put (置)}{put}\label{glyph:put}
Put, place, leave behind.\\\hline
It is worth spending a moment to ensure that you can clearly see the differences between this kanji and the others learned earlier:  直 (\textit{`straight'}, Entry \ref{glyph:straight}), and 真 (\textit{`true'}, Entry \ref{glyph:straight}).
\end{glyph}

\begin{comps}
\comprtwocone & 直 + 罒 \\\hline
\comprthreecone & Straight (\ref{glyph:straight}) + Net \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Put} the \remcom{net} \remcom{straight-away} there.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{チ (CHI)} & \textbf{お (o)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\rye{3}{お is invariably the reading in the first position, with チ more common in the second.}
\end{prons}

\begin{remprons}
\hooglig{Put} behind \comon{cheap} \comkun{obligations}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{As can be seen from the middle compound below, this is another kanji that can absorb its accompanying hiragana.}
置く & \textit{put; place; leave} & お{\middel}く \mbox{(o{\middel}ku)}\\\hline
置場 & put + place = \textit{place (for something)} & おき{\middel}ば \mbox{(oki{\middel}ba)}\\\hline
配置 & distribute + put = \textit{arrangement; layout} & ハイ{\middel}チ \mbox{(HAI{\middel}CHI)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%13-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Solo (個)}{solo}\label{glyph:solo}
This kanji emphasizes the separateness of individual objects (including people).  It is, in fact, used as a general counter for `pieces'---the final compound shows the aspect of this meaning.
\end{glyph}

\begin{comps}
\comprtwocone & 亻 + 固 \\\hline
\comprthreecone & Man/Person + Hard (\ref{glyph:hard}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Solo} \remcom{people} live a \remcom{hard} life.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{コ (KO)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Solo} \comon{Cop}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
個人 & solo + person = \textit{an individual} & コ{\middel}ジン \mbox{(KO{\middel}JIN)}\\\hline
個別 & solo + different = \textit{individually; one by one} & コ{\middel}ベツ \mbox{(KO{\middel}BETSU)}\\\hline
三個 & three + solo = \textit{three (pieces)} & サン{\middel}コ \mbox{(SAN{\middel}KO)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%14-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Become (成)}{become}\label{glyph:become}
Become.\\\hline
Pay close attention to the stroke order of this kanji.
\end{glyph}

\begin{comps}
\comprtwocone & 戈 + 丿 \\\hline
\comprthreecone & Spear/Halberd + Slash \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Become} a \remcom{slashing} \remcom{halberd}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{セイ (SEI)} & \textbf{な (na)}\\\hline
\pronsrthreecone & ジョウ (J\={O}) & --- \\\hline
\rye{3}{The kun-yomi can incorporate its hiragana ending in certain compounds.}
\end{prons}

\begin{remprons}
\hooglig{Become} \comkun{numb} in your \ucomon{jaw} from all the \comon{sailing}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
成る & \textit{become} & な{\middel}る \mbox{(na{\middel}ru)}\\\hline
成年 & become + year = \textit{adult; age of majority} & セイ{\middel}ネン \mbox{(SEI{\middel}NEN)}\\\hline
成立 & become + stand = \textit{formation} & セイ{\middel}リツ \mbox{(SEI{\middel}RITSU)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%15-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Work (働)}{work}\label{glyph:work}
Work.
\end{glyph}

\begin{comps}
\comprtwocone & 亻 + 動 \\\hline
\comprthreecone & Man/Person + Move (\ref{glyph:move}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Working} \remcom{people} \remcom{move}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & --- & \textbf{はたら (hatara)}\\\hline
\pronsrthreecone & ドウ (D\={O}) & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Work} hard delivering oxygen from \ucomon{door} to door in ``\comkun{Hataraku} Saibou''.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
働く & \textit{work} & はたら{\middel}く \mbox{(hatara{\middel}ku)}\\\hline
働き口 & work + mouth = \textit{job; position} & はたら{\middel}き　ぐち \mbox{(hatara{\middel}ki guchi)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%16-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Hall (堂)}{hall}\label{glyph:hall}
This kanji refers to a larger-sized hall, usually in a public building.
\end{glyph}

\begin{comps}
\comprtwocone & 小 + 冖 + 土 \ref{glyph:earth} \\\hline
\comprthreecone & Small + Cover/Crown + Earth \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Halls} \remcom{covered} with \remcom{earth} are of \remcom{small} value.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ドウ (D\={O})} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Hallway's} \comon{door}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
食堂 & eat + hall = \textit{dining hall; cafeteria} & ショク{\middel}ドウ \mbox{(SHOKU{\middel}D\={O})}\\\hline
本堂 & main + hall = \textit{main hall; main temple} & ホン{\middel}ドウ \mbox{(HON{\middel}D\={O})}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%17-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Association (組)}{association}\label{glyph:association}
Association, set.  This kanji has to do with things coming together or being put together in groups.
\end{glyph}

\begin{comps}
\comprtwocone & 糸 + 且 \\\hline
\comprthreecone & Thread + Besides/Moreover (\ref{glyph:besides}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Set} the \remcom{tread} \remcom{beside} the needle.\\\hline
\end{remglyph}

\begin{prons}
\rye{3}{The kunyomi become voiced in the second position according to the rules we learned for 付 in Entry \ref{glyph:attach}.}
\pronsrtwocone & --- & \textbf{く、くみ (ku, kumi)}\\\hline
\pronsrthreecone & ソ (SO) & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Association} \ucomon{solved} the \comkun{cumin} \comkun{coup}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{Note the mix of on- and kun-yomi in the final compound.}
組む & \textit{put together; form} & く{\middel}む \mbox{(ku{\middel}mu)}\\\hline
組合 & association + match = \textit{association; (labor) union} & くみ{\middel}あい \mbox{(kumi{\middel}ai)}\\\hline
番組 & order + association = \textit{a program} & バン{\middel}ぐみ \mbox{(BAN{\middel}gumi)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%18-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Arise (起)}{arise}\label{glyph:arise}
Arise, get up, happen.\\\hline
The kanji for `run' always stretches out like this when appearing on the left of another character as a component.
\end{glyph}

\begin{comps}
\comprtwocone & 走 + 己 \\\hline
\comprthreecone & Run + Oneself \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Arise} \remcom{oneself} and start \remcom{running}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{キ (KI)} & \textbf{お (o)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Arise} for the \comon{kill} you are \comkun{obligated} to carry out.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{The first two examples below function as a normal intransitive/transitive verb pair.}
起こる (intr) & \textit{to happen; come to pass} & お{\middel}こる \mbox{(o{\middel}koru)}\\\hline
起こす (tr) & \textit{to bring out; raise up} & お{\middel}こす \mbox{(o{\middel}kosu)}\\\hline
おきる (intr) & textit{to get up / rise} & お{\middel}きる \mbox{(o{\middel}kiru)}\\\hline
起点 & arise + point = \textit{starting point} & キ{\middel}テン \mbox{(KI{\middel}TEN)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%19-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Dispute (争)}{dispute}\label{glyph:dispute}
Dispute, Argue.
\end{glyph}

\begin{comps}
\comprtwocone & 勹 + 亅 + 彐彑 \\\hline
\comprthreecone & Wrap/Embrace + Hook + Pig's Head/Snout \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Dispute} about who should \remcom{wrap} the \remcom{skewered} \remcom{pig's head}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ソウ (S\={O})} & \textbf{あらそ (araso)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Dispute} \comkun{arouses} \comon{soreness} of heart.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
争う & \textit{dispute; contend} & あらそ{\middel}う \mbox{(araso{\middel}u)}\\\hline
争点 & dispute + point = \textit{point of dispute} & ソウ{\middel}テン \mbox{(S\={O}{\middel}TEN)}\\\hline
内争 & inside + dispute = \textit{internal dispute} & ナイ{\middel}ソウ \mbox{(NAI{\middel}S\={O})}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%20-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Wind (風)}{wind}\label{glyph:wind}
Wind (as in breeze).\\\hline
An important secondary meaning relates to the ideas of \textbf{style} and \textbf{appearance}; the final compound provides an example.
\end{glyph}

\begin{comps}
\comprtwocone & 風 \\\hline
\comprthreecone & Wind \\\hline
\end{comps}

\begin{remglyph}
Part of the Kanji radicals (\#{182}).\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{フウ (F\={U})} & \textbf{かざ、かぜ (kaza, kaze)}\\\hline
\rye{3}{フウ is the most common of the 3 readings, while かざ　appears only in the first position.  かぜ is the reading used when the kanji stand on its own.  かぜ rarely occurs in the first position, but it is as common as フウ when this character is the last in a compound.\\\hline
フウ　is the reading used in compounds realting to style and appearance.}
\pronsrthreecone & フ (FU) & --- \\\hline
\rye{3}{フ is used in \textit{`bath'}: 風呂\notcov (フ{\middel}ロ/FU{\middel}RO), from the kanji `wind' and `spine'.  (`Spine' is not covered in this book.)}
\end{prons}

\begin{remprons}
\hooglig{The wind} in \comkun{Kazakstan} made a \ucomon{foofy} of its \comkun{Kazekage}, which is \comon{food} for thought.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
風 & \textit{wind} & かぜ \mbox{(kaze)}\\\hline
風車 & wind + car = \textit{windmill} & かざ{\middel}ぐるま \mbox{(kaza{\middel}guruma)}\\\hline
台風 & platform + wind = \textit{typhoon} & タイ{\middel}フウ \mbox{(TAI{\middel}F\={U})}\\\hline
日本風 & Japan + wind (style) = \textit{Japanese Style} & ニ{\middel}ホン{\middel}フウ \mbox{(NI{\middel}HON{\middel}F\={U})}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%21-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Pain (痛)}{pain}\label{glyph:pain}
Pain.
\end{glyph}

\begin{comps}
\comprtwocone & 疒 + マ + 用 \\\hline
\comprthreecone & Sickness + MA + Utilize \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Pain} \remcom{utilizes} \remcom{sickness} to hurt \remcom{mother}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ツウ (TS\={U})} & \textbf{いた (ita)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Painful} \comkun{Italy} and \comon{Tsuu'Tina} nations.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{The final compound below uses a less common onyomi for 頭 (\ref{glyph:head}).}
痛い & \textit{painful} & いた{\middel}い \mbox{(ita{\middel}i)}\\\hline
痛み止め & pain + stop = \textit{painkiller} & いた{\middel}み　ど{\middel}め \mbox{(ita{\middel}mi do{\middel}me)}\\\hline
頭痛 & head + pain = \textit{headache} & ズ{\middel}ツウ \mbox{(ZU{\middel}TS\={U})}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%22-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Officer (吏)}{officer}\label{glyph:officer}
An officer, or official, typically one of lower standing.\\\hline
We will find more use for this kanji as a component than as a character on its own.
\end{glyph}

\begin{comps}
\comprtwocone & 一 + 史 \\\hline
\comprthreecone & One + History (\ref{glyph:history}) \\\hline
\end{comps}

\begin{remglyph}
That \hooglig{Officer} is \remcom{one} going down in \remcom{history}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{リ (RI)} & --- \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Officers} \comon{leaping} for joy.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
官吏 & official + officer = \textit{government officer/official} & カン{\middel}リ \mbox{(KAN{\middel}RI)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%23-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Sleep (寝)}{sleep}\label{glyph:sleep}
Sleep.
\end{glyph}

\begin{comps}
\comprtwocone & 宀 + 冖 + 彐彑 + 又 + 爿 \\\hline
\comprthreecone & Roof + Cover + Pig Head/Snout + Again + Bed \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Sleeping} \remcom{again} with my \remcom{piggish head} under the \remcom{cover} of the \remcom{roof} in my \remcom{bed}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{シン (SHIN)} & \textbf{ね (ne)}\\\hline
\rye{3}{The kunyomi is the more common reading here.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{The sleeping} \comkun{necromancer's} deathly \comon{sheen} will put you immediately to sleep.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
寝る & \textit{sleep} & ね{\middel}る \mbox{(ne{\middel}ru)}\\\hline
昼寝 & daytime + sleep = \textit{afternoon nap} & ひる{\middel}ね \mbox{(hiru{\middel}ne)}\\\hline
寝室 & sleep + room = \textit{bedroom} & シン{\middel}シツ \mbox{(SHIN{\middel}SHITSU)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%24-------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Checkpoint (関)}{checkpoint}\label{glyph:checkpoint}
Checkpoint.  Barrier.
\end{glyph}

\begin{comps}
\comprtwocone & 門 + 艹 + 天 \\\hline
\comprthreecone & Gate + Grass + Heaven (\ref{glyph:heaven}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{The checkpoint} of \remcom{heaven's} \remcom{gate} was marked with beautiful green \remcom{grass}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{カン (KAN)} & \textbf{せき (seki)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Checkpoint} made in the \comon{cunning} game of \comkun{Sekiro}: Shadows die twice.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
間 & \textit{checkpoint; barrier} & せき \mbox{(seki)}\\\hline
機関 & mechanism + checkpoint = \textit{engine; organization} & キ{\middel}カン \mbox{(KI{\middel}KAN)}\\\hline
関係 & checkpoint + connection = \textit{relationship; connection} & カン{\middel}ケイ \mbox{(KAN{\middel}KEI)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
%25-------------------------------------------------------------------
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Side (側)}{side}\label{glyph:side}
Side.
\end{glyph}

\begin{comps}
\comprtwocone &  人亻 + 則 \\\hline
\comprthreecone & Man/Person + Rule (\ref{glyph:rule}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{The side} where \remcom{people} \remcom{rule}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ソク (SOKU)} & \textbf{がわ (gawa)}\\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
On the \hooglig{side}, you'll find \comkun{guavas} stuffed in \conom{socks}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
側 & \textit{side} & がわ \mbox{(gawa)}\\\hline
左側 & left + side = \textit{left side} & ひだり{\middel}がわ \mbox{(hidari{\middel}gawa)}\\\hline
側近 & side + near = \textit{close associate} & ソッ{\middel}キン \mbox{(SOK{\middel}KIN)}\\\hline
\end{somme}
\end{multicols}
\vhrulefill{5pt}
\vspace*{\fill}
\pagebreak
%%--------------------------------------------------------------------
