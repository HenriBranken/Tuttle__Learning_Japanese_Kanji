...
\begin{remglyph}
\hooglig{Dispute} about who should \remcom{wrap} the \remcom{pig's head} on the \remcom{pole}.\\\hline
\end{remglyph}
...
%&\mbox{()}\\\hline
\begin{somme}
\specialc{争う\\(intr.)}&\textit{dispute; contend}&あらそ{\middel}う \mbox{(araso{\middel}u)}\\\hline
争い&\textit{dispute; strife}&あらそ{\middel}い \mbox{(araso{\middel}i)}\\\hline
言い争い&speak + dispute = \textit{a quarrel; argument}&い{\middel}い{\middel}あらそ{\middel}い \mbox{(i{\middel}i{\middel}araso{\middel}i)}\\\hline
係争物&contention + matter = \textit{matter in dispute}&ケイ{\middel}ソウ{\middel}ブツ \mbox{(KEI{\middel}S\={O}{\middel}BUTSU)}\\\hline
口争い&mouth + dispute = \textit{quarrelling}&くちあらそい \mbox{(kuchi{\middel}araso{\middel}i)}\\\hline
水争い&water + dispute = \textit{dispute over water rice}&みず{\middel}あらそ{\middel}い \mbox{(mizu{\middel}araso{\middel}i)}\\\hline
...
An important secondary meaning relates to the ideas of \textit{style} and \textit{appearance}.
\end{glyph}
...
\rye{3}{かざ　only appears in the first position.  かぜ is used when the 漢字 stand on its own.\\\hline
...
風通し&wind + passage = \textit{ventilation}&かぜ{\middel}とお{\middel}し \mbox{(kaze{\middel}t\={o}{\middel}shi)}\\\hline
...
{\diff}頭痛&head + pain = \textit{headache}&ズ{\middel}ツウ \mbox{(ZU{\middel}TS\={U})}\\\hline
...
神経痛&nerves + pain = \textit{nerve pain; neuralgia}&シン{\middel}ケイ{\middel}ツウ \mbox{(SHIN{\middel}KEI{\middel}TS\={U})}\\\hline
...
An officer, or official, typically one of lower standing.
\end{glyph}
...
官公吏か&official + public + officer = \textit{officialdom}&カン{\middel}コウ{\middel}リ \mbox{(KAN{\middel}K\={O}{\middel}RI)}\\\hline
\end{somme}
...
寝食い&sleep + eat = \textit{living in idleness}&ね{\middel}グ{\middel}い \mbox{(ne{\middel}GU{\middel}i)}\\\hline
...
\begin{comps}
\comprtwocone&門 + 关\\\hline
\comprthreecone&Gate + Relation\\\hline
\rye{2}{关 = 艹 + 天.  Relation = Grass + Heaven (p. \pageref{glyph:heaven}).  \hooglig{Relations} with \remcom{grass} in \remcom{heaven}.}
\end{comps}
...
無関心&nil + concern = \textit{apathetic}&ム{\middel}カン{\middel}シン \mbox{(MU{\middel}KAN{\middel}SHIN)}\\\hline
