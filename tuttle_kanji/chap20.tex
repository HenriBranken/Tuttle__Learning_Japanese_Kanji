...
共働き&together + work = \textit{husband \&wife working}&とも{\middel}ばたら{\middel}き \mbox{(tomo{\middel}batara{\middel}ki)}\\\hline
働き手&work + hand = \textit{breadwinner}&はたら{\middel}き{\middel}て \mbox{(hatara{\middel}ki{\middel}te)}\\\hline
実働&actual + work = \textit{actual work}&ジツ{\middel}ドウ \mbox{(JITSU{\middel}D\={O})}\\\hline
下働き&lower + work = \textit{assistant; servant}&した{\middel}ばたら{\middel}き \mbox{(shita{\middel}batara{\middel}ki)}\\\hline
悪事を働く&crime + work = \textit{to commit a crime}&あく{\middel}じ{\middel}を{\middel}はたら{\middel}く (AKU{\middel}JI{\middel}wo{\middel}hatara{\middel}ku)\\\hline
働き方&work + direction = \textit{way of work}&はたら{\middel}き{\middel}かた \mbox{(hatara{\middel}ki{\middel}kata)}\\\hline
\end{somme}
...
\rye{2}{Classroom = ⺌小 + 冖 = Small + Cover/Crown.  \hooglig{Classrooms} that are so \remcom{small} you can \remcom{cover} them.}
\end{comps}
...
ユダヤ教会堂&Judaism + church = \textit{synagogue}&ユダヤ{\middel}キョウ{\middel}カイ{\middel}ドウ (YUDAYA{\middel}KY\={O}{\middel}KAI{\middel}D\={O})\\\hline
...
洗礼堂&baptism + hall = \textit{baptistery}&セン{\middel}レイ{\middel}ドウ \mbox{(SEN{\middel}REI{\middel}D\={O})}\\\hline
\end{somme}
\end{multicols}
...
This 漢字 has to do with things \textit{coming together} or \textit{being put together in groups}.
\end{glyph}
...
木組み&tree + association = \textit{wooden framework}&き{\middel}ぐ{\middel}み \mbox{(ki{\middel}gu{\middel}mi)}\\\hline
\end{somme}
\end{multicols}
...
%&\mbox{()}\\\hline
\begin{somme}
\specialc{起こす\\(tr.)}&\textit{to bring out; raise up (sth.)}&お{\middel}こす \mbox{(o{\middel}kosu)}\\\hline
\specialc{起こる\\(intr.)}&\textit{to happen; come to pass}&お{\middel}こる \mbox{(o{\middel}koru)}\\\hline
\specialc{おきる\\(intr.)}&\textit{to get up; rise}&お{\middel}きる \mbox{(o{\middel}kiru)}\\\hline
...
発起人&proposal + person = \textit{originator; promoter}&ホッ{\middel}キ{\middel}ニン \mbox{(HOK{\middel}KI{\middel}NIN)}\\\hline
