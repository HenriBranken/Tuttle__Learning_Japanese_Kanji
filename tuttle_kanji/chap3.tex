...
\comprthreecone & head/above/lid + Mouth (\ref{glyph:mouth}) + Small (\ref{glyph:small}) \\\hline
\end{comps}
...
上京 & upper + capital = \textit{come/go to Tokyo} & ジョウ{\middel}キョウ (J\={O}{\middel}KY\={O})\\\hline
    \notyet{東京} & east + capital = \textit{Tokyo} & トウ{\middel}キョウ (T\={O}{\middel}KY\={O})\\\hline
\end{somme}
...
\begin{remglyph}
    \hooglig{See} that \remcom{person's} \remcom{eyes}.\\\hline
\end{remglyph}
...
月見 & moon + see = \textit{moon viewing} & つき{\middel}み (tsuki{\middel}mi)\\\hline
    \notyet{見失う} & see + lose = \textit{to lose sight of} & み{\middel}うしな{\middel}う (mi{\middel}ushina{\middel}u)\\\hline
    \notyet{外見} & outside + see = \textit{external appearance} & ガイ{\middel}ケン (GAI{\middel}KEN)\\\hline
\end{somme}
...
中止 & middle + stop = \textit{discontinue} & チュウ{\middel}シ (CH\={U}{\middel}SHI)\\\hline
    \notyet{立ち止まる} & stand + stop = \textit{discontinue} & た{\middel}ち　ど{\middel}まる (ta{\middel}chi do{\middel}maru)\\\hline
    \notyet{車止め} & car + stop = \textit{closed to vehicles} & くるま　ど{\middel}め (kuruma do{\middel}me)\\\hline
\end{somme}
...
七十 & seven + ten = \textit{seventy} & なな{\middel}ジュウ (nana{\middel}J\={U})\\\hline
    \notyet{七時} & seven + time = \textit{seven o'clock} & シチ{\middel}ジ (SHICHI{\middel}JI)\\\hline
\end{somme}
...
\comprtwocone & 小 + ノ \\\hline
    \comprthreecone & Small (\ref{glyph:small}) + NO [katakana]/slash\\\hline
\end{comps}
...
少女 & few + woman = \textit{girl} & ショウ{\middel}ジョ (SH\={O}{\middel}JO)\\\hline
    \notyet{少年} & few + year = \textit{boy} & ショウ{\middel}ネン (SH\={O}{\middel}NEN)\\\hline
    \notyet{最小} & most + few = \textit{fewest} & サイ{\middel}ショウ (SAI{\middel}SH\={O})\\\hline
\end{somme}
...
南米 & south + rice = \textit{Southern America} & ナン{\middel}ベイ (NAN{\middel}BEI)\\\hline
    \notyet{最南} & most + south = \textit{southernmost} & サイ{\middel}ナン (SAI{\middel}NAN)\\\hline
    \notyet{南西} & south + west = \textit{southwest} & ナン{\middel}セイ (NAN{\middel}SEI)\\\hline
    \notyet{南東} & south + east = \textit{south east} & ナン{\middel}トウ (NAN{\middel}T\={O})\\\hline
\end{somme}
...
人工 & person + craft = \textit{artificial}  & ジン{\middel}コウ (JIN{\middel}K\={O})\\\hline
    \notyet{人工林} & person + craft + grove = \textit{planted forest} & ジン{\middel}コウ{\middel}リン (JIN{\middel}K\={O}{\middel}RIN)\\\hline
    \notyet{工学} & craft + study = \textit{engineering} & コウ{\middel}ガク (K\={O}{\middel}GAKU)\\\hline
    \notyet{工学士} & craft + study + gentleman = \textit{Bachelor of Engineering} & コウ{\middel}ガク{\middel}シ (K\={O}{\middel}GAKU{\middel}SHI)\\\hline
    \notyet{刀工} & sword + craft = \textit{swordmaker} & トウ{\middel}コウ (T\={O}{\middel}K\={O})\\\hline
\end{somme}
...
\comprtwocone & 一 + ノ + 工\\\hline
    \comprthreecone & One (\ref{glyph:one}) + NO ノ [katakana]/slash + Craft (\ref{glyph:craft})\\\hline
\end{comps}

\begin{remglyph}
    \hooglig{Left}-handed \remcom{slashes} is one \remcom{crafty} move.\\\hline
\end{remglyph}
...
左下 & left + lower = \textit{lower left} & ひだり{\middel}した (hidari{\middel}shita)\\\hline
    \notyet{左手} & left + hand = \textit{left hand} & ひだり{\middel}て (hidari{\middel}te)\\\hline
    \notyet{左足} & left + leg = \textit{left leg} & ひだり{\middel}あし (hidari{\middel}ashi)\\\hline
\end{somme}
...
高山 & tall + mountain = \textit{alpine} & コウ{\middel}ザン (K\={O}{\middel}ZAN)\\\hline
    \notyet{高校} & tall + school = \textit{high school} & コウ{\middel}コウ (K\={O}{\middel}K\={O})\\\hline
    \notyet{円高} & circle (yen) + tall = \textit{strong yen} & エン{\middel}だか (EN{\middel}daka)\\\hline
    \notyet{最高} & most + tall = \textit{highest; the best} & サイ{\middel}コウ (SAI{\middel}K\={O})\\\hline
\end{somme}
...
買上げ & buy + upper = \textit{a purchase} & かい　あ{\middel}げ (kai a{\middel}ge)\\\hline
    \notyet{買い物} & buy + thing = \textit{shopping} & か{\middel}い　もの (ka{\middel}i mono)\\\hline
    \notyet{買い入れる} & buy + enter = \textit{to stock up on} & か{\middel}い　い{\middel}れる (ka{\middel}i i{\middel}reru)\\\hline
    \notyet{買い手} & buy + hand = \textit{buyer} & か{\middel}い　て (ka{\middel}i te)\\\hline
...
\comprthreecone & wilderness + above/head/lid\\\hline
\end{comps}

\begin{remglyph}
    A \hooglig{circular} sun \remcom{above} the \remcom{wilderness}.\\\hline
\end{remglyph}
...
半円 & haf + circle = \textit{semi-circle} & ハン{\middel}エン (HAN{\middel}EN)\\\hline
    \notyet{円安} & yen + ease = \textit{weak yen} & エン{\middel}やす (EN{\middel}yasu)\\\hline
    \notyet{円周} & circle + around = \textit{circumference} & エン{\middel}シュウ (EN{\middel}SH\={U})\\\hline
\end{somme}
...
\comprtwocone & 二 + 儿 \\\hline
    \comprthreecone & Two (\ref{glyph:two}) + man/person [at the bottom of a
    character]\\\hline
\end{comps}

\begin{remglyph}
    \hooglig{Basically} \remcom{two} \remcom{people} are required in a relationship.\\\hline
\end{remglyph}
...
元日 & basis + sun (day) = \textit{New Year's Day} & ガン{\middel}ジツ (GAN{\middel}JITSU)\\\hline
    \notyet{手元} & hand + basis = \textit{at hand} & て{\middel}もと (te{\middel}moto)\\\hline
    \notyet{元気} & basis + spirit = \textit{high spirits} & ゲン{\middel}キ (GEN{\middel}KI)\\\hline
    \notyet{火元} & fire + basis = \textit{origin of a fire} & ひ{\middel}もと (hi{\middel}moto)\\\hline
    \notyet{元来} & basis + come = \textit{by nature} & ガン{\middel}ライ (GAN{\middel}RAI)\\\hline
\end{somme}
...
\begin{remglyph}
    \hooglig{Necklace} \remcom{yourself} with \remcom{grass}.\\\hline
\end{remglyph}
...
自首 & self + neck = \textit{surrender} & ジ{\middel}シュ (JI{\middel}SHU)\\\hline
    \notyet{手首} & hand + neck = \textit{wrist} & て{\middel}くび (te{\middel}kubi)\\\hline
    \notyet{足首} & leg + neck = \textit{ankle} & あし{\middel}くび (ashi{\middel}kubi)\\\hline
\end{somme}
...
歩き回る & walk + rotate = \textit{to ramble about} & ある{\middel}き　まわ{\middel}る (aru{\middel}ki mawa{\middel}ru)\\\hline
    \notyet{歩道} & walk + road = \textit{sidewalk} & ホ{\middel}ドウ (HO{\middel}D\={O})\\\hline
    \notyet{歩行} & walk + go = \textit{walking} & ホ{\middel}コウ (HO{\middel}K\={O})\\\hline
    \notyet{歩行者} & walk + go + individual = \textit{pedestrian} & ホ{\middel}コウ{\middel}シャ (HO{\middel}K\={O}{\middel}SHA)\\\hline
\end{somme}
...
男子 & man + child = \textit{men's} & ダン{\middel}シ (DAN{\middel}SHI)\\\hline
    ...
    \notyet{子牛} & child + cow = \textit{calf} & こ{\middel}うし (ko{\middel}ushi)\\\hline
    \notyet{電子} & electric + child = \textit{electron} & デン{\middel}シ (DEN{\middel}SHI)\\\hline
\end{somme}
...
人好き & person + like = \textit{amiability} & ひと　ず{\middel}き (hito zu{\middel}ki)\\\hline
    \notyet{話し好き} & speak + like = \textit{talkative} & はな{\middel}し　ず{\middel}き (hana{\middel}shi zu{\middel}ki)\\\hline
    \notyet{友好} & friend + like = \textit{friendship} & ユウ{\middel}コウ (Y\={U}{\middel}K\={O})\\\hline
    \notyet{同好} & same + like = \textit{same tastes} & ドウ{\middel}コウ (D\={O}{\middel}K\={O})\\\hline
\end{somme}
...
中古 & middle + old = \textit{secondhand} & チュウ{\middel}コ (CH\={U}{\middel}KO)\\\hline
    \notyet{最古} & most + old = \textit{oldest} & サイ{\middel}コ (SAI{\middel}KO)\\\hline
    \notyet{古語} & old + words = \textit{archaic word} & コ{\middel}ゴ (KO{\middel}GO)\\\hline
    \notyet{古来} & old + come = \textit{time honored} & コ{\middel}ライ (KO{\middel}RAI)\\\hline
    \notyet{古里} & old + hamlet = \textit{old hamlet} & ふる{\middel}さと (furu{\middel}sato)\\\hline
\end{somme}
...
火山 & fire + mountain = \textit{volcano} & カ{\middel}ザン (KA{\middel}SEI)\\\hline
    \notyet{花火} & flower + fire = \textit{fireworks} & はな{\middel}び (hana{\middel}bi)\\\hline
    \notyet{火曜日} & fire + day of the week + sun (day) = \textit{Tuesday} & カ{\middel}ヨウ{\middel}び (KA{\middel}Y\={O}{\middel}bi)\\\hline
    \notyet{火星} & fire + star = \textit{Mars} & カ{\middel}セイ (KA{\middel}SEI)\\\hline
\end{somme}
...
\comprthreecone & overtaking/go + dot + dot\\\hline
\end{comps}
...
冬 & \textit{winter} & ふゆ (fuyu)\\\hline
    \notyet{冬休み} & winter + rest = \textit{winter vacation} & ふゆ　やす{\middel}み (fuyu yasu{\middel}mi)\\\hline
    \notyet{立冬} & stand + winter = \textit{first day of winter} & リッ{\middel}トウ (RIT{\middel}T\={O})\\\hline
    \notyet{冬物} & winter + thing = \textit{winter clothing} & ふゆ{\middel}もの (fuyu{\middel}mono)\\\hline
    \notyet{冬空} & winter + empty = \textit{winter sky} & ふゆ{\middel}ぞら (fuyu{\middel}zora)\\\hline
\end{somme}
