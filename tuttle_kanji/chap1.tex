...
\begin{remglyph}
\hooglig{Bright} \remcom{sun} and \remcom{moon} combination.\\\hline
\end{remglyph}
...
\rye{3}{The second 漢字 incidentally is not covered in this book.  Such
characters will be included only when they form words in which the
pronunciation of the character we are considering has an irregular reading.
Irregular readings containing these 漢字 will be identified with a
star{\notcov}. }
\end{irrr}
...
\begin{remglyph}
\textit{This is how most people feel when their work ends at five o'clock.}  An uncommon 漢字 radical.\\\hline
\end{remglyph}
...
Eye.\\\hline
In some instances it is read with its 訓読み, and indicates the \textit{-th suffix} in words such as ``fourth'' and ``seventh''; it also expresses the related endings of ``first'', ``second'', ``third'', etc.
\end{glyph}
...
This 漢字 can also serves as an abbreviation for China.\\\hline
...
\begin{remglyph}
\hooglig{Middle} \remcom{poles} will kill \remcom{Dracula}.\\\hline
\end{remglyph}
...
{\diff}八日 & eight + sun (day) = \textit{the eighth day of the month} & よう{\middel}か \mbox{(y\={o}{\middel}ka)}\\\hline
...
\begin{prons}
\pronsrtwocone & \textbf{ショウ (SH\={O})} & \textbf{こ、ちい (ko, chii)}\\\hline
