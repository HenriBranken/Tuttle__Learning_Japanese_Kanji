...
When appearing at the end of country names, this 漢字 denotes an individual's nationality.\index{Person　人}
\end{glyph}
...
Two.\\\hline
This 漢字 also includes ideas of ``double'' and ``bi-'', etc.\index{Two　二}
\end{glyph}
...
Three.  Triple.  Tri-, etc.\index{Three　三}
\end{glyph}
...
Sun.  Day.\\\hline
It is the initial 漢字 in the compound for \textit{Japan}.\index{Sun　日}
\end{glyph}
...
White.\\\hline
Shades of \textit{cleanliness} and \textit{purity}.\index{White　白}
\end{glyph}
...
This 漢字 also relates to \textit{openings} in general, from caves and harbours to taps and bottles, for instance.\index{Mouth　口}
\end{glyph}
...
This 漢字 is well-known to Japanese baseball fans as the character used to denote ``innings''.\index{Rotate　回}
\end{glyph}
...
Bright.  Clear.  Light.\index{Bright　明}
\end{glyph}
...
Eye.\\\hline
In some instances it is read with its 訓読み, and indicates the \textit{-th suffix} in words such as ``fourth'' and ``seventh''; it also expresses the related endings of ``first'', ``second'', ``third'', etc.\index{Eye　目}
\end{glyph}
...
Large.  Big.\\\hline
The meaning of \textit{important} and \textit{great} (not necessarily always in a positive sense) can also be implied in words containing this 漢字.\index{Large　大}
\end{glyph}
...
日 is used as the equivalent with respect to Japan.\index{Middle　中}
\end{glyph}
