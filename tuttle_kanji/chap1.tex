\vspace*{\fill}
\begin{glyph}{Mountain (山)}{mountain}\label{glyph:mountain}
Mountain\\\hline
Obvious from the glyph at first glance.
\end{glyph}

\begin{comps}
\comprtwocone & 山 \\\hline
\comprthreecone & Mountain (\ref{glyph:mountain}) \\\hline
\end{comps}

\begin{remglyph}
%Any \remcom{jelly-bean} will turn \hooglig{white} on the \remcom{sun}.\\\hline
---\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{サン (SAN)} & \textbf{やま (yama)} \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
On the \hooglig{mountain}, my \comon{son} played a \comkun{yamaha} piano.\\\hline
\end{remprons}

\begin{somme}
山 & \textit{mountain} & やま (yama) \\\hline
一山 & one + mountain = \textit{a pile of something} & ひと{\middel}やま (hito{\middel}yama)\\\hline
高山 & tall + mountain = \textit{alpine} & コウ{\middel}ザン (K\={O}{\middel}ZAN)\\\hline
火山 & fire + mountain = \textit{volcano} & カ{\middel}ザン (KA{\middel}ZAN)\\\hline
山村 & mountain + village = \textit{mountain village} & サン{\middel}ソン (SAN{\middel}SON)\\\hline
\end{somme}

\begin{sentence}
\ruby{\underline{山}}{やま}に\ruby{\underline{木}}{き}が\underline{ありません}。\\\hline
yama ni ki ga arimasen.\\\hline
(mountain) (tree) (are not)\\\hline
=\textit{There are no trees on the mountain.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%--------------------------------------------------------------------
%--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Person (人)}{person}\label{glyph:person}
Person; Human being. \\\hline
When appearing at the end of country names, this kanji denotes an individual's nationality.
\end{glyph}

\begin{comps}
\comprtwocone & 人 \\\hline
\comprthreecone & Person (\ref{glyph:person}) \\\hline
\end{comps}

\begin{remglyph}
---\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ジン、ニン (JIN, NIN)} & \textbf{ひと (hito)} \\\hline
\pronsrthreecone & --- & --- \\\hline
\rye{3}{\textbf{JIN} tends to signify that a person belongs to a certain subgroup of humanity.  Look for \textbf{JIN} in the first position and at the end of words signifying countries.\\\hline
\textbf{NIN} tends to indicate a person engaged in an activity specified by the kanji preceding it.\\\hline
The \textbf{kun-yomi} becomes always voiced when it appears outside of the first position.}
\end{prons}

\begin{remprons}
That \comon{nincompoop} of a \hooglig{person} \comon{jinxes} himself with \comkun{hit-or-miss} attempts.\\\hline
\end{remprons}

\begin{somme}
人 & \textit{person} & ひと (hito) \\\hline
白人 & white + person = \textit{caucasian} & ハク{\middel}ジン (HAKU{\middel}JIN) \\\hline
人口 & person + mouth = \textit{population} & ジン{\middel}コウ (JIN{\middel}K\={O}) \\\hline
三人 & three + person = \textit{three people} & サン{\middel}ニン (SAN{\middel{NIN}})\\\hline
四人 & four + person = \textit{four people} & よ{\middel}{ニン} (yo{\middel}{NIN})\\\hline
村人 & village + person = \textit{villager} & むら{\middel}{びと} (mura{\middel}bito)\\\hline
外国人 & outside + country + person = \textit{foreigner} & ガイ{\middel}コク{\middel}ジン (GAI{\middel}KOKU{\middel}JIN)\\\hline
\end{somme}

\begin{irrr}
一人 & one + person = \textit{one person} & ひと{\middel}り (hito{\middel}ri)\\\hline
二人 & two + person = \textit{two people} & ふた{\middel}り (futa{\middel}ri)\\\hline
大人 & large + person = \textit{adult} & おとな (otona)\\\hline
\rye{3}{The irregular readings in the first two examples belong to 人.  In the final compound, both readings are irregular.}
\end{irrr}

\begin{sentence}
\underline{あの} \ruby{\underline{人}}{ひと} の\ruby{\underline{手}}{て}\ruby{は}{わ}\ruby{\underline{小}}{ちい}\underline{さい}です。\\\hline
ano hito no te wa chii{\middel}sai desu.\\\hline
(that) (person) (hand) (small).\\\hline
=\textit{That person's hands are small.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%--------------------------------------------------------------------
%--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{One (一)}{one}\label{glyph:one}
One.
\end{glyph}

\begin{comps}
\comprtwocone & 一 \\\hline
\comprthreecone & One (\ref{glyph:one}) \\\hline
\end{comps}

\begin{remglyph}
\hooglig{One} \remcom{bun/patty}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{イチ、イツ (ICHI, ITSU)} & \textbf{ひと (hito)} \\\hline
\pronsrthreecone & --- & --- \\\hline
\rye{3}{When this kanji appears as the \textbf{first} character in a compound it is almost always read with its \textbf{on-yomi}.  \\\hline
Its pronounciation \textbf{ICHI} or \textbf{ITSU} will depend on whether the initial sound of the following kanji is voiced or voiceless}
\end{prons}

\begin{remprons}
\hooglig{One} \comkun{hit-or-miss} attempt to \comon{eat soup} that reduces \comon{itchiness}.\\\hline
\end{remprons}

\begin{somme}
一 & \textit{one} & イチ (ICHI)\\\hline
一つ & \textit{one (general counter)} & ひと{\middel}つ (hito{\middel}tsu)\\\hline
一日 & one + sun (day) = \textit{one day} & イチ{\middel}ニチ (ICHI{\middel}NICHI)\\\hline
一月 & one + moon (month) = \textit{January} & イチ{\middel}ガツ (ICHI{\middel}GATSU)\\\hline
同一 & same + one = \textit{identical} & ドウ{\middel}イツ (D\={O}{\middel}ITSU)\\\hline
一回 & one + rotate = \textit{once} & イッ{\middel}カイ (IK{\middel}KAI)\\\hline
一時 & one + time = \textit{one o'clock} & イチ{\middel}ジ (ICHI{\middel}JI)\\\hline
\end{somme}

\begin{irrr}
一日 & one + sun (day) = \textit{first day of the month} & ついたち (tsuitachi)\\\hline
\rye{3}{The same compound is shown above with a different pronounciation and meaning.  Context will determine which is the appropriate reading to apply.  For example, when 一日 is seen at the top of a newspaper page, it clearly refers to the first day of the month.}
\end{irrr}

\begin{sentence}
\ruby{\underline{一時}}{イチジ}に\ruby{\underline{会}}{あ}\underline{いましょう}。\\\hline
ICHI{\middel}JI ni a{\middel}imash\={o}.\\\hline
(one o'clock) (let's meet)\\\hline
=\textit{Let's meet at one o'clock.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
\vspace*{\fill}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\pagebreak
\begin{multicols}{2}
\begin{glyph}{Two (二)}{two}\label{glyph:two}
Two.  Including the ideas of ``double'' and ``bi-'', etc.
\end{glyph}

\begin{comps}
\comprtwocone & 一 + 一 \\\hline
\comprthreecone & One (\ref{glyph:one}) + One (\ref{glyph:one}) \\\hline
\end{comps}

\begin{remglyph}
---\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ニ (NI)} & \textbf{ふた (futa)} \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\comkun{Who tans} with \hooglig{two} \comon{nicks} in their costume?\\\hline
\end{remprons}

\begin{somme}
ニ & \textit{two} & ニ (NI)\\\hline
二つ & \textit{two (general counter)} & ふた{\middel}つ (futa{\middel}tsu)\\\hline
二月 & two + moon (month) = \textit{February} & ニガ{\middel}ツ (NI{\middel}GATSU)\\\hline
二十 & two + ten = \textit{twenty} & ニ{\middel}ジュウ (NI{\middel}J\={U})\\\hline
二百 & two + hundred = \textit{two hundred} & ニ{\middel}ヒャク (NI{\middel}HYAKU)\\\hline
二時 & two + time = \textit{two o'clock} & ニ{\middel}ジ (NI{\middel}JI)\\\hline
二週間 & two + week + interval = \textit{two weeks} & ニ{\middel}シュウ{\middel}カン (NI{\middel}SH\={U}{\middel}KAN)\\\hline
\end{somme}

\begin{irrr}
二日 & two + sun (day) = \textit{second day of the month} & ふつか (futsuka)\\\hline
二十日 & two + ten + sun (day) = \textit{twentieth day of the month} & はつか (hatsuka)\\\hline
二十歳 & two + ten + annual = \textit{twenty years old} & はたち (hatachi)\\\hline
\end{irrr}

\begin{sentence}
\ruby{\underline{二月}}{ニガツ}の\underline{オーストラリア}\ruby{は}{わ}\ruby{\underline{美}}{うつく}\underline{しい}。\\\hline
NI{\middel}GATSU no \={o}sutoraria wa utsuku{\middel}shii.\\\hline
(February) (Australia) (beautiful)\\\hline
=\textit{Australia is beautiful in February.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Three (三)}{three}\label{glyph:three}
Three/Triple/Tri-, etc.
\end{glyph}

\begin{comps}
\comprtwocone & 一 + 一 + 一 \\\hline
\comprthreecone & One (\ref{glyph:one}) + One (\ref{glyph:one}) + One (\ref{glyph:one}) \\\hline
\end{comps}

\begin{remglyph}
---\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{サン (SAN)} & --- \\\hline
\pronsrthreecone & --- & みっ、み (mi-, mi) \\\hline
\end{prons}

\begin{remprons}
My \hooglig{triplet} \comon{sons} \ucomkun{mirror} one another.\\\hline
\end{remprons}

\begin{somme}
三 & \textit{three} & サン (SAN)\\\hline
三人 & three + person = \textit{three people} & サン{\middel}ニン (SAN{\middel}NIN)\\\hline
三月 & three + moon (month) = \textit{March} & サン{\middel}ガツ (SAN{\middel}GATSU)\\\hline
三時 & three + time = \textit{three o'clock} & サン{\middel}ジ (SAN{\middel}JI)\\\hline
三十 & three + ten = \textit{thirty} & サン{\middel}ジュウ (SAN{\middel}J\={U})\\\hline
三千 & three + thousand = \textit{three thousand} & サン{\middel}ゼン (SAN{\middel}ZEN)\\\hline
三日 & three + sun (day) = \textit{third day of the month} & みっ{\middel}か (mik{\middel}ka)\\\hline
三つ & \textit{three (general counter)} & みっ{\middel}つ (mit{\middel}tsu)\\\hline
\end{somme}

\begin{sentence}
\ruby{\underline{三人}}{サンニン}が\ruby{\underline{早}}{はや}\underline{く} \ruby{\underline{来}}{き}\underline{ました}。\\\hline
SAN{\middel}NIN ga haya{\middel}ku ki{\middel}mashita.\\\hline
(three people) (early) (came).\\\hline
=\textit{Three people came early.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\pagebreak
\begin{multicols}{2}
\begin{glyph}{Sun (日)}{sun}\label{glyph:sun}
Sun, and Day.\\\hline
It is the initial kanji in the compound for Japan.
\end{glyph}

\begin{comps}
\comprtwocone & 日 \\\hline
\comprthreecone & Sun (\ref{glyph:sun}) \\\hline
\end{comps}

\begin{remglyph}
---\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ニチ、ジツ (NICHI, JITSU)} & \textbf{ひ (hi)} \\\hline
\pronsrthreecone & --- & か (ka) \\\hline
\rye{3}{\textbf{NICHI} will be encountered overwhelmingly in the first position (\textbf{hi} only makes a few appearances).\\\hline
\textbf{JITSU} is typically encountered in the second position.\\\hline
\textbf{hi} is typically encountered in the third position.\\\hline
\textbf{ka} is used only to identify the days of the month from 2--10, 14, 17, 20, 24, and 27.}
\end{prons}

\begin{remprons}
Many \ucomkun{customs} are \comkun{heaped} under the \hooglig{sun}, including \comon{nichiren} \comon{ju-jitsu}.\\\hline
\end{remprons}

\begin{somme}
日 & \textit{sun; day} & ひ (hi)\\\hline
一日 & one + day = \textit{one day} & イチ{\middel}ニチ (ICHI{\middel}NICHI) \\\hline
日本 & sun + main = \textit{Japan} & ニ{\middel}ホン、ニッ{\middel}ポン (NI{\middel}HON, NIP{\middel}PON)\\\hline
休日 & rest + day = \textit{holiday} & キュウ{\middel}ジツ (KY\={U}{\middel}JITSU)\\\hline
毎日 & every + day = \textit{daily} & マイ{\middel}ニチ (MAI{\middel}NICHI)\\\hline
朝日 & morning + sun = \textit{morning sun} & あさ{\middel}ひ (asa{\middel}hi)\\\hline
日曜日 & sun + day of the week + day + \textit{Sunday} & ニチ{\middel}ヨウ{\middel}ビ (NICHI{\middel}Y\={O}{\middel}bi)\\\hline
\end{somme}

\begin{irrr}
一日 & one + day = \textit{first day of the month} & ついたち (tsuitachi)\\\hline
昨日 & past + day = \textit{yesterday} & きのう (kin\={o})\\\hline
今日 & now + day = \textit{today} & きょう (ky\={o})\\\hline
明日 & bright + day = \textit{tomorrow} & あした、あす (ashita, asu)\\\hline
\end{irrr}

\begin{sentence}
\ruby{\underline{毎日}}{マイニチ}{\;}\ruby{\underline{日本語}}{ニホンゴ}の\ruby{\underline{本}}{ホン}\ruby{を}{お}\ruby{\underline{読}}{よ}\underline{みます}。\\\hline
MAI{\middel}NICHI NI{\middel}HON{\middel}GO no HON o yo{\middel}mimasu.\\\hline
(daily) (Japanese) (book) (read).\\\hline
=\textit{(I) read Japanese books everyday.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{White (白)}{white}\label{glyph:white}
White.  As well as shades of cleanliness and purity.
\end{glyph}

\begin{comps}
\comprtwocone & 丶 + 日 \\\hline
\comprthreecone & Dot + Sun (\ref{glyph:sun}) \\\hline
\end{comps}

\begin{remglyph}
Any \remcom{dot} will turn \hooglig{white} on the \remcom{sun}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ハク (HAKU)} & \textbf{しろ (shiro)} \\\hline
\pronsrthreecone & ビャク (BYAKU) & しら (shira) \\\hline
\end{prons}

\begin{remprons}
The \comkun{sheer obstinate} \ucomon{BYAKUYA} challenging \comon{HAKU}, in their \hooglig{white} robes, resulted in \ucomkun{sheer ugliness}.\\\hline
\end{remprons}

\begin{somme}
白 & \textit{white (noun)} & しろ (shiro)\\\hline
白い & \textit{white (adjective)} & しろ{\middel}い (shiro{\middel}i)\\\hline
白人 & white + person = \textit{caucasian} & ハク{\middel}ジン (HAKU{\middel}JIN)\\\hline
明白 & bright + white = \textit{obvious} & メイ{\middel}ハク (MEI{\middel}HAKU)\\\hline
白鳥 & white + bird = \textit{swan} & ハク{\middel}チョウ (HAKU{\middel}CH\={O})\\\hline
白米 & white + rice = \textit{polished rice} & ハク{\middel}マイ (HAKU{\middel}MAI)\\\hline
\end{somme}

\begin{sentence}
\underline{この} \ruby{\underline{白}}{しろ}\underline{い} \ruby{\underline{馬}}{うま}の\ruby{\underline{名前}}{なまえ}\ruby{は}{わ}\ruby{\underline{``雪''}}{ゆき}です。\\\hline
kono shiro{\middel}i uma no na{\middel}mae wa yuki desu.\\\hline
(this) (white) (horse) (name) (``snow'').\\\hline
=\textit{This white horse's name is ``Snow''}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\pagebreak
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Mouth (口)}{mouth}\label{glyph:mouth}
Mouth, and things oral.  This kanji also relates to openings in general, from cavers and harbors to taps and bottles, for instance.
\end{glyph}

\begin{comps}
\comprtwocone & 口 \\\hline
\comprthreecone & Mouth (\ref{glyph:mouth}) \\\hline
\end{comps}

\begin{remglyph}
Dracula \hooglig{mouth}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{コウ (K\={O})} & \textbf{くち (kuchi)} \\\hline
\pronsrthreecone & ク (KU) & --- \\\hline
\end{prons}

\begin{remprons}
\comon{Cause} the \ucomon{cook} \comkun{coochie-cooed} with his \hooglig{mouth}.\\\hline
\end{remprons}

\begin{somme}
口 & \textit{mouth} & くち (kuchi)\\\hline
人口 & person + mouth = \textit{population} & ジン{\middel}コウ (JIN{\middel}K\={O})\\\hline
早口 & early (fast) + mouth = \textit{rapid speaking} & はや{\middel}くち (haya{\middel}kuchi)\\\hline
口語 & mouth + words = \textit{colloquial language} & コウ{\middel}ゴ (K\={O}{\middel}GO)\\\hline
出口 & exit + mouth = \textit{exit} & で{\middel}ぐち (de{\middel}guchi)\\\hline
口先 & mouth + precede = \textit{lip service} & くち{\middel}さき (kuchi{\middel}saki)\\\hline
\end{somme}

\begin{sentence}
\underline{この} \ruby{\underline{国}}{くに}の\ruby{\underline{人口}}{ジンコウ}\ruby{は}{わ}\ruby{\underline{少}}{すく}\underline{ない}。\\\hline
kono kuni no JIN{\middel}K\={O} wa suku{\middel}nai.\\\hline
(this) (country) (population) (few).\\\hline
=\textit{This country has a small population.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Rotate (回)}{rotate}\label{glyph:rotate}
The general sense of this kanji relates to the ideas of rotation and things going around in both space and time.\\\hline
It can also refer to the vicinity of things (neighbourhoods, surroundings, etc.).\\\hline
This kanji is well-known to Japanese baseball fans as the character used to denote innings.
\end{glyph}

\begin{comps}
\comprtwocone & 囗 + 口 \\\hline
\comprthreecone & Enclosure + Mouth (\ref{glyph:mouth}) \\\hline
\end{comps}

\begin{remglyph}
\remcom{Dracula} turned into a bat when put in the \remcom{enclosure} and endlessly \hooglig{rotated} due to dizzy spells.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{カイ (KAI)} & \textbf{まわ (mawa)} \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\comkun{Ma wacks} \hooglig{rotating} \comon{kites}.\\\hline
\end{remprons}

\begin{somme}
回る (intr) & \textit{to go around} & まわ{\middel}る (mawa{\middel}ru)\\\hline
回す (tr) & \textit{to send around} & まわ{\middel}す (mawa{\middel}su)\\\hline
一回 & one + rotate = \textit{once} & イッ{\middel}カイ (IK{\middel}KAI)\\\hline
二回 & two + rotate = \textit{twice} & ニ{\middel}カイ (NI{\middel}KAI)\\\hline
引き回す & pull + rotate = \textit{to pull around} & ひ{\middel}き　まわ{\middel}す (hi{\middel}ki mawa{\middel}su)\\\hline
言い回し & say + rotate = \textit{turn of expression} & い{\middel}い　まわ{\middel}し (i{\middel}i mawa{\middel}shi)\\\hline
\rye{3}{Note the difference between intransitive (intr) and transitive (tr) verbs.  An intransitive verb does not require an object to complete the action, whereas a transitive verb does.}
\end{somme}

\begin{sentence}
\ruby{\underline{私}}{わたし}\ruby{は}{わ}\ruby{\underline{二回}}{ニカイ}{\;}\underline{カナダ}\ruby{へ}{え}\ruby{\underline{行}}{い}\underline{きました}。\\\hline
watashi wa NI{\middel}KAI Canada e i{\middel}kimashita.\\\hline
(I) (twice) (Canada) (went)\\\hline
I went to Canada twice.\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
\vspace*{\fill}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\pagebreak
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Four (四)}{four}\label{glyph:four}
Four, Quad-, etc.
\end{glyph}

\begin{comps}
\comprtwocone & 囗 + 儿 \\\hline
\comprthreecone & Enclosure + person (at the bottom of a character) \\\hline
\end{comps}

\begin{remglyph}
\remcom{People} dance to the \hooglig{four} corners of their \remcom{prison} cell.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{シ (SHI)} & \textbf{よん、よ (yon, yo)} \\\hline
\pronsrthreecone & --- & よっ (yo-) \\\hline
\end{prons}

\begin{remprons}
The soldiers \comkun{yomped} with \comon{shields} to the \hooglig{four} corners of the world \ucomkun{yonder}.\\\hline
\end{remprons}

\begin{somme}
四 & \textit{four} & シ (SHI) \\\hline
四人 & four + person = \textit{four people} & よ{\middel}ニン (yo{\middel}NIN) \\\hline
四月 & four + moon (month) = \textit{April} & シ{\middel}ガツ (SHI{\middel}GATSU) \\\hline
四時 & four + time = \textit{four o'clock} & よ{\middel}ジ (yo{\middel}JI) \\\hline
四十 & four + ten = \textit{forty} & よん{\middel}ジュウ (yon{\middel}J\={U}) \\\hline
四百 & four + hundred = \textit{four hundred} & よん{\middel}ヒャク (yon{\middel}HYAKU) \\\hline
四日 & four + day = \textit{the fourth day of the month} & よっか (yok{\middel}ka) \\\hline
四つ & \textit{four (general counter)} & よっつ (yot{\middel}tsu) \\\hline
\end{somme}

\begin{sentence}
\ruby{\underline{四月}}{シガツ}に\ruby{は}{わ}\ruby{\underline{雨}}{あめ}が\ruby{\underline{多}}{おお}\underline{い}。\\\hline
(April) (rain) (many).\\\hline
=\textit{There's a lot of rain in April.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Moon (月)}{moon}\label{glyph:moon}
Moon, Month.
\end{glyph}

\begin{comps}
\comprtwocone & 月 \\\hline
\comprthreecone & Moon \\\hline
\end{comps}

\begin{remglyph}
---\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ゲツ、ガツ (GETSU, GATSU)} & \textbf{つき (tsuki)} \\\hline
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
I had the \comon{guts} to \hooglig{moon} everyone in the \comkun{stew kitchen} through my \comon{Getz'} window.\\\hline
\end{remprons}

\begin{somme}
月 & \textit{moon; month} & つき (tsu{\middel}ki)\\\hline
一月 & one + month = \textit{January} & イチ{\middel}ガツ (ICHI{\middel}GATSU)\\\hline
二月 & two + month = \textit{February} & ニ{\middel}ガツ (NI{\middel}GATSU)\\\hline
三月 & three + month = \textit{March} & サン{\middel}ガツ (SAN{\middel}GATSU)\\\hline
四月 & four + month = \textit{April} & シ{\middel}ガツ (SHI{\middel}GATSU)\\\hline
先月 & precede + month = \textit{last month} & セン{\middel}ゲツ (SEN{\middel}GETSU)\\\hline
月見 & moon + see = \textit{moon viewing} & つき{\middel}み (tsuki{\middel}mi)\\\hline
\end{somme}

\begin{sentence}
\ruby{\underline{九}}{ク}\ruby{\underline{月}}{ガツ}に\ruby{\underline{月}}{つき}\ruby{\underline{見}}{み}に\ruby{\underline{行}}{い}きました。\\\hline
KU{\middel}GATSU ni tsuki{\middel}mi ni i{\middel}kimashita.\\\hline
(September) (moon viewing) (went)\\\hline
I went moon viewing in September.\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
\vspace*{\fill}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\pagebreak
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Bright (明)}{bright}\label{glyph:bright}
Bright; Clear; Light.
\end{glyph}

\begin{comps}
\comprtwocone & 日 + 月 \\\hline
\comprthreecone & Sun (\ref{glyph:sun}) + Moon (\ref{glyph:moon}) \\\hline
\end{comps}

\begin{remglyph}
Together the \remcom{sun} and the \remcom{moon} must be very \hooglig{bright}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{メイ (MEI)} & \textbf{あ、あか (a, aka)}\\\hline
\pronsrthreecone & ミョウ、ミン (MY\={O}, MIN) & あき (aki)\\\hline
\end{prons}

\begin{remprons}
The \hooglig{bright} \ucomon{myou} dolls, \comkun{a.k.a.} \ucomkun{akimbo} dolls, generated a \ucomon{mingled} \comon{mayhem} after the \comkun{update}.\\\hline
\end{remprons}

\begin{somme}
明るい & \textit{bright} & あか{\middel}るい (aka{\middel}rui)\\\hline
明かり & \textit{clearness} & あ{\middel}かり (a{\middel}kari)\\\hline
明白 & bright + white = \textit{obvious} & メイ{\middel}ハク (MEI{\middel}HAKU)\\\hline
不明 & not + bright = \textit{unclear} & フ{\middel}メイ (FU{\middel}MEI)\\\hline
\end{somme}

\begin{sentence}
\ruby{\underline{月}}{つき}\ruby{は}{わ}\ruby{\underline{明}}{あか}\underline{るい}です。\\\hline
tsuki wa aka{\middel}rui desu.\\\hline
(moon) (bright)\\\hline
The moon is bright.\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Tree (木)}{tree}\label{glyph:tree}
Tree; Wood.
\end{glyph}

\begin{comps}
\comprtwocone & 木 \\\hline
\comprthreecone & Tree (\ref{glyph:tree}) \\\hline
\end{comps}

\begin{remglyph}
---\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{モク、ボク (MOKU, BOKU)} & \textbf{き (ki)}\\\hline
\pronsrthreecone & --- & こ (ko)\\\hline
\rye{3}{Look for \textbf{MOKU} to show up in the first position.\\\hline
Look for \textbf{BOKU} to show up in the second position.\\\hline
\textbf{ki} occurs less often, but appears with equal frequency in the first or second position.}
\end{prons}

\begin{remprons}
From the \hooglig{tree} I was drinking \comon{mock} \comon{bock} to \comkun{keep} the \ucomkun{cobbles} of everyday life away.\\\hline
\end{remprons}

\begin{somme}
木 & \textit{tree} & き (ki)\\\hline
木曜日 & tree + day of the week + sun (day) = \textit{Thursday} & モク{\middel}ヨウ{\middel}ビ (MOKU{\middel}Y\={O}{\middel}BI)\\\hline
木星 & tree + star = \textit{Jupiter (planet)} & モク{\middel}セイ (MOKU{\middel}SEI)\\\hline
大木 & large + tree = \textit{large tree} & タイ{\middel}ボク (TAI{\middel}BOKU)\\\hline
土木 & earth +tree = \textit{civil engineering} & ド{\middel}ボク (DO{\middel}BOKU)\\\hline
\end{somme}

\begin{irrr}
木綿* & tree + cottom = \textit{cotton (cloth)} & も{\middel}メン (mo{\middel}MEN)\\\hline
\rye{3}{The second kanji incidentally is not covered in this book.  Such characters will be included only when they form words in which the pronounciation of the character we are considering has an irregular reading.  Irregular readings containing these kanji will be identified with an asterisk (*).}
\end{irrr}

\begin{sentence}
\underline{あの} \ruby{\underline{高}}{たか}\underline{い} \ruby{\underline{木}}{き} の\ruby{\underline{名前}}{なまえ}が\ruby{\underline{分}}{わ}\underline{かります}か。\\\hline
ano taka{\middel}i ki no na{\middel}mae ga wa{\middel}karimasu ka.\\\hline
(that) (tall) (tree) (name) (understand).\\\hline
=\textit{Do you know the name of that tall tree?}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
\vspace*{\fill}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\pagebreak
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Five (五)}{five}\label{glyph:five}
Five/Pent-.
\end{glyph}

\begin{comps}
\comprtwocone & 五 \\\hline
\comprthreecone & Five (\ref{glyph:five}) \\\hline
\end{comps}

\begin{remglyph}
This is how most people feel when their work ends at five o'clock.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ ゴ (GO)} & ---\\\hline
\pronsrthreecone & --- & いつ (itsu)\\\hline
\end{prons}

\begin{remprons}
When I was about to \ucomkun{eat soup} at \hooglig{five} o'clock, I realised other people \comkun{gobbled} it already.\\\hline
\end{remprons}

\begin{somme}
五 & \textit{five} & ゴ (GO)\\\hline
五人 & five + person = \textit{five people} & ゴ{\middel}ニン (GO{\middel}NIN)\\\hline
五月 & five + moon (month) = \textit{May} & ゴ{\middel}ガツ (GO{\middel}GATSU)\\\hline
五十 & five + ten = \textit{fifty} & ゴ{\middel}ジュウ (GO{\middel}J\={U})\\\hline
五時 & five + time = \textit{five o'clock} & ゴ{\middel}ジ (GO{\middel}JI)\\\hline
五日 & five + sun (day) = \textit{fifth day of the month} & いつ{\middel}か (itsu{\middel}ka)\\\hline
五つ & \textit{five (general counter)} & いつ{\middel}つ (itsu{\middel}tsu)\\\hline
\end{somme}

\begin{sentence}
\ruby{\underline{五十人}}{ゴジュウニン}で\ruby{\underline{山}}{やま}え\ruby{\underline{行}}{い}\underline{きました}。\\\hline
GO{\middel}J\={U}{\middel}NIN de yama e i{\middel}kimashita.\\\hline
(fifty people) (mountain) (went)\\\hline
=\textit{Fifty (of us) went to the mountain.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Eye (目)}{eye}\label{glyph:eye}
Eye.\\\hline
In some instances it is read with its kun-yomi, and indicates the \textbf{``-th'' suffix} in words such as ``fourth'' and ``seventh''; it also expresses the related endings of ``first'', ``second'', ``third'', etc.
\end{glyph}

\begin{comps}
\comprtwocone & 目 \\\hline
\comprthreecone & Eye (\ref{glyph:eye}) \\\hline
\end{comps}

\begin{remglyph}
Cyclops.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{モク (MOKU)} & \textbf{め (me)}\\\hline
\pronsrthreecone & ボク (BOKU) & ま (ma)\\\hline
\rye{3}{\textbf{me} is the more common reading of the two.}
\end{prons}

\begin{remprons}
I \hooglig{eyed} the \ucomkun{muddy} \comkun{mess} caused by the \comon{mock} \ucomon{bock}.\\\hline
\end{remprons}

\begin{somme}
目 & \textit{eye} & め (me)\\\hline
目玉 & eye + jewel = \textit{eyeball} & め{\middel}だま (medama)\\\hline
八回目 & eight + rotate + eye = \textit{the eighth time} & ハチ{\middel}カイ{\middel}め (HACHI{\middel}KAI{\middel}me)\\\hline
目立つ & eye + stand = \textit{to stand out} & め{\middel}だつ (me{\middel}datsu)\\\hline
注目 & pour + eye = \textit{notice} & チュウ{\middel}モク (CH\={U}{\middel}MOKU)\\\hline
\end{somme}

\begin{sentence}
\ruby{\underline{王子}}{オウジ}の\ruby{\underline{目}}{め} \ruby{は}{わ}\ruby{\underline{青い}}{あおい}です。\\\hline
\={O}{\middel}JI no me wa ao{\middel}i desu.\\\hline
(prince) (eye) (blue)\\\hline
=\textit{The prince's eyes are blue.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
\vspace*{\fill}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\pagebreak
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Woman (女)}{woman}\label{glyph:woman}
Woman; Female.
\end{glyph}

\begin{comps}
\comprtwocone & 女 \\\hline
\comprthreecone & Woman (\ref{glyph:woman}) \\\hline
\end{comps}

\begin{remglyph}
---\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ジョ (JO)} & \textbf{おんな (onna)}\\\hline
\pronsrthreecone & ニョ、ニョウ (NYO, NY\={O}) & め (me)\\\hline
\end{prons}

\begin{remprons}
Many \hooglig{woman}, whose \comon{jobs} are in \ucomkun{messy} \ucomon{Neon} areas (an appropriate \ucomon{neologism}), are \comkun{on narcotic} drugs.\\\hline
\end{remprons}

\begin{somme}
女 & \textit{woman} & おんな (onna)\\\hline
女王 & woman + king = \textit{queen} & ジョ{\middel}オウ (JO{\middel}\={O})\\\hline
少女 & few + woman = \textit{girl} & ショウ{\middel}ジョ (SH\={O}{\middel}JO)\\\hline
女心 & woman + heart = \textit{a woman's heart} & おんな{\middel}ごころ (onna{\middel}gokoro)\\\hline
雪女 & snow + heart = \textit{snow fairy} & ゆき{\middel}おんな (yuki{\middel}onna)\\\hline
男女 & man + woman = \textit{men and women} & ダン{\middel}ジョ (DAN{\middel}JO)\\\hline
\end{somme}

\begin{sentence}
\ruby{\underline{女}}{おんな}の\ruby{\underline{人}}{ひと}は\ruby{\underline{木}}{き}の\ruby{\underline{下}}{した}で\ruby{\underline{止}}{と}\underline{まりました}。\\\hline
onna no hito wa ki no shita de to{\middel}marimashita.\\\hline
(woman) (person) (tree) (under) (stopped).\\\hline
=\textit{The woman stopped under the tree.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Large (大)}{large}\label{glyph:large}
Large; Big.\\\hline
The meaning of ``important'' and ``great (not necessarily always in a positive sense)'' can also be implied in words containing this kanji.
\end{glyph}

\begin{comps}
\comprtwocone & 大 \\\hline
\comprthreecone & Large (\ref{glyph:large}) \\\hline
\end{comps}

\begin{remglyph}
A \hooglig{large} sumo wrestler.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ダイ、タイ (DAI, TAI)} & \textbf{おお (\={o})}\\\hline
\pronsrthreecone & --- & ---\\\hline
\rye{3}{\textbf{DAI} and \textbf{TAI} occur with equal frequency when this kanji appears in the first position.  \textbf{\={o}} much less often.\\\hline When it occupies other positions in a compound, however, \textbf{DAI} is by far the more common reading.}
\end{prons}

\begin{remprons}
The \hooglig{large}, \comon{tight} fabric had an \comkun{awful} \comon{dye} to it.\\\hline
\end{remprons}

\begin{somme}
大きい & \textit{large} & おお{\middel}きい (\={o}{\middel}kii)\\\hline
大木 & large + tree = \textit{large tree} & タイ{\middel}ボク (TAI{\middel}BOKU) \\\hline
大好き & large + like = \textit{to really like} & ダイ　す{\middel}き (DAI su{\middel}ki)\\\hline
大体 & large + body = \textit{in general} & ダイ{\middel}タイ (DAI{\middel}TAI)\\\hline
大雨 & large + rain = \textit{heavy rain} & おお{\middel}あめ (\={o}{\middel}ame)\\\hline
大切 & large + cut = \textit{important} & ダイ{\middel}セツ (TAI{\middel}SETSU)\\\hline
大学 & large + study = \textit{university} & ダイ{\middel}ガク (DAI{\middel}GAKU)\\\hline
\end{somme}

\begin{sentence}
\underline{あの} \ruby{\underline{森}}{もり} にわ\ruby{\underline{大木}}{タイボク}が\ruby{\underline{多}}{おお}\underline{い}。\\\hline
ano mori ni wa TAI{\middel}BOKU ga \={o}i.\\\hline
(that) (forest) (large trees) (many)\\\hline
=\textit{There are many large trees in that forest}.\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
\vspace*{\fill}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\pagebreak
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Middle (中)}{middle}\label{glyph:middle}
A versatile character that expresses the ideas of ``middle'', ``medium'', ``average'', etc.\\\hline
This kanji can also serves as an abbreviation for China.\\\hline
日 is used as the equivalent with respect to Japan.
\end{glyph}

\begin{comps}
\comprtwocone & 口 + 丨 \\\hline
\comprthreecone & Mouth (\ref{glyph:mouth}) + Pole (Vertical Stroke) \\\hline
\end{comps}

\begin{remglyph}
Strike the \remcom{pole} through the \hooglig{middle} of Dracula's \remcom{mouth} and he will die.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{チュウ (CH\={U})} & \textbf{なか (naka)}\\\hline
\pronsrthreecone & ジュウ (J\={U}) & ---\\\hline
\end{prons}

\begin{remprons}
I had the \comkun{knack} to \comon{chew} the pulp in the \hooglig{middle} of the \ucomon{juice} container.\\\hline
\end{remprons}

\begin{somme}
中 & \textit{middle} & なか (naka)\\\hline
中心 & middle + heart = \textit{center} & チュウ{\middel}シン (CH\={U}{\middel}SHIN)\\\hline
中米 & middle + rice = \textit{Central America} & チュウ{\middel}ベイ (CH\={U}{\middel}BEI)\\\hline
中国 & middle + country = \textit{China} & チュウ{\middel}ゴク (CH\={U}{\middel}GOKU)\\\hline
中立 & middle + stand = \textit{neutrality} & チュウ{\middel}リツ (CH\={U}{\middel}RITSU)\\\hline
中古 & middle + old = \textit{secondhand} & クウ{\middel}コ (CH\={U}{\middel}KO)\\\hline
水中 & water + middle = \textit{underwater/in the water} & スイ{\middel}チュウ (SUI{\middel}CH\={U})\\\hline
\end{somme}

\begin{sentence}
\ruby{\underline{車}}{くるま}の\ruby{\underline{中}}{なか}に\ruby{\underline{何}}{なに}が\underline{あります}か。\\\hline
kuruma no naka ni nani ga arimasu ka.\\\hline
(car) (middle) (what) (is)\\\hline
=\textit{What is in your car?}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\begin{multicols}{2}
\begin{glyph}{Eight (八)}{eight}\label{glyph:eight}
Eight.
\end{glyph}

\begin{comps}
\comprtwocone & 八 \\\hline
\comprthreecone & Eight (\ref{glyph:eight})\\\hline
\end{comps}

\begin{remglyph}
Volcano.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ハチ (HACHI)} & ---\\\hline
\pronsrthreecone &  & よう、やっ、や (y\={o}, ya-, ya)\\\hline
\end{prons}

\begin{remprons}
\hooglig{\comon{Hachi}} \ucomkun{yachted} in the days of \ucomkun{yore}.\\\hline
\end{remprons}

\begin{somme}
八 & \textit{eight} & ハチ (HACHI)\\\hline
八月 & eight + moon (month) = \textit{August} & ハチ{\middel}ガツ (HACHI{\middel}GATSU)\\\hline
八人 & eight + person = \textit{eight people} & ハチ{\middel}ニン (HACHI{\middel}NIN)\\\hline
八円 & eight + circle = \textit{eight yen} & ハチ{\middel}エン (HACHI{\middel}EN)\\\hline
八時 & eight + time = \textit{eight o'clock} & ハチ{\middel}ジ (HACHI{\middel}JI)\\\hline
八日 & eight + sun (day) = \textit{the eighth day of the month} & よう{\middel}か (y\={o}{\middel}ka)\\\hline
八つ & \textit{eight (general counter)} & やっ{\middel}つ (yat{\middel}tsu)\\\hline
\end{somme}

\begin{sentence}
\ruby{\underline{八月}}{ハチガツ}に\ruby{\underline{秋田}}{あきた}え\ruby{\underline{行}}{い}\underline{きましょう}。\\\hline
HACHI{\middel}GATSU ni Aki{\middel}ta e i{\middel}kimash\={o}.\\\hline
(August) (Akita) (let's go)\\\hline
=\textit{Let's go to Akita in August.}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
\vspace*{\fill}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\pagebreak
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Small (小)}{small}\label{glyph:small}
Small; Little.
\end{glyph}

\begin{comps}
\comprtwocone & 亅 + 八 \\\hline
\comprthreecone & Hook + Eight (\ref{glyph:eight})\\\hline
\end{comps}

\begin{remglyph}
What is the \hooglig{small} \remcom{hook} doing in the massive \remcom{volcano}?\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ショウ (SH\={O})} & \textbf{こ (ko)}\\\hline
\pronsrthreecone & --- & お (o)\\\hline
\end{prons}

\begin{remprons}
It is an \ucomkun{obligation} to eat the \hooglig{small} \comkun{cheese} \comkun{cobbles} along the \comon{shore}.\\\hline
\end{remprons}

\begin{somme}
小さい & \textit{small} & ちい{\middel}さい (chiisai)\\\hline
小春 & small + spring = \textit{Indian Summer} & こ{\middel}はる (ko{\middel}haru)\\\hline
小国 & small + country = \textit{small country} & ショウ{\middel}コク (SH\={O}{\middel}KOKU)\\\hline
小高い & small + tall = \textit{slightly elevated} & こ　だ{\middel}かい (ko da{\middel}kai)\\\hline
最小 & most + small = \textit{smallest} & サイ{\middel}ショウ (SAI{\middel}SH\={O})\\\hline
\end{somme}

\begin{sentence}
\underline{あの} \ruby{\underline{車}}{くるま} \ruby{は}{わ}\underline{とても} \ruby{\underline{小}}{ちい}\underline{さい} \underline{ですね}。\\\hline
ano kuruma wa totemo chii{\middel}sai desu ne.\\\hline
(that) (car) (really) (small) (isn't it).\\\hline
=\textit{That car is really small, isn't it?}\\\hline
\end{sentence}
\end{multicols}
\vspace*{-\baselineskip}
\vhrulefill{5pt}
\vspace*{\fill}
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
\pagebreak