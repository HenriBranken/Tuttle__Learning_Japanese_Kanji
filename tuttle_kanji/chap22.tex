...
Court.\\\hline
This kanji can refer to courts of law (as in the initial two compounds below), and imperial courts (as in the last).
\end{glyph}
...
\rye{3}{The kunyomi is the more common reading.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}
...
This kanji works together with those that follow it, and with its companion sets some type of condition on the kanji preceding them.
\end{glyph}
...
Title, sign.\\\hline
This is a versatile kanji that can refer to everything from traffic signals to newspaper editions.  The idea of a title or sign, however, is usually present in some way.
\end{glyph}
...
Hot.\\\hline
This kanji relates solely to air temperature and weather; it's opposite is thus 寒, `cold' (Entry \ref{glyph:cold}).
\end{glyph}
...
\rye{3}{The kunyomi can incorporate its hiragana ending in certain compounds.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}
...
Lady, woman.\\\hline
It's worthwhile to see how this kanji differs visually from 寝 `sleep' (Entry \ref{glyph:sleep}), and 帰 `homeward' (Entry \ref{glyph:homeward}).
\end{glyph}
...
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{Recall from Entry \ref{glyph:husband} that the final compound is one of the only instances in which 夫 is read with its less common onyomi of フウ.}
...
Nature.\\\hline
This kanji emphasizes the nature or personality of the character preceding it.
\end{glyph}
...
\rye{3}{You are likely to meet this reading in one commonly-used compound: 天然 (テン{\middel}ネン/TEN{\middel}NEN) `natural', from the kanji `heaven' and `nature'.}
\end{prons}
...
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{Note that the second compound features the only instance of 自 being read with its less common onyomi of シ.}
...
Heat, hot (in aspects other than weather), and the ideas of enthusiasm and passion.  The opposite of this kanji is 冷, `cool' (Entry \ref{glyph:cool}).
\end{glyph}
...
Use.\\\hline
This is the last of a group of five kanji that we can now review as a whole.  The building patter for the present character has been as follows: 史 `history' (Entry \ref{glyph:history}), to 吏 `officer' (Entry \ref{glyph:officer}), to 使.  A pair of similar-looking but unrelated kanji were constructed in this way: 更 `anew' (Entry \ref{glyph:anew}), to 便 `delivery' (Entry \ref{glyph:delivery.})
\end{glyph}
...
This is another important negating kanji, along with 不 `not' (Entry \ref{glyph:not}), and 非 `un-' (Entry \ref{glyph:un}).  It imparts a sense of `-less' or `-free' in many of the compounds in which it appears.
\end{glyph}
...
An important secondary meaning is present when this kanji appears as the final character in certain compounds; in such instances it can indicate a government minister (as in the final example below).
\end{glyph}
...
Long.\\\hline
This kanji can refer to the `chief' or `head' of entities such as cities and companies (the final compound provides an example).
\end{glyph}
...
\begin{remglyph}
Part of the Kanji radicals (\#{168}).\\\hline
\end{remglyph}
