...
Cargo, burden.\\\hline
As mentioned in Entry \ref{glyph:what}, this is the only general-use kanji in which the character for `what' appears as a component.
\end{glyph}
...
\rye{3}{The kun-yomi is by far the more common reading.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{The cargo} \comkun{niggled} us that we started \comon{cussing}.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{Note that the second compound uses the less common onyomi of 物 (``thing'', from Entry \ref{glyph:thing}).}
...
\begin{remglyph}
\hooglig{The hitting} \remcom{hand} must be able to \remcom{block}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ダ (DA)} & \textbf{う (u)}\\\hline
\rye{3}{う is the more common reading.  This is another kanji that will often incorporate its accompanying hiragana when forming nouns.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}
...
Round.\\\hline
By extension are the ideas of completion and wholeness, etc.
\end{glyph}
...
Manor, although this kanji can appear in compounds referring to anything from stately embassies to simple country inns.
\end{glyph}
...
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{Recall from Entry \ref{glyph:technique} that the first two kanji of the final compound mean `art' when taken together.}
...
\begin{remglyph}
Part of the Kanji radicals (\#{183}).\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ヒ (HI)} & \textbf{と (to)}\\\hline
\rye{3}{The kunyomi can absorb its accompanying hiragan when forming nouns.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}

\begin{remprons}
\hooglig{Take flight} to \comon{heal} your \comkun{torrent} of sorrows.\\\hline
\end{remprons}

%{\middel}
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{When taken together, the first two kanji in the final compound mean `flight', or `flying'.}
...
Number.\\\hline
When in the first position, this kanji will sometimes carry the meaning of `several' of whatever is indicated by the character following it (e.g. years, persons, days, etc.).
\end{glyph}
...
\rye{3}{スウ is the most common reading, and the one used to express the idea of `several' mentioned above.  On rare occasions, the onyomi can become voiced; the final compound is an example.\\\hline
...
Learn.\\\hline
Take care not to confuse this kanji with 皆 (\ref{glyph:all}).
\end{glyph}
...
\begin{remglyph}
\hooglig{Learn} the \remcom{white} hues of \remcom{feathers}.\\\hline
\end{remglyph}
...
\begin{comps}
\comprtwocone & 尸 + 衣 + 艹 \\\hline
\comprthreecone & Corpse/Body + Clothing + Plant/Grass \\\hline
\end{comps}

\begin{remglyph}
\hooglig{A display} of a \remcom{corpse} and its \remcom{clothing} lying on the \remcom{grass}.\\\hline
\end{remglyph}
...
Underlying the meaning of this character is a sense of the surface appearance of things.
\end{glyph}
...
\begin{remglyph}
Part of the Kanji radicals (\#{176}).\\\hline
\end{remglyph}
...
\begin{remglyph}
\hooglig{Select} by \remcom{oneself} who you are \remcom{walking} \remcom{together} with.\\\hline
\end{remglyph}
...
\rye{3}{The kunyomi becomes voiced (as in the second compound) according to the same rules as those for 付 (Entry \ref{glyph:attach}).}
\pronsrthreecone & --- & --- \\\hline
\end{prons}
...
Topic, Theme.\\\hline
This is the only instance in the general-use kanji in which the character for `approve' is stretched out.
\end{glyph}
...
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{Recall from Entry \ref{glyph:reach} that the first 2 kanji in the final compound mean `urgen' when taken together.}
...
When used as a suffix, this kanji translates as `... years old'.
\end{glyph}
...
The famous word in the second example is the only time this reading becomes voiced; recall from Entry \ref{glyph:ten_thousand} that 万 is read with is less common onyomi here.}
...
When used as a suffix as in the second compound below, the meaning becomes `intended for' or `suited to'.  Take care not to confuse this kanji with 同 `same' from Entry \ref{glyph:same}.
\end{glyph}
