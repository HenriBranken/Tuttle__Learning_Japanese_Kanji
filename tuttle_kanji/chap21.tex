...
\begin{remglyph}
\hooglig{Member} of the \remcom{vampire} club collect \remcom{shells}.\\\hline
\end{remglyph}
...
荷が下りる&cargo + lower = \textit{feeling relieved}&に{\middel}が{\wsplit}お{\middel}りる \mbox{(ni{\middel}ga{\wsplit}ori{\middel}ru)}\\\hline
荷が重い&cargo + heavy = \textit{great responsibility}&に{\middel}が{\wsplit}おも{\middel}い \mbox{(ni{\middel}ga{\wsplit}omo{\middel}i)}\\\hline
入荷待ち&goods received + wait = \textit{backordering}&ニュウ{\middel}カ{\wsplit}まち \mbox{(NY\={U}{\middel}KA{\wsplit}ma{\middel}chi)}\\\hline
\end{somme}
...
\multicolumn{5}{|r|}{\textit{He put down his baggage.}}\\\hline
\end{decomp}
\end{multicols}
...
%&\mbox{()}\\\hline
\begin{somme}
港&\textit{port; harbour}&みなと \mbox{(minato)}\\\hline
...
港内&port + inside = \textit{inside the harbour}&コウ{\middel}ナイ \mbox{(K\={O}{\middel}NAI)}\\\hline
母港&mother + port = \textit{home port}&ボ{\middel}コウ \mbox{(BO{\middel}K\={O})}\\\hline
\end{somme}
...
\rye{3}{う is the more common reading.}
\pronsrthreecone&---&---\\\hline
\end{prons}
\rindex{DA}{打}
\rindex{u}{打}
...
\splitter{打ち切る}{tr}&hit + cut = \textit{stop doing sth; break off}&う{\middel}ち{\wsplit}き{\middel}る \mbox{(u{\middel}chi{\wsplit}ki{\middel}ru)}\\\hline
打ち切り&hit + cut = \textit{finish}&う{\middel}ち{\wsplit}き{\middel}り \mbox{(u{\middel}chi{\wsplit}ki{\middel}ri)}\\\hline
打開&hit + open = \textit{break in the deadlock}&ダ{\middel}カイ \mbox{(DA{\middel}KAI)}\\\hline
打者&hit + individual = \textit{batter; hitter}&ダ{\middel}シャ \mbox{(DA{\middel}SHA)}\\\hline
打力&hit + strength = \textit{hitting power}&ダ{\middel}リョク \mbox{(DA{\middel}RYOKU)}\\\hline
頭打ち&head + hit = \textit{reaching a limit}&あたま{\wsplit}う{\middel}ち \mbox{(atama{\wsplit}u{\middel}chi)}\\\hline
仕打ち&serve + hit = \textit{poor treatment; bad behaviour}&シ{\wsplit}う{\middel}ち \mbox{(SHI{\wsplit}u{\middel}chi)}\\\hline
早打ち&early + hit = \textit{fast typing/beating}&はや{\wsplit}う{\middel}ち \mbox{(haya{\wsplit}u{\middel}chi)}\\\hline
不意打ち&unforseen + hit = \textit{surprise attack}&フ{\middel}不{\wsplit}う{\middel}ち \mbox{(FU{\middel}I{\wsplit}u{\middel}chi)}\\\hline
\end{somme}
...
%&\mbox{()}\\\hline
\begin{somme}
列島&row + island = \textit{island chain}&レッ{\middel}トウ \mbox{(RET{\middel}T\={O})}\\\hline
...
系列&system + row = \textit{corporate group}&ケイ{\middel}レツ \mbox{(KEI{\middel}RETSU)}\\\hline
行列&go + row = \textit{line}&ギョウ{\middel}レツ \mbox{(GY\={O}{\middel}RETSU)}\\\hline
直列&straight + row = \textit{series (e.g. electrical)}&チョク{\middel}レツ \mbox{(CHOKU{\middel}RETSU)}\\\hline
配列&distribute + row = \textit{arrangement}&ハイ{\middel}レツ \mbox{(HAI{\middel}RETSU)}\\\hline
...
Round.\\\hline
By extension are the ideas of \textsuperscript{[1]}\textit{completion} and \textsuperscript{[2]}\textit{wholeness}, etc.
\end{glyph}
...
%&\mbox{()}\\\hline
\begin{somme}
\splitter{丸める}{tr}&\textit{to make round}&まる{\middel}める \mbox{(maru{\middel}meru)}\\\hline
丸々&round $(\times 2)$ = \textit{plump; chubby}&まる{\middel}まる \mbox{(maru{\middel}maru)}\\\hline
丸い&\textit{round}&まる{\middel}い \mbox{(maru{\middel}i)}\\\hline
丸で&\textit{entirely; completely}&まる{\middel}で \mbox{(maru{\middel}de)}\\\hline
日の丸&sun + round = \textit{The Japanese flag}&ひ{\wsplit}の{\wsplit}まる \mbox{(hi{\wsplit}no{\wsplit}maru)}\\\hline
...
丸太&round + fat = \textit{log}&まる{\middel}た \mbox{(maru{\middel}ta)}\\\hline
真ん丸い&true + round = \textit{perfectly round}&ま{\middel}ん{\wsplit}まる{\middel}い \mbox{(ma{\middel}n{\wsplit}maru{\middel}i)}\\\hline
本丸&main + round = \textit{core; centre; focus}&ホン{\middel}まる \mbox{(HON{\middel}maru)}\\\hline
丸出し&round + exit = \textit{exposing fully}&まる{\wsplit}だ{\middel}し \mbox{(maru{\wsplit}da{\middel}shi)}\\\hline
丸見え&round + see = \textit{full view; plain sight}&まる{\wsplit}み{\middel}え \mbox{(maru{\wsplit}mi{\middel}e)}\\\hline
\end{somme}
...
This 漢字 can appear in compounds referring to anything from \textit{stately embassies} to \textit{simple country inns}.
\end{glyph}
\gindex{Manor}{館}\strindex{16}{館}

\begin{comps}
\comprtwocone&食 + 官\\\hline
\comprthreecone&Eat/Food + Official (p. \pageref{glyph:official})\\\hline
\end{comps}

\begin{remglyph}
\hooglig{Manor} for \remcom{eating} \remcom{officials}.\\\hline
\end{remglyph}
...
会館&meet + manor = \textit{assembly hall}&カイ{\middel}カン \mbox{(KAI{\middel}KAN)}\\\hline
...
\splitter{飛ばす}{tr}&\textit{to let fly}&と{\middel}ばす \mbox{(to{\middel}basu)}\\\hline
\splitter{売り飛ばす}{tr}&sell + fly = \textit{to dispose of \ldots}&う{\middel}り{\wsplit}と{\middel}ばす \mbox{(u{\middel}ri{\wsplit}to{\middel}basu)}\\\hline
\splitter{飛び出す}{intr}&fly + exit = \textit{to fly out; jump out}&と{\middel}び{\wsplit}だ{\middel}す \mbox{(to{\middel}bi{\wsplit}da{\middel}su)}\\\hline
飛び切り&fly + cut = \textit{best; unequalled}&と{\middel}び{\wsplit}き{\middel}り \mbox{(to{\middel}bi{\wsplit}ki{\middel}ri)}\\\hline
飛行機&flight + mechanism = \textit{aeroplane}&ヒ{\middel}コウ{\middel}キ \mbox{(HI{\middel}K\={O}{\middel}KI)}\\\hline
飛行場&flight + place = \textit{airport}&ヒ{\middel}コウ{\middel}ジョウ \mbox{(HI{\middel}K\={O}{\middel}J\={O})}\\\hline
飛行士&flight + gentleman = \textit{pilot}&ヒ{\middel}コウ{\middel}シ \mbox{(HI{\middel}K\={O}{\middel}SHI)}\\\hline
高飛車&tall + fly + car = \textit{high-handed}&たか{\middel}ビ{\middel}シャ \mbox{(taka{\middel}BI{\middel}SHA)}\\\hline
%低空飛行&low altitude + aviation = \textit{low-altitude flying}&テイ{\middel}クウ{\middel}ヒ{\middel}コウ \mbox{(TEI{\middel}K\={U}{\middel}HI{\middel}K\={O})}\\\hline
飛び石&fly + stone = \textit{stepping stones}&と{\middel}び{\wsplit}い{\middel}し \mbox{(to{\middel}bi{\wsplit}i{\middel}shi)}\\\hline
\end{somme}
...
Number.\\\hline
When in the first position, this 漢字 will indicate \textit{several} of whatever follows 数 (e.g. years, persons, days, etc.).
\end{glyph}
...
かず is the general word for \textit{number}.\\\hline
かぞ is a verb stem.}
...
\begin{remprons}
\hooglig{The number} of people following \ucomon{suit} by selling \comkun{kazoos} in the \comkun{car zone} were \comon{sued}.\\\hline
...
%&\mbox{()}\\\hline
\begin{somme}
\splitter{数える}{tr}&\textit{to count}&かぞ{\middel}える \mbox{(kazo{\middel}eru)}\\\hline
数&\textit{number}&かず \mbox{(kazu)}\\\hline
数学&number + study = \textit{mathematics}&スウ{\middel}ガク \mbox{(S\={U}{\middel}GAKU)}\\\hline
数字&number + character = \textit{numeric character}&スウジ \mbox{(S\={U}{\middel}JI)}\\\hline
...
多数決&many + decide = \textit{majority decision}&タ{\middel}スウ{\middel}ケツ \mbox{(TA{\middel}S\={U}{\middel}KETSU)}\\\hline
多数回&many + rotate = \textit{many times; multi-}&タ{\middel}スウ{\middel}カイ \mbox{(TA{\middel}S\={U}{\middel}KAI)}\\\hline
無数&nil + number = \textit{innumerable}&ム{\middel}スウ \mbox{(MU{\middel}S\={U})}\\\hline
\end{somme}
...
%&\mbox{()}\\\hline
\begin{somme}
\splitter{習う}{tr}&\textit{to learn; take lessons}&なら{\middel}う \mbox{(nara{\middel}u)}\\\hline
\splitter{見習う}{tr}&\textit{to follow another's example}&み{\wsplit}なら{\middel}う \mbox{(mi{\wsplit}nara{\middel}u)}\\\hline
悪習&bad + learn = \textit{bad habit; vice}&アク{\middel}シュウ \mbox{(AKU{\middel}SH\={U})}\\\hline
...
\notyet{練習}&train + learn = \textit{practice; training}&レン{\middel}シュウ \mbox{(REN{\middel}SH\={U})}\\\hline
\end{somme}
...
%&\mbox{()}\\\hline
\begin{somme}
展開&display + open = \textit{unfolding}&テン{\middel}カイ \mbox{(TEN{\middel}KAI)}\\\hline
...
面白い&mask + white = \textit{interesting}&おも{\wsplit}しろ{\middel}い \mbox{(omo{\middel}shiro{\middel}i)}\\\hline
面接&mask + join = \textit{interview}&メン{\middel}セツ \mbox{(MEN{\middel}SETSU)}\\\hline
面会&mask + meet = \textit{meeting (face-to-face)}&メン{\middel}カイ \mbox{(MEN{\middel}KAI)}\\\hline
...
場面&place + mask = \textit{scene}&ば{\middel}メン \mbox{(ba{\middel}MEN)}\\\hline
\end{somme}
...
%&\mbox{()}\\\hline
\begin{somme}
\splitter{選ぶ}{tr}&\textit{to select/choose}&えら{\middel}ぶ \mbox{(era{\middel}bu)}\\\hline
選集&select + gather = \textit{selected extracts}&セン{\middel}シュウ \mbox{(SEN{\middel}SH\={U})}\\\hline
...
選局&select + office = \textit{channel selection}&セン{\middel}キョク \mbox{(SEN{\middel}KYOKU)}\\\hline
...
\comprthreecone&Knife + (Broom = Pig head + Cover + Cloth)\\\hline
\rye{2}{\hooglig{The broom} is a \remcom{pig's snout} covered in \remcom{cloth}.}
\end{comps}

\begin{remglyph}
\hooglig{Homeward}, you'll find a \textit{sword} and a \textit{broom}.\\\hline
\end{remglyph}
...
%&\mbox{()}\\\hline
\begin{somme}
\splitter{持ち帰り}{tr}&hold + homeward = \textit{to bring back}&も{\middel}ち{\wsplit}かえ{\middel}り \mbox{(mo{\middel}chi{\wsplit}kae{\middel}ri)}\\\hline
\splitter{帰る}{intr}&\textit{to go back}&かえ{\middel}る \mbox{(kae{\middel}ru)}\\\hline
\splitter{帰す}{tr}&\textit{to send someone back}&かえ{\middel}す \mbox{(kae{\middel}su)}\\\hline
帰り&\textit{return; coming back}&かえ{\middel}り \mbox{(kae{\middel}ri)}\\\hline
日帰り&sun + homeward = \textit{day trip}&ひ{\wsplit}がえ{\middel}り \mbox{(hi{\wsplit}gae{\middel}ri)}\\\hline
...
大部分&large + part = \textit{most part; majority}&ダイ{\middel}ブ{\middel}ブン \mbox{(DAI{\middel}BU{\middel}BUN)}\\\hline
...
\notyet{部長}&section + long = \textit{head of a department}&ブ{\middel}チョウ \mbox{(BU{\middel}CH\={O})}\\\hline
\end{somme}
...
\begin{remglyph}
\hooglig{Topic} of \remcom{approval} includes \remcom{face-recognition}.\\\hline
\end{remglyph}
...
問題点&problem + point = \textit{the point at issue}&モン{\middel}ダイ{\middel}テン \mbox{(MON{\middel}DAI{\middel}TEN)}\\\hline
...
%&\mbox{()}\\\hline
\begin{somme}
時期&time + period = \textit{time; season}&ジ{\middel}キ \mbox{(JI{\middel}KI)}\\\hline
...
%予期&beforehand + period = \textit{forecast; expectation}&ヨ{\middel}キ \mbox{(YO{\middel}KI)}\\\hline
...
\begin{remglyph}
\hooglig{Grant} your \remcom{actions} to those who should \remcom{receive}.\\\hline
\end{remglyph}
...
%&\mbox{()}\\\hline
\begin{somme}
\splitter{授ける}{tr}&\textit{to grant/give}&さず{\middel}ける \mbox{(sazu{\middel}keru)}\\\hline
\splitter{授かる}{intr}&\textit{to be awarded}&さず{\middel}かる \mbox{(sazu{\middel}karu)}\\\hline
授受&grant + receive = \textit{giving and receiving}&ジュ{\middel}ジュ \mbox{(JU{\middel}JU)}\\\hline
授業&grant + business = \textit{lesson; instruction}&ジュ{\middel}ギョウ \mbox{(JU{\middel}GY\={O})}\\\hline
授業料&lesson + fee = \textit{tuition fee}&ジュ{\middel}ギョウ{\middel}リョウ \mbox{(JU{\middel}GY\={O}{\middel}RY\={O})}\\\hline
授与&grant + confer = \textit{awarding}&ジュ{\middel}ヨ \mbox{(JU{\middel}YO)}\\\hline
教授&teach + grant = \textit{teaching; professor}&キョウ{\middel}ジュ \mbox{(KY\={O}{\middel}JU)}\\\hline
授与&grant + confer = \textit{awarding; granting}&ジュ{\middel}ヨ \mbox{(JU{\middel}YO)}\\\hline
...
This character has an array of meanings, from \textsuperscript{[1]}\textit{postal services}, \textsuperscript{[2]}\textit{convenience} and \textsuperscript{[3]}\textit{opportunity to bodily excretion functions}.  Underlying everything, is a sense of \textit{delivering things} or of \textit{things being delivered} in some way.
\end{glyph}
...
%&\mbox{()}\\\hline
\begin{somme}
便り&\textit{news; tidings}&たよ{\middel}り \mbox{(tayo{\middel}ri)}\\\hline
...
不便&not + delivery = \textit{inconvenience}&フ{\middel}ベン \mbox{(FU{\middel}BEN)}\\\hline
...
\begin{comps}
\comprtwocone&止 + (戌 = 戊 + 一) + 小\\\hline
\comprthreecone&Stop + (Sign of the Dog = 5\textsuperscript{th} + One) + Small\\\hline
\rye{2}{\hooglig{The Sign of the Dog} is the \remcom{5\textsuperscript{th}} \remcom{one}.}
\end{comps}
...
歳時記&annual + time + note = \textit{almanac of seasonal words}&サイ{\middel}ジ{\middel}キ \mbox{(SAI{\middel}JI{\middel}KI)}\\\hline
\end{somme}
...
\begin{remglyph}
\hooglig{Exclusive} to \remcom{one} \remcom{cause} is the \remcom{size} problem.\\\hline
\end{remglyph}
...
%&\mbox{()}\\\hline
\begin{somme}
専ら&\textit{wholly; solely}&もっぱ{\middel}ら \mbox{(moppa{\middel}ra)}\\\hline
専門&exclusive + gate = \textit{speciality}&セン{\middel}モン \mbox{(SEN{\middel}MON)}\\\hline
専門家&speciality + house = \textit{expert}&セン{\middel}モン{\middel}カ \mbox{(SEN{\middel}MON{\middel}KA)}\\\hline
専売&exclusive + sell = \textit{monopoly}&セン{\middel}バイ \mbox{(SEN{\middel}BAI)}\\\hline
専用&exclusive + utilize = \textit{exclusive use}&セン{\middel}ヨウ \mbox{(SEN{\middel}Y\={O})}\\\hline
専用線&exclusive use + line = \textit{dedicated line}&セン{\middel}ヨウ{\middel}セン \mbox{(SEN{\middel}Y\={O}{\middel}SEN)}\\\hline
...
専心&exclusive + heart = \textit{undivided attention}&セン{\middel}シン \mbox{(SEN{\middel}SHIN)}\\\hline
...
To orient.  To look on.  The overall sense is of things either \textsuperscript{[1]}\textit{orienting themselves}, or \textsuperscript{[2]}\textit{being oriented} in a literal or figurative direction.\\\hline
...
夏向き&summer + orient = \textit{for summer}&なつ{\wsplit}む{\middel}き \mbox{(natsu{\wsplit}mu{\middel}ki)}\\\hline
向学心&orient + study + heart = \textit{love of learning}&コウ{\middel}ガク{\middel}シン \mbox{(K\={O}{\middel}GAKU{\middel}SHIN)}\\\hline
向上&orient + upper = \textit{elevation}&コウ{\middel}ジョウ \mbox{(K\={O}{\middel}J\={O})}\\\hline
意向&mind + orient = \textit{inclination}&イ{\middel}コウ \mbox{(I{\middel}K\={O})}\\\hline
方向&direction + orient = \textit{orientation}&ホウ{\middel}コウ \mbox{(H\={O}{\middel}K\={O})}\\\hline
動向&move + orient = \textit{trend; tendency}&ドウ{\middel}コウ \mbox{(D\={O}{\middel}K\={O})}\\\hline
...
練り&\textit{kneading; tempering}&ね{\middel}り \mbox{(ne{\middel}ri)}\\\hline
...
教練&teach + train = \textit{drill}&キョウ{\middel}レン \mbox{(KY\={O}{\middel}REN)}\\\hline
...
\begin{remglyph}
\hooglig{Raise} the \remcom{top} brass in \remcom{private} as if it's your own \remcom{flesh}.\\\hline
\end{remglyph}
...
\splitter{育つ}{intr}&\textit{to be brought up}&そだ{\middel}つ \mbox{(soda{\middel}tsu)}\\\hline
発育&discharge + raise = \textit{physical growth}&ハツ{\middel}イク \mbox{(HATSU{\middel}IKU)}\\\hline
育ち&\textit{growth; breeding}&そだ{\middel}ち \mbox{(soda{\middel}chi)}\\\hline
...
%&\mbox{()}\\\hline
\begin{somme}
\splitter{考える}{tr}&\textit{to consider; think about}&かんが{\middel}える \mbox{(kanga{\middel}eru)}\\\hline
\splitter{考え出す}{tr}&consider + exit = \textit{to think out}&かんが{\middel}え{\wsplit}だす \mbox{(kanga{\middel}e{\wsplit}dasu)}\\\hline
考え&\textit{thinking; thought}&かんが{\middel}え \mbox{(kanga{\middel}e)}\\\hline
思考&think + consider = \textit{thought; consideration}&シ{\middel}コウ \mbox{(SHI{\middel}K\={O})}\\\hline
考え方&consider + direction = \textit{way of thinking}&かんが{\middel}え{\wsplit}かた \mbox{(kanga{\middel}e{\wsplit}kata)}\\\hline
考古学&consider + old + study = \textit{archeology}&コウ{\middel}コ{\middel}ガク \mbox{(K\={O}{\middel}KO{\middel}GAKU)}\\\hline
選考&select + consider = \textit{selection; screening}&セン{\middel}コウ \mbox{(SEN{\middel}K\={O})}\\\hline
考え直す&consider + straight = \textit{to reconsider}&かんが{\middel}え{\wsplit}なお{\middel}す \mbox{(kanga{\middel}e{\wsplit}nao{\middel}su)}\\\hline
...
\begin{center}
	...
	{
{\includegraphics[width=3.2cm]{cargo.pdf}} &
{\includegraphics[width=3.2cm]{port.pdf}} &
{\includegraphics[width=3.2cm]{hit.pdf}} &
{\includegraphics[width=3.2cm]{row.pdf}} &
{\includegraphics[width=3.2cm]{round.pdf}} \\
{\includegraphics[width=3.2cm]{manor.pdf}} &
{\includegraphics[width=3.2cm]{fly.pdf}} &
{\includegraphics[width=3.2cm]{number.pdf}} &
{\includegraphics[width=3.2cm]{learn.pdf}} &
{\includegraphics[width=3.2cm]{display.pdf}} \\
{\includegraphics[width=3.2cm]{mask.pdf}} &
{\includegraphics[width=3.2cm]{select.pdf}} &
{\includegraphics[width=3.2cm]{homeward.pdf}} &
{\includegraphics[width=3.2cm]{section.pdf}} &
{\includegraphics[width=3.2cm]{topic.pdf}} \\
{\includegraphics[width=3.2cm]{period.pdf}} &
{\includegraphics[width=3.2cm]{grant.pdf}} &
{\includegraphics[width=3.2cm]{delivery.pdf}} &
{\includegraphics[width=3.2cm]{annual.pdf}} &
{\includegraphics[width=3.2cm]{exclusive.pdf}} \\
{\includegraphics[width=3.2cm]{orient.pdf}} &
{\includegraphics[width=3.2cm]{train.pdf}} &
{\includegraphics[width=3.2cm]{raise.pdf}} &
{\includegraphics[width=3.2cm]{consider.pdf}} &
{\includegraphics[width=3.2cm]{member.pdf}} \\
	};
	\end{tikzpicture}
\end{center}
\vspace*{\fill}