...
Cool, Cold.\\\hline
Objects that are cool or cold to the touch (the idea of cold weather, however, is generally conveyed by the kanji in Entry 446).
\end{glyph}
...
\begin{remglyph}
\hooglig{Cool} \remcom{command} to turn into \remcom{ice}.\\\hline
\end{remglyph}
...
寒冷 & cold + cool = \textit{cold; chilly} & カン{\middel}レイ \mbox{(KAN{\middel}REI)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
...
\begin{comps}
\comprtwocone & ニ + 丨 + 日 \\\hline
\comprthreecone & Two + Pole + Sun \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Long ago} there were \remcom{two} \remcom{suns} separated by a \remcom{pole}.\\\hline
\end{remglyph}
...
\vspace*{\fill}
\begin{multicols}{2}
\begin{glyph}{Medicine (薬)}{music}\label{glyph:music}
Medicine.\\\hline
...
\rye{3}{The kunyomi is always voiced outside of the first position, as in the second below.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}
...
\begin{remglyph}
\hooglig{Line} \remcom{threads} in the \remcom{springs}.\\\hline
\end{remglyph}
...
\begin{comps}
\comprtwocone & 厂 + 立 + 彡 + 頁 \\\hline
\comprthreecone & Cliff + Stand + Hair/Fur + Big shell \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Face} off \remcom{fur} \remcom{erected} on the \remcom{cliff} of \remcom{big shells}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ガン (GAN)} & \textbf{かお (kao)}\\\hline
\rye{3}{The kunyomi is almost always voiced when not in the first position, as in the third compound.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}
...
\begin{comps}
\comprtwocone & 方 + 丿乀乁 + 一 + 矢 \\\hline
\comprthreecone & Way/Direction + Slash + One + Arrow\\\hline
\end{comps}

\begin{remglyph}
\hooglig{Tribe} \remcom{locals} \remcom{slashing} \remcom{one} another with \remcom{arrows}.\\\hline
\end{remglyph}
...
\hooglig{水族館} & water + tribe + manor = \textit{(public aquarium)} & スイ{\middel}ゾク{\middel}カン \mbox{(SUI{\middel}ZOKU{\middel}KAN)}\\\hline
\end{somme}
\end{multicols}
\vspace*{\fill}
\pagebreak
...
Teach.\\\hline
A religious element can be present in this kanji, as shown in the second compound below.
\end{glyph}
...
\begin{remglyph}
Teach with the whip to instil filial piety.\\\hline
\end{remglyph}
...
Third.\\\hline
You will rarely meet this kanji on its own; its value for us is as a component in other more common characters.  It is, however, found in document of a more official nature, where it is used to indicate the third item or clause in a list.
\end{glyph}
...
\begin{comps}
\comprtwocone & 宀 + 白 + 人亻 \\\hline
\comprthreecone & Roof + White + Man/Person \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Lodge} with \remcom{people} under the \remcom{white} \remcom{roof}.\\\hline
\end{remglyph}
...
\begin{remglyph}
\hooglig{Try} to \remcom{type} what I \remcom{say}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{シ (SHI)} & \textbf{こころ、ため (kokoro, tame)}\\\hline
\rye{3}{The 2 kunyomi are verb stems for the first and second examples below.  Though they both mean to `try', 試す (ため{\middel}す/tame{\middel}su) has the added implication of testing something so as to ascertain its value or validity.}
\pronsrthreecone & --- & --- \\\hline
\end{prons}
...
% &  \mbox{()}\\\hline
\begin{somme}
\rye{3}{Note the mix ofr onyomi and kunyomi in the third compound below.}
...
This kanji conveys a sense of doing things anew, be it through revision, reform or renewal, amongst others.\\\hline
...
System.\\\hline
As can be seen from the final compounds below, this kanji also appears in words related to lineage and genealogy.
\end{glyph}
...
\begin{remglyph}
Part of the Kanji radicals (\#{209}).\\\hline
\end{remglyph}
...
This kanji can stand for Asia in the same way that 日 does for Japan and 中 does for Chine.\\\hline
...
\rye{3}{We met the first two of these compounds in Entries 6 and 85, as the readings of both kanji were irregular.  The third we encounter here, as とし is the normal kunyomi of 年.  Also note how the addition of thei hiragana は in the second example below (which creates the most famous of all Japanese greetings) can change the readings of both kanji from the first example above.}
\end{irrr}
...
Refer back to the kanji for ``water'' (Entry \#{61}) in order to see how the ``harpoon'' component of that character has become transformed in this one.
\end{glyph}
...
Ice.\\\hline
As you can see, this kanji is very similar visually to 永.  The only other thing to keep in mind here is that the stroke order of the `water' component is broken---the dot is written immediately after the initial harpoon strike.
\end{glyph}
