...
\comprtwocone & 言 + 売 \\\hline
    \comprthreecone & Say + Sell (\ref{glyph:sell}) \\\hline
\end{comps}
...
\begin{remprons}
\hooglig{Reading} about the \ucomon{talking} \comkun{yomp} moving
    \comon{dockward}.\\\hline
\end{remprons}
...
\begin{remglyph}
\hooglig{Road} travelling of with my \remcom{neck}-support.\\\hline
\end{remglyph}
...
\begin{remprons}
    The \hooglig{road} in \comkun{Michigan} is so \ucomon{torturous} that you'd
    rather open the \comon{door}, step out, and start walking.\\\hline
\end{remprons}
...
% &  ()\\\hline
\begin{somme}
    \rye{3}{Note how the 音読み for ``country'' is voiced in the first example.
    ...
    reading.}
    天国  & heaven + country = \textit{Heaven}  & テン{\middel}コク \mbox{(TEN{\middel}KOKU)} \\\hline
...
\begin{remprons}
    The \hooglig{thing} about this \comkun{monologue} is the \ucomon{motsurella}
    \comon{boots}.\\\hline
\end{remprons}
...
\begin{remglyph}
Part of the Kanji radicals (\#{69}).\\\hline
\end{remglyph}
...
\begin{remglyph}
\hooglig{New} \remcom{trees} can with\remcom{stand} \remcom{axes}.\\\hline
\end{remglyph}
...
\begin{remprons}
    \hooglig{Flowers} are \comkun{hungup} when they start \comon{cussing}.\\\hline
\end{remprons}
...
\begin{sentence}
    ...
    Nishi{\middel}da-san wa i{\middel}ke bana ga J\={O}{\middel}ZU desu ne.\\\hline
...
\begin{remglyph}
What is the \hooglig{name} of that \remcom{evening} \remcom{vampire}? \\\hline
\end{remglyph}
...
\begin{remglyph}
Part of the Kanji radicals (\#{80}).\\\hline
\end{remglyph}
...
\comprthreecone & Two-branch tree + Private \\\hline
\end{comps}
...
\begin{remglyph}
    At \hooglig{night} a \remcom{person} goes into
    \remcom{evening} mode to the \remcom{top}.\\\hline
\end{remglyph}
...
\begin{remprons}
    A \hooglig{night}-time \comkun{yomp} to the \comon{yach} with
    \comkun{Yoruichi}.\\\hline
\end{remprons}
...
Before/Front.\\\hline
    It is worth comparing the compounds below to those for 先 (Entry \ref{glyph:precede}) to get a sense of the
...
\begin{remglyph}
The \hooglig{writing}-\remcom{brush} is dipped into the \remcom{sun}.\\\hline
\end{remglyph}
...
\rye{3}{The third and the firth examples below show that this kanji can act like
    買.  Be aware that the 訓読み is always voiced when it appears in the
    ...
    書き入れ  & write + enter = \textit{an entry}  & か{\middel}き　い{\middel}れ \mbox{(ka{\middel}ki i{\middel}re)} \\\hline
    書道  & write + road = \textit{calligraphy}  & ショ{\middel}ドウ \mbox{(SHO{\middel}D\={O})} \\\hline
    前書  & before + write = \textit{preface}  & まえ{\middel}がき \mbox{(mae{\middel}gaki)}  \\\hline
\end{somme}
...
\begin{remglyph}
Part of the Kanji radicals (\#{180}).\\\hline
\end{remglyph}
...
\begin{remprons}
The \hooglig{sound} was an \ucomon{inkling} of a \ucomkun{necromancer}
...
\begin{somme}
    ...
    both kanji are read with their 音読み.}
...
\begin{sentence}
    ...
    are wa mizu no oto da to omo{\middel}imasu.\\\hline
...
\begin{remglyph}
    \hooglig{Meet} \remcom{two} \remcom{people} in \remcom{private}.\\\hline
\end{remglyph}
...
\begin{somme}
    ...
    大会  & large + meet = \textit{convention; tournament}  & タイ{\middel}カイ \mbox{(TAI{\middel}KAI)} \\\hline
...
\begin{sentence}
    ...
    BEI{\middel}KOKU{\middel}TAI{\middel}KAI wa KU{\middel}GATSU ni arimasu ka.\\\hline
...
\begin{glyph}{Hear (聞)}{hear}\label{glyph:hear}
    Hear/Ask.  Like 早 (Entry \ref{glyph:early}), this is another kanji with two unrelated and
    distinct meanings.
\end{glyph}
