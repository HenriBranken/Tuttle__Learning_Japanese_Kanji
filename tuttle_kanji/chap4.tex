...
水玉 & water + jewel = \textit{water droplet} & みず{\middel}だま (mizu{\middel}dama)\\\hline
    ...
    \notyet{水曜日} & water + day of the week + sun (day) = \textit{Wednesday} & スイ{\middel}ヨウ{\middel}び (SUI{\middel}Y\={O}{\middel}bi)\\\hline
...
\begin{remglyph}
\hooglig{Summer} will \remcom{overtake} and establish \remcom{itself}
    \remcom{one} day.\\\hline
\end{remglyph}
...
\comprthreecone & NO katakana/slash + Ten (\ref{glyph:thousand}) \\\hline
\end{comps}
...
\begin{remglyph}
\hooglig{Resting} \remcom{people} under the shade of \remcom{trees}.\\\hline
\end{remglyph}
...
\comprtwocone & 亻 + 匕 \\\hline
    \comprthreecone & Person [variant] (\ref{glyph:person}) + spoon/ladle \\\hline
\end{comps}
...
\begin{remglyph}
\textit{Obvious from the glyph itself.}\\\hline
\end{remglyph}
...
\begin{remglyph}
    \textit{Somewhat obvious from the kanji itself.}\\\hline
\end{remglyph}
...
\comprthreecone & roof/cover + pig\\\hline
\end{comps}
...
\begin{remglyph}
    \textit{Sound waves traveling outward when you say something.}\\\hline
\end{remglyph}
