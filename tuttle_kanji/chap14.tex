...
Roof.\\\hline
When this 漢字 is the final character in a compound, it will often mean a ``shop'' when referring to a building, or a ``dealer'' when associated with a person.
\end{glyph}
...
\rye{3}{や is by far the more common reading, and is invariably used when this 漢字 appears in the final position of a compound.  There is often a mixture of 音読み and 訓読み in such words (as in the final 2 examples below).}
...
\begin{remglyph}
\hooglig{Form} \remcom{a} \remcom{two-handed} \remcom{feather}.\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{ケイ (KEI)} & \textbf{かたち、かた (katachi, kata)}\\\hline
\rye{3}{かたち is the choice when this 漢字 appears on its own; in such a case it means ``form'' or ``shape''.  In compounds, look for ケイ in the first position (with a notable exception being the second example below), and a split between ケイ and かた after that, keeping in mind that かた is almost always voiced in such instances.}
\pronsrthreecone & ギョウ (GY\={O}) & --- \\\hline
\rye{3}{This is a very uncommon reading that you are only likely to meet in the word for ``doll'', 人形 (NIN{\middel}GY\={O}/ニン{\middel}ギョウ) from the 漢字 for `person' and `form'.}
\end{prons}
...
{\warn}形見 & form + see = \textit{memento; keepsake} & かた{\middel}み \mbox{(kata{\middel}mi)}\\\hline
...
Target.\\\hline
This is an important character that is used as a suffix to express such ideas as the English equivalent of ``-istic'' in word such as ``art\textit{istic}''.\\\hline
If you spend some time getting to know this 漢字, you will quickly notice your vocabulary growing in sophistication.
\end{glyph}
...
Arrow.\\\hline
As these are the same two components used to make the 漢字 for ``Lose'' (Entry \ref{glyph:lose}, 失), ensure that you are able to tell them apart.
\end{glyph}
...
\begin{remglyph}
\hooglig{The world} of \remcom{rice fields} needs an \remcom{intervention}.\\\hline
\end{remglyph}
...
\begin{remglyph}
\hooglig{Forget} your \remcom{concealed} \remcom{heart}.\\\hline
\end{remglyph}
...
\begin{remprons}
\hooglig{Moreover}, you should let \comkun{customers} \ucomon{shop}.\\\hline
\end{remprons}
...
\end{glyph}
\gindex{Besides}{都}\strindex{11}{都}

\begin{comps}
\comprtwocone & 者 + ${\cdot}$⻏⾢  \\\hline
...
\rye{3}{You will only meet this 音読み in two compounds: the first being 都合 (ツ{\middel}ゴウ/TSU{\middel}G\={O}) ``circumstances''/``arrangements'', composed of the 漢字 `metropolis' and `match'.\\\hline
The second compound is 都度 (ツ{\middel}ド/TSU{\middel}DO) ``each time''/``whenever'' from the 漢字 `metropolis' and `degree'.}
\end{prons}
...
\begin{comps}
\comprtwocone & 十 + 阝${\cdot}$阜 + 夂 \\\hline
...
\rye{3}{This reading forms an intransitive/transitive verb pair (降りる/降ろす) that has the same pronunciation as one we met earlier with the 漢字 下 (Entry \ref{glyph:lower}): 下りる/下ろす.  The characters are sometimes used interchangeably, although you are more likely to see them written as 下りる and 下ろす.}
\end{prons}
...
\begin{remglyph}
\hooglig{Marrying} a \remcom{woman} into a \remcom{clan} on a certain \remcom{day}.\\\hline
\end{remglyph}
...
\begin{remglyph}
\hooglig{Festivals} during full \remcom{moon} are \remcom{again} on \remcom{display}.\\\hline
\end{remglyph}
...
\begin{remglyph}
\hooglig{Fingers} show the \remcom{gist} of your \remcom{hand}.\\\hline
\end{remglyph}
...
\comprthreecone & Hook + MA カタカナ\\\hline
\end{comps}
...
A secondary meaning related to Buddhist sutras make use of the less common 音読み.
\end{glyph}
...
he&は&Honolulu&by way of&で&to return\\\hline
...
\begin{remglyph}
The \hooglig{head} is \remcom{a kind of a vessel}, like a \remcom{big shell}\ldots\\\hline
\end{remglyph}

\begin{prons}
\pronsrtwocone & \textbf{トウ (T\={O})} & \textbf{あたま (atama)}\\\hline
\pronsrthreecone & ズ、ト (ZU, TO) & かしら (kashira) \\\hline
\rye{3}{You are most likely to encounter ズ in only a pair of words: 頭上 (ズ{\middel}ジョウ/ZU{\middel}J\={O}) ``overhead'', from the 漢字 `head' and `upper', and the final sample compound of Entry \ref{glyph:pain}.}
\end{prons}
...
This 漢字 has to do with the actual nature or reality of things.
\end{glyph}
