...
\rye{3}{You will only meet this on-yomi in two compounds: the first being 都合 (ツ{\middel}ゴウ/TSU{\middel}G\={O}) \textit{``circumstances''}, \textit{``arrangements''}, composed of the 漢字 ``metropolis'' and ``match''.\\\hline
The second compound is 都度 (ツ{\middel}ド/TSU{\middel}DO) \textit{``each time''}/\textit{``whenever''} from the 漢字 ``metropolis'' and ``degree''.}
\end{prons}
...
\rye{3}{This reading forms an intransitive/transitive verb pair (降りる/降ろす) that has the same pronunciation as one we met earlier with the 漢字 下 (Entry 31): 下りる/下ろす.  The characters are sometimes used interchangeably, although you are more likely to see then written as 下りる and 下ろす.}
\end{prons}
