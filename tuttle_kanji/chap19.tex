...
\rye{3}{This reading is found in only one compound, which we met back at Entry \ref{glyph:how_many}: 幾何学 (キ{\middel}カ{\middel}ガク/KI{\middel}KA{\middel}GAKU) \textit{`geometry'}, from the 漢字 `how many' + `what' + `study'.}
\end{prons}
...
\begin{remglyph}
\hooglig{So busy} it feels like my \remcom{heart} is \remcom{perishing}.\\\hline
\end{remglyph}
...
\begin{remglyph}
\hooglig{Slowly} \remcom{walking} beside the \remcom{sheep} \remcom{corpse}.\\\hline
\end{remglyph}
...
Fee.  Charge.\\\hline
Another term relates to ``materials'' (as the second compound of Entry \ref{glyph:history} provides an example).\\\hline
...
\comprthreecone & Lined Up + Grass + Not Yet \\\hline
\end{comps}

\begin{remglyph}
\hooglig{Businesses} \remcom{lined up} for the \remcom{grass} that has \remcom{not yet} grown.\\\hline
\end{remglyph}
