...
What, although the sense of `how many' can also be expressed in certain compounds (such as the first example of Entry \ref{glyph:raise}).\\\hline
There is, incidentally, only one character in the 2136 general-use list that uses this 漢字 as a component---you will meet it in Entry \ref{glyph:cargo}.\\\hline
...
\rye{3}{This reading is found in only one compound, which we met back at Entry \ref{glyph:how_many}: 幾何学 (キ{\middel}カ{\middel}ガク/KI{\middel}KA{\middel}GAKU) \textit{`geometry'}, from the 漢字 `how many', `what', and `study'.}
\end{prons}
...
Ensure that you can clearly distinguish this 漢字 from 科 (`course', Entry \ref{glyph:course}).
\end{glyph}
...
\rye{3}{You will only encounter this reading as an alternate to the second compound below: 黄金 (こ{\middel}がね/ko{\middel}gane) \textit{`gold'} or \textit{`money'}.  Nevertheless, it is far more common for 黄金色 (こ{\middel}がね{\middel}いろ/ko{\middel}gane{\middel}iro) \textit{`gold color'}, to be read with the kunyomi of these 漢字.}
\end{prons}
...
This 漢字 also conveys the idea of `opportunity' or `chance' in some of the compounds in which it appears (as in the first compound).\\\hline
...
Company.\\\hline
This 漢字 also refers to Shinto shrines; note how the second compound below can encompass both meanings.
\end{glyph}
